...
internal object GraphEdgeHistoryWriter {
    ...
    fun append(
        ...
        var chain = operation.sql { session ->
            session.createNativeQuery(
                "select set_config('openbank.bank_scope', :bank, true)",
                String::class.java,
            ).setParameter("bank", bankScope).singleResult
        }.flatMap {
            appendEvent(operation, bankScope, generation, eventKey, aggregateRef, recordedAt, hash)
        }
        sorted.forEach { edge ->
            chain = chain.flatMap {
                appendEdge(operation, bankScope, generation, eventKey, aggregateRef, edge, recordedAt)
            }
        }
        return chain
    }
    ...
    private fun appendEvent(
        ...
        recordedAt: Instant,
        hash: String,
    ): Uni<Void> = operation.sql { session ->
        ...
        operation.sql { session ->
            session.createNativeQuery(
                ...
                .singleResult
        }.invoke { stored -> check(stored == hash) { "conflicting graph edge event" } }
            .replaceWithVoid()
    }
        ...
        return operation.sql { session ->
            session.createNativeMutationQuery(
                ...
                .setParameter(
                    "generation",
                    generation,
                ).setParameter("fromKey", edge.from).setParameter("toKey", edge.to)
                .setParameter("relation", edge.relation).setParameter("source", edge.source)
                .setParameter(
                    "aggregate",
                    aggregate,
                ).setParameter("evidence", evidence).setParameter("at", edge.validFrom)
            ...
            operation.sql { session ->
                session.createNativeQuery(
                    ...
                    String::class.java,
                ).setParameter("id", observationId).singleResult
            }.invoke { stored ->
                ...
                operation.sql { session ->
                    session.createNativeMutationQuery(
                        ...
                        .setParameter("source", edge.source).executeUpdate()
                }.replaceWithVoid()
            }
        }
    }
