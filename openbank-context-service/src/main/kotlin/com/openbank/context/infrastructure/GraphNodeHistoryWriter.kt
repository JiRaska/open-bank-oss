...
internal object GraphNodeHistoryWriter {
    ...
    fun append(
        ...
        nodes.forEach { node ->
            chain = chain.flatMap {
                appendNode(operation, bankScope, generation, eventKey, aggregateRef, node, recordedAt)
            }
        }
        return chain
    }
    ...
    private fun appendEvent(
        ...
        operation.sql { session ->
            session.createNativeQuery(
                ...
                .singleResult
        }.invoke { stored -> check(stored == hash) { "conflicting graph node revision" } }
            .replaceWithVoid()
    }
        ...
        return operation.sql { session ->
            session.createNativeMutationQuery(
                ...
                .setParameter(
                    "evidence",
                    evidence,
                ).setParameter("label", node.label).setParameter("at", node.occurredAt)
                .setParameter("recorded", recordedAt).setParameter("version", node.version)
                .setParameter("owner", node.authoritative).setParameter("hash", hash).executeUpdate()
        }.flatMap {
            operation.sql { session ->
                session.createNativeQuery(
