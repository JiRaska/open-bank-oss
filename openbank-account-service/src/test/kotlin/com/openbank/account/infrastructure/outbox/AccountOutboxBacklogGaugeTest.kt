 ...
 */
class AccountOutboxBacklogGaugeTest {

    @Test
    fun `registers an account backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<AccountOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("account"), capture(supplier)) } returns Unit

        val gauge = AccountOutboxBacklogGauge(repo, metrics)
        gauge.register()
