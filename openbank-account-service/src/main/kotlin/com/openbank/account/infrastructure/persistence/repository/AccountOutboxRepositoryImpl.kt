...
import com.openbank.account.infrastructure.persistence.entity.AccountOutboxEntity
...
@ApplicationScoped
class AccountOutboxRepositoryImpl(private val clock: Clock) :
    AccountOutboxRepository,
    PanacheRepository<AccountOutboxEntity> {
    ...
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, AccountOutboxEntity::class.java)
