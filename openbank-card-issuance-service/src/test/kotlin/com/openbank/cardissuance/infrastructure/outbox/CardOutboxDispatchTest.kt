 ...
 *    ([OutboxFailurePolicy]).
 *  - N2: the partition key is the aggregate id ([OutboxKafkaHeaders.partitionKey]).
 *  - N3: the produced headers carry the event id (ce-id / idempotency-key) and the event type
 *    (ce-type) ([OutboxKafkaHeaders.headersFor]).
 */
class CardOutboxDispatchTest {

    private fun entry(eventType: String = "card.issued.v1") = OutboxEntry(
    ...
    fun `headers carry the event id and event type (N3)`() {
        val e = entry(eventType = "card.status_changed.v1")
