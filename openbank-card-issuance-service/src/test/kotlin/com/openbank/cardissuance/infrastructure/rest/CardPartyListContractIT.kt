...
class CardPartyListContractIT {
    @Test
    @TestSecurity(user = "00000000-0000-0000-0000-000000000099", roles = ["ROLE_ADMIN"])
    fun `bounded party card read preserves the array response contract`() {
        Given {
            queryParam("limit", 51)
        } When {
            get("/api/v1/cards/party/${UUID.randomUUID()}")
        } Then {
            statusCode(200)
            body("size()", equalTo(0))
        }
    ...
    @Test
    @TestSecurity(user = "00000000-0000-0000-0000-000000000099", roles = ["ROLE_ADMIN"])
    fun `bounded party card read rejects invalid limit`() {
        Given {
            queryParam("limit", 0)
        } When {
            get("/api/v1/cards/party/${UUID.randomUUID()}")
