...
/**
 * Regression guard (C7/ADR-0030): no card-issuance endpoint must be opened without role-gating.
 * Pure reflection — no Quarkus boot needed.
 */
class CardSecurityTest {

    @Test
    fun `no CardResource endpoint is @PermitAll`() {
        val methods = CardResource::class.java.declaredMethods.filter { m ->
