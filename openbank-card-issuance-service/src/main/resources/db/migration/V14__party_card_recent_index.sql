...
-- Supports the bounded Customer 360 party card read. The existing party-only
...
-- Rollback after rolling back the bounded read: DROP INDEX IF EXISTS idx_cards_party_recent;
CREATE INDEX IF NOT EXISTS idx_cards_party_recent
    ON cards (party_id, created_at DESC, id DESC);
