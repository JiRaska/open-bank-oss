 ...
 */
class ScaOutboxBacklogGaugeTest {

    @Test
    fun `registers an sca backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<ScaOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("sca"), capture(supplier)) } returns Unit

        val gauge = ScaOutboxBacklogGauge(repo, metrics)
        gauge.register()
