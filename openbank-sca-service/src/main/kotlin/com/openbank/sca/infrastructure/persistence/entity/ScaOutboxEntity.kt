...
/**
 * `claimed_at` is sca-only — added straight on this entity, not the shared
 ...
 * `ScaOutboxRepositoryImpl.claimProcessable`'s atomic claim query on DISPATCHING; read back
 * by the same query to decide if a DISPATCHING row is stale enough to reclaim.
 */
@Entity
@Table(name = "sca_outbox")
class ScaOutboxEntity : PanacheOutboxEntity() {
