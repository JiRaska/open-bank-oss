...
import com.openbank.sepainstant.application.port.`in`.*
import com.openbank.sepainstant.infrastructure.rest.dto.*
...
class SctInstResource @Inject constructor(
    ...
    fun listByDebtor(
        ...
        getUseCase.listByDebtor(debtorAccountId, page, size).map { list ->
            Response.ok(list.map(::toResponse)).build()
        }
