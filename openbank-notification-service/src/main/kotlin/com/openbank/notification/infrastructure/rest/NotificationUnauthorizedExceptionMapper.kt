 ...
 *
 * Without this, an anonymous caller refused by `@RolesAllowed` gets RESTEasy's built-in plain-text
 * body `Not Authenticated` under the negotiated `application/json` content type, breaking any
 * client's JSON parser (#8993, #9009).
 */
@Provider
class NotificationUnauthorizedExceptionMapper : UnauthorizedExceptionMapper()
