...
/**
 * Publishes the notification outbox backlog as `openbank.outbox.backlog{service="notification"}`
 * (ADR-0077 / ADR-0079). A rising backlog means notification events are stuck on their way to Kafka.
 * All the logic lives in [AbstractOutboxBacklogGauge] (ADR-0049 consolidation); this bean only binds
 * the notification repository + the CDI/scheduler annotations (which, per ADR-0013, must sit on the
 * concrete bean).
 */
@Startup
@ApplicationScoped
class NotificationOutboxBacklogGauge : AbstractOutboxBacklogGauge {
    private lateinit var outboxRepository: NotificationOutboxRepository

    @Inject
    constructor(outboxRepository: NotificationOutboxRepository, metrics: DomainMetrics) : super(metrics) {
    ...
    override val service: String = "notification"
