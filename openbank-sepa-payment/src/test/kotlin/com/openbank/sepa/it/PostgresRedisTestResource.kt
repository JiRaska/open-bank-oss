 ...
 * defaults.
 *
 * Both are needed for [com.openbank.sepa.integration.SepaPaymentBootSmokeIT] to reach a green
 ...
 * Valkey is needed too. The single `@Channel("events-out")` Kafka emitter is switched to the
 * in-memory connector in the IT, so no broker container is required.
 *
...
class PostgresRedisTestResource : QuarkusTestResourceLifecycleManager {
    ...
    override fun start(): Map<String, String> {
        val pg = PostgreSQLContainer(DockerImageName.parse("postgres:16.3-alpine"))
            ...
            .withDatabaseName("openbank_sepa_payment_it")
        pg.start()
        postgres = pg
        ...
        return mapOf(
            "quarkus.datasource.reactive.url" to
                "vertx-reactive:postgresql://$pgHost:$pgPort/openbank_sepa_payment_it",
            "quarkus.datasource.jdbc.url" to
                "jdbc:postgresql://$pgHost:$pgPort/openbank_sepa_payment_it",
