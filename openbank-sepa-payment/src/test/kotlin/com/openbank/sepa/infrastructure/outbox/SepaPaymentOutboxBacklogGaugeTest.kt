 ...
 */
class SepaPaymentOutboxBacklogGaugeTest {

    @Test
    fun `registers a sepa-payment backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<SepaPaymentOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("sepa-payment"), capture(supplier)) } returns Unit

        val gauge = SepaPaymentOutboxBacklogGauge(repo, metrics)
        gauge.register()
