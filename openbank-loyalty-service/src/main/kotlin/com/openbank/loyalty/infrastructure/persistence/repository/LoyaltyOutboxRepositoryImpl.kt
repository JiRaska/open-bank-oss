...
/**
 * Reference implementation, same atomic `FOR UPDATE SKIP LOCKED` claim query, stale-claim reclaim
 * and failure policy as `LedgerOutboxRepositoryImpl` / `EngagementOutboxRepositoryImpl` (#1201).
 * See that KDoc for the cross-pod claim reasoning; not repeated here.
 *
 * [persistInTransaction] is the seam every write path in this service uses: the caller is already
 * inside `Panache.withTransaction`, so the domain row and its outbox row commit together or not
 * at all (ADR-0050).
 */
@ApplicationScoped
class LoyaltyOutboxRepositoryImpl(private val clock: Clock) :
    ...
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, LoyaltyOutboxEntity::class.java)
    ...
    override suspend fun markFailed(eventId: UUID, error: String, failedAt: Instant): OutboxStatus =
        Panache.withTransaction {
            find("eventId", eventId).firstResult().map { e ->
                // A row that is not found is unreachable in practice — the dispatcher only calls
                // this on a row it just claimed — but degrade rather than throw out of a batch
                // that is otherwise mid-flight (#5128 finding 3).
                if (e != null) applyFailure(e, error, failedAt) else OutboxStatus.FAILED
            }
        }.awaitSuspending()

    private fun applyFailure(e: LoyaltyOutboxEntity, error: String, at: Instant): OutboxStatus {
        e.attemptCount += 1
        ...
        if (next == OutboxStatus.DEAD) {
            log.warnf(
                "loyalty.outbox.dead event_id=%s aggregate_id=%s event_type=%s attempts=%d last_error=%s",
                e.eventId,
                e.aggregateId,
                e.eventType,
                e.attemptCount,
    ...
    companion object {
        private val log: Logger = Logger.getLogger(LoyaltyOutboxRepositoryImpl::class.java)

        @Suppress("MaxLineLength")
        private const val CLAIM_SQL = """
            UPDATE loyalty_outbox
