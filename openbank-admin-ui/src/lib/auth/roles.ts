...
export const PERMISSIONS = {
  ...
  // Business onboarding (ADR-0284). `attest` confirms how a legal entity is represented — how many
  // signatures a framework agreement needs, and from which offices — so it mirrors kyb-service's
  // own @RolesAllowed(OPERATOR, ADMIN, KYC) on those routes. Nav/route gating only: the BFF relays
  // the operator's own bearer and kyb-service + OPA decide the real answer.
  "business-onboarding:view":   [ROLES.ADMIN, ROLES.OPERATOR, ROLES.COMPLIANCE, ROLES.KYC],
