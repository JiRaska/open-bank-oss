...
export function ConfirmTransitionDialog({
  ...
  // Radix restores focus to a Dialog.Trigger on close, but this dialog is mounted
  // conditionally by the caller rather than opened from a Trigger — so there is no
  // trigger ref for Radix to find, and its built-in restore is a silent no-op.
  // Capture the element that had focus when this dialog first rendered (the
  // caller's own transition button) and return focus to it ourselves.
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          onCloseAutoFocus={e => { e.preventDefault(); previousFocusRef.current?.focus() }}
          style={{
            position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
            zIndex: 60, width: '100%', maxWidth: '460px', padding: '22px 24px',
            background: 'var(--surface-1)', borderRadius: 'var(--radius, 8px)',
          }}
          className="card"
        >
          <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', marginBottom: '14px' }}>
            ...
            <div>
              <Dialog.Title style={{ fontSize: '15px', fontWeight: 700, color: 'var(--text-primary)' }}>
                {label}
              </Dialog.Title>
              <Dialog.Description style={{ fontSize: '12px', color: 'var(--text-secondary)', marginTop: '4px' }}>
