...
function BpmnDiagram({ process }: { process: BpmnProcess }) {
  ...
  const renderNode = (step: BpmnStep) => {
    ...
    if (type === 'end') return (
      <g key={step.id}>
        <circle cx={x} cy={y} r={18} fill="var(--map-core)" stroke="var(--info-text)" strokeWidth="3" />
        <text x={x} y={y + 32} textAnchor="middle" fontSize="9" fill="var(--text-secondary)">{label}</text>
      </g>
    )
        ...
        return (
          <g key={i}>
            <line x1={from.x} y1={from.y} x2={to.x} y2={to.y}
              stroke={isAsync ? ASYNC_COLOR : 'var(--text-tertiary)'}
              strokeWidth="1.5"
            ...
            {caption && (
              <text x={mx} y={my - 5} textAnchor="middle"
                ...
                fill={isAsync ? ASYNC_COLOR : 'var(--text-tertiary)'}>{caption}</text>
            )}
          </g>
        )
      })}
              ...
              <div>
                ...
                {events.map((e) => (
                  <div key={e.dir + e.topic} style={{
                    ...
                    color: ASYNC_COLOR, background: 'var(--accent-bg)', border: `1px solid color-mix(in srgb, ${ASYNC_COLOR} 20%, transparent)`,
    ...
    <div>
        ...
        {processes.map((p) => (
          <button key={p.slug} type="button" aria-pressed={active === p.slug} onClick={() => setActive(p.slug)}
            style={{
              ...
              color: active === p.slug ? 'var(--on-accent)' : 'var(--text-secondary)',
      ...
      <div className="card" style={{ padding: '24px', marginBottom: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div style={{
            // The regulation chip carries 11px text, so its semantic foreground/background
            // pair is governed by the shared contrast gate in both themes (#9749).
      ...
      <div className="card" style={{ padding: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          {[
            ...
            { shape: 'circle', color: 'var(--map-core)', label: 'End event' },
            { shape: 'circle', color: 'var(--danger)', label: 'Error end' },
            { shape: 'diamond', color: 'var(--warning)', label: 'Gateway (rozhodnutí)' },
