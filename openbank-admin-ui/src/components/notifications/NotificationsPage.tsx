 ...
 */
export function OperatorMessageApprovals() {
  const { t } = useLanguage()
  ...
  const decide = async () => {
    if (!pendingDecision || decisionRequestRef.current) return
    decisionRequestRef.current = true
    setBusy(true); setResult(null)
    try {
      const decision = await opsMessageApi.decide(pendingDecision.id, pendingDecision.approve)
      ...
      decisionRequestRef.current = false
      setBusy(false)
    }
  }
  ...
  return (
    <div className="card" style={{ overflow: 'hidden', marginBottom: '16px' }}>
      ...
      <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        {result?.ok && (
          <span role="status" aria-live="polite" style={{ fontSize: '12px', color: 'var(--green)' }}>{result.text}</span>
        )}
      </div>
      {pendingDecision && <div
        ref={dialogRef}
        role="alertdialog"
        aria-modal="true"
        aria-labelledby="notification-decision-title"
      ...
      ><div className="card" style={{ width: 'min(560px, 100%)', maxHeight: 'calc(100dvh - 40px)', overflowY: 'auto', padding: 22 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          <AlertTriangle aria-hidden="true" size={19} style={{ color: pendingDecision.approve ? 'var(--success)' : 'var(--danger)', flexShrink: 0, marginTop: 2 }} />
          <div>
            <h2 id="notification-decision-title" style={{ margin: 0, fontSize: 17, fontWeight: 750 }}>
              {pendingDecision.approve
                ? t('Schválit odeslání této zprávy?', 'Approve sending this message?')
                : t('Zamítnout odeslání této zprávy?', 'Reject sending this message?')}
            </h2>
            <p id="notification-decision-impact" style={{ margin: '6px 0 0', color: 'var(--text-secondary)', fontSize: 12.5, lineHeight: 1.55 }}>
              {pendingDecision.approve
                ? t('Po potvrzení může služba zprávu předat do doručovacího kanálu. Tuto akci používejte jen po kontrole obsahu a příjemce v původním požadavku.', 'After confirmation, the service may hand the message to its delivery channel. Use this action only after reviewing the content and recipient in the original request.')
                : t('Po potvrzení nebude tento požadavek na zprávu odeslán. Zamítnutí se vztahuje pouze k níže uvedenému ID.', 'After confirmation, this message request will not be sent. The rejection applies only to the approval ID below.')}
            </p>
          </div>
        </div>
        <div style={{ marginTop: 14, padding: '11px 12px', border: '1px solid var(--border)', borderRadius: 8, background: 'var(--surface-2)' }}>
          <div style={{ fontSize: 11, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '.06em' }}>{t('ID schválení', 'Approval ID')}</div>
          <code style={{ display: 'block', marginTop: 5, overflowWrap: 'anywhere', fontSize: 12.5 }}>{pendingDecision.id}</code>
        </div>
        <p id="notification-decision-governance" style={{ margin: '12px 0 0', padding: '10px 12px', borderRadius: 8, color: 'var(--warning-text)', background: 'var(--warning-bg)', border: '1px solid var(--warning-border)', fontSize: 12, lineHeight: 1.5 }}>
          {t('Služba znovu ověří vaše oprávnění a odmítne schválení vlastní zprávy. Konzole tuto kontrolu nenahrazuje.', 'The service re-checks your permission and refuses self-approval. This console does not replace that control.')}
        </p>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 18, flexWrap: 'wrap' }}>
          ...
          <button
            type="button"
            className={pendingDecision.approve ? 'btn btn-primary' : 'btn btn-danger'}
            disabled={busy}
            aria-busy={busy}
            onClick={() => void decide()}
          >
            {busy
              ...
              : pendingDecision.approve
                ? t('Potvrdit schválení', 'Confirm approval')
                : t('Potvrdit zamítnutí', 'Confirm rejection')}
          </button>
        </div>
      </div></div>}
    </div>
