...
export function TestIntelligenceFlow({ report }: { report?: TestIntelligenceReport | null }) {
  ...
  return <section className="ti-system" aria-labelledby="test-intelligence-flow-title">
    ...
    <style jsx>{`
      ...
      .ti-signals{display:grid;grid-template-columns:repeat(6,1fr);gap:8px;margin-top:10px}.ti-signals>div{display:grid;grid-template-columns:auto 1fr;gap:2px 7px;align-items:center;padding:9px 11px;border-radius:10px;background:rgba(148,163,184,.07);color:#7dd3fc}.ti-signals span{font-size:8px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted)}.ti-signals strong{grid-column:2;font-size:12px;color:var(--ink)}.ti-signals .state-passed{color:#4ade80}.ti-signals .state-failed{color:#f87171}.ti-signals .state-not-run,.ti-signals .state-unknown{color:#94a3b8}
