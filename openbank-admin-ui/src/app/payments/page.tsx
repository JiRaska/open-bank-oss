...
function PaymentsContent() {
  ...
  useEffect(() => {
    if (activeTab === 'sct-inst') void Promise.resolve().then(loadSct)
  }, [activeTab, loadSct])
          ...
          {activeTab === 'all' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '20px' }}>
              {[
                ...
                { label: t('Načtené tuzemské platby', 'Loaded domestic payments'), value: sourceEvidence.DOMESTIC.failure ? '—' : domesticCount, color: 'var(--green)' },
                { label: t('Načtené čekající / zpracovávané', 'Loaded pending / processing'), value: visibleFailures.length ? '—' : pendingCount, color: 'var(--yellow)' },
              ].map(s => (
