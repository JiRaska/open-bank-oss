...
function Synthetics({ report }: { report: TestIntelligenceReport }) {
  ...
  return <div style={{ display: 'grid', gap: 12 }}>
    ...
    {row.ci?.variants && <div aria-label={t(`Browserová matice ${row.title}`, `Browser matrix for ${row.title}`)} style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 9 }}>
      {row.ci.variants.map(variant => <span key={variant.browser} title={variant.detail} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 8px', borderRadius: 999, border: '1px solid var(--border)', background: 'var(--surface-2)', fontSize: 11 }}><strong>{variant.browser}</strong><StateBadge state={variant.state} />{variant.run ? <a href={variant.run.url} target="_blank" rel="noreferrer">run {variant.run.id}</a> : null}</span>)}
    </div>}
