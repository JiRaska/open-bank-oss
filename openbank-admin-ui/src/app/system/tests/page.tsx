    ...
    <>
      ...
      <div style={{ overflowX: 'auto', border: '1px solid var(--border)', borderRadius: 10 }}>
        <table style={tableStyle}>
          <thead><tr><th style={thStyle}>Component</th>{KINDS.map(kind => <th key={kind} style={thStyle}>{kind}</th>)}<th style={thStyle}>Kover lines</th></tr></thead>
          <tbody>{sorted.map(component => (
            <tr key={component.component}>
              <td style={{ ...tdStyle, fontWeight: 650 }}>{component.component}{component.moneyPath && <span style={{ marginLeft: 6, color: '#dc2626', fontSize: 9 }}>MONEY PATH</span>}</td>
    ...
    <div style={{ overflowX: 'auto', border: '1px solid var(--border)', borderRadius: 10 }}>
      <table style={tableStyle}><thead><tr>{['Component', 'Kind', 'State', 'Discovered', 'Executed', 'Passed', 'Failed', 'Skipped', 'Observed'].map(label => <th key={label} style={thStyle}>{label}</th>)}</tr></thead>
        <tbody>{rows.map((row, index) => <tr key={`${row.component}-${row.kind}-${index}`}>
          ...
          <td style={tdStyle}>{row.counts?.failed ?? '—'}</td><td style={tdStyle}>{row.counts?.skipped ?? '—'}</td><td style={tdStyle}>{row.observedAt ? new Date(row.observedAt).toLocaleString() : '—'}</td>
        </tr>)}</tbody>
      </table>
    </div>
  )
...
function TestCases({ report }: { report: TestIntelligenceReport }) {
  ...
  return <div style={{ display: 'grid', gap: 18 }}>
    ...
    <div style={{ padding: 12, border: '1px solid color-mix(in srgb, #16a34a 35%, var(--border))', borderRadius: 9, color: 'var(--text-secondary)', fontSize: 12 }}>
      A test is marked flaky only after pass and fail observations on the same commit. Ownership comes from CODEOWNERS. Triage never changes the deterministic CI verdict or skips money-path controls.
    </div>
    ...
    </table>{report.testCases.length === 0 && <div style={{ padding: 18, color: 'var(--text-tertiary)', fontSize: 12 }}>No per-test run envelopes have been retained yet. Suite verdicts remain authoritative.</div>}</div>
  </div>
}
...
function History({ report }: { report: TestIntelligenceReport }) {
  ...
  return <div style={{ display: 'grid', gap: 18 }}><div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 18, background: 'var(--surface-1)' }}>
    <div style={{ marginBottom: 16 }}><strong>Fleet evidence history</strong><div style={{ color: 'var(--text-secondary)', fontSize: 12, marginTop: 4 }}>Immutable deployment snapshots retained as CI artifacts. Bars never infer missing runs.</div></div>
    ...
    <div style={{ display: 'flex', alignItems: 'end', gap: 8, minHeight: 190, overflowX: 'auto', paddingTop: 12 }}>
      {report.history.map(point => <div key={point.collectedAt} title={`${new Date(point.collectedAt).toLocaleString()} · ${point.componentsWithExecutionEvidence}/${point.components} evidenced · ${point.failingEvidence} failing`} style={{ minWidth: 38, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'end', gap: 3, height: 170 }}>
        ...
        <span style={{ fontSize: 9, color: 'var(--text-tertiary)', textAlign: 'center' }}>{new Date(point.collectedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>
      </div>)}
    </div>
    ...
    <div style={{ padding: '16px 18px 8px' }}><strong>Immutable service attempts</strong><div style={{ color: 'var(--text-secondary)', fontSize: 12, marginTop: 4 }}>Latest versioned CI envelopes; reruns remain separate attempts.</div></div>
    <table style={tableStyle}><thead><tr><th style={thStyle}>Component</th><th style={thStyle}>Run / attempt</th><th style={thStyle}>Commit</th><th style={thStyle}>Evidence states</th><th style={thStyle}>Runtime</th><th style={thStyle}>Observed</th></tr></thead>
      <tbody>{report.runHistory.slice(0, 100).map(item => <tr key={`${item.component}-${item.run.id}-${item.run.attempt}`}><td style={{ ...tdStyle, fontWeight: 650 }}>{item.component}</td><td style={tdStyle}>{item.run.id} / {item.run.attempt}</td><td style={tdStyle}>{item.run.commit.slice(0, 8)}</td><td style={tdStyle}>{Object.entries(item.states).map(([kind, state]) => <span key={kind} style={{ marginRight: 9 }}>{kind}: <StateBadge state={state} /></span>)}</td><td style={tdStyle}>{item.infrastructureStarted} start · {item.infrastructureStopped} stop</td><td style={tdStyle}>{new Date(item.run.observedAt).toLocaleString()}</td></tr>)}</tbody>
    </table>
    {report.runHistory.length === 0 && <div style={{ padding: 18, color: 'var(--text-tertiary)', fontSize: 12 }}>No retained service-run envelopes are bundled yet.</div>}
  </div></div>
}
...
function RuntimeInfrastructure({ report }: { report: TestIntelligenceReport }) {
  ...
  return <div style={{ overflowX: 'auto', border: '1px solid var(--border)', borderRadius: 10 }}><table style={tableStyle}>
    <thead><tr><th style={thStyle}>Component</th><th style={thStyle}>Declared topology</th><th style={thStyle}>Runtime proof</th><th style={thStyle}>Lifecycle</th><th style={thStyle}>Observed</th></tr></thead>
    <tbody>{rows.map(row => {
...
function Coverage({ report }: { report: TestIntelligenceReport }) {
  return <div style={{ overflowX: 'auto', border: '1px solid var(--border)', borderRadius: 10 }}><table style={tableStyle}>
    ...
    <tbody>{report.components.map(row => <tr key={row.component}><td style={{ ...tdStyle, fontWeight: 650 }}>{row.component}</td><td style={tdStyle}><StateBadge state={row.coverage.state} /></td>
      ...
      <td style={tdStyle}>{row.coverage.observedAt ? new Date(row.coverage.observedAt).toLocaleString() : '—'}</td><td style={tdStyle}>{row.coverage.source ?? '—'}</td></tr>)}</tbody>
  </table></div>
}
...
function Synthetics({ report }: { report: TestIntelligenceReport }) {
  return <div style={{ display: 'grid', gap: 12 }}>{report.syntheticJourneys.map(row => <div key={row.id} style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 18, background: 'var(--surface-1)' }}>
    ...
    {row.live && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 8, marginTop: 12, padding: 12, borderRadius: 8, background: 'var(--surface-2)', fontSize: 11 }}>
      <div><span style={{ color: 'var(--text-tertiary)' }}>Last scheduled</span><br /><strong>{row.live.lastScheduledAt ? new Date(row.live.lastScheduledAt).toLocaleString() : 'never observed'}</strong></div>
      <div><span style={{ color: 'var(--text-tertiary)' }}>Last success</span><br /><strong>{row.live.lastSuccessfulAt ? new Date(row.live.lastSuccessfulAt).toLocaleString() : 'never observed'}</strong></div>
      <div><span style={{ color: 'var(--text-tertiary)' }}>Failures / 30m</span><br /><strong>{row.live.failuresLast30m ?? 'unavailable'}</strong></div>
      <div><span style={{ color: 'var(--text-tertiary)' }}>Evidence freshness</span><br /><strong>{row.live.freshnessSeconds === null ? 'unknown' : `${Math.round(row.live.freshnessSeconds / 60)} min`}</strong></div>
      <div><span style={{ color: 'var(--text-tertiary)' }}>Recent Kubernetes runs</span><br /><strong>{row.live.recentRuns.length || 'none retained'}</strong></div>
    </div>}
...
export default function TestIntelligencePage() {
  ...
  return <div style={{ padding: '28px 32px', maxWidth: 1600, animation: 'fadeIn 0.2s ease-out' }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 20, marginBottom: 22 }}>
      <div><div className="breadcrumb"><span>OpenBank</span><span className="breadcrumb-sep">/</span><span>{t('Systém', 'System')}</span><span className="breadcrumb-sep">/</span><span className="breadcrumb-current">Test Intelligence</span></div>
        <h1 className="page-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><FlaskConical size={19} style={{ color: 'var(--accent)' }} />Test Intelligence</h1>
        <p className="page-subtitle">{t('Jednotný pohled na běhy, pokrytí kódu, kontrakty, mutace, výkon a sandboxové syntetické scénáře.', 'One evidence view for execution, code coverage, contracts, mutation, performance, and sandbox synthetic journeys.')}</p>
      </div>
      <button className="btn btn-secondary btn-sm" onClick={() => void load()} disabled={loading}><RefreshCw size={13} style={{ animation: loading ? 'spin 0.8s linear infinite' : 'none' }} />{t('Obnovit', 'Refresh')}</button>
    </div>
    ...
    <div role="tablist" style={{ display: 'flex', gap: 2, overflowX: 'auto', borderBottom: '1px solid var(--border)', marginBottom: 20 }}>{tabs.map(item => <button key={item.id} role="tab" aria-selected={tab === item.id} onClick={() => setTab(item.id)} style={{ display: 'flex', gap: 6, alignItems: 'center', padding: '9px 13px', whiteSpace: 'nowrap', border: 'none', borderBottom: tab === item.id ? '2px solid var(--accent)' : '2px solid transparent', background: 'none', color: tab === item.id ? 'var(--accent)' : 'var(--text-secondary)', cursor: 'pointer', fontWeight: tab === item.id ? 650 : 450 }}>{item.icon}{item.label}</button>)}</div>
    ...
    {report && <div style={{ marginTop: 18, color: 'var(--text-tertiary)', fontSize: 11 }}>Schema v{report.schemaVersion} · collected {new Date(report.collectedAt).toLocaleString()} · absence is never rendered as zero</div>}
  </div>
}
