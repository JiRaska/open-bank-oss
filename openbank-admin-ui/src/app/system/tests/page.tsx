...
function Synthetics({ report }: { report: TestIntelligenceReport }) {
  ...
  return <div style={{ display: 'grid', gap: 12 }}>
    {coverage && <section aria-label={t('Pokrytí money-path syntetickými cestami', 'Money-path synthetic journey coverage')} style={{ padding: 18, borderRadius: 12, border: '1px solid color-mix(in srgb, #7c3aed 35%, var(--border))', background: 'linear-gradient(135deg, color-mix(in srgb, #7c3aed 8%, var(--surface-1)), var(--surface-1))' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 11 }}>{coverage.services.map(service => <span key={service.component} title={service.reason ?? service.journeys.join(', ')} style={{ padding: '4px 7px', borderRadius: 999, fontSize: 10, border: '1px solid var(--border)', color: service.state === 'covered' ? '#16a34a' : '#d97706', background: 'var(--surface-2)' }}>{service.component.replace(/^openbank-/, '').replace(/-service$/, '')}</span>)}</div>
    </section>}
