...
function AssuranceBoard({ report, selectTab }: { report: TestIntelligenceReport; selectTab: (tab: Tab) => void }) {
  ...
  const ciState: EvidenceState = report.totals.failingEvidence > 0 ? 'failed'
    : (report.totals.unknownEvidence ?? 0) > 0 ? 'unknown'
  ...
  return (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12, marginBottom: 20 }}>
        ...
        <Stat label="Unknown evidence" value={report.totals.unknownEvidence ?? 0} tone={(report.totals.unknownEvidence ?? 0) ? '#64748b' : '#16a34a'} />
      </div>
...
function History({ report }: { report: TestIntelligenceReport }) {
  ...
  return <div style={{ display: 'grid', gap: 18 }}><div style={{ border: '1px solid var(--border)', borderRadius: 10, padding: 18, background: 'var(--surface-1)' }}>
    ...
    <div style={{ display: 'flex', gap: 14, marginTop: 12, fontSize: 11, color: 'var(--text-secondary)' }}><span style={{ color: '#16a34a' }}>● evidenced</span><span style={{ color: '#64748b' }}>● unknown</span><span style={{ color: '#d97706' }}>● missing</span><span style={{ color: '#dc2626' }}>● failing</span></div>
