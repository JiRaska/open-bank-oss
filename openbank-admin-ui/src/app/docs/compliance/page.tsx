...
const COMPLIANCE_AREAS: ComplianceArea[] = [
  ...
  },
  {
    ...
    status: 'compliant',
    items: [
      { req: ['PEP screening zákazníků', 'PEP screening of customers'], status: 'ok', note: ['pep_flag + pep_category v parties V2', 'pep_flag + pep_category in parties V2'] },
      { req: ['Screening sankčních seznamů', 'Sanctions list screening'], status: 'ok', note: ['sanctions-service + sledování matched_list V2', 'sanctions-service + matched_list tracking V2'] },
      { req: ['Sledování podání SAR', 'SAR filing tracking'], status: 'ok', note: ['sar_filed + sar_reference + sar_filed_at V2', 'sar_filed + sar_reference + sar_filed_at V2'] },
      { req: ['Eskalace na MLRO', 'MLRO escalation'], status: 'ok', note: ['escalated_to_mlro + escalated_at V2', 'escalated_to_mlro + escalated_at V2'] },
      { req: ['Dokumentace false positive', 'False positive documentation'], status: 'ok', note: ['false_positive_reason + false_positive_by V2', 'false_positive_reason + false_positive_by V2'] },
      ...
      { req: ['Rizikové hodnocení zákazníka', 'Customer risk rating'], status: 'ok', note: ['risk_rating v parties V2 (LOW/MEDIUM/HIGH)', 'risk_rating in parties V2 (LOW/MEDIUM/HIGH)'] },
    ],
  },
  {
    id: 'kyc',
    ...
    status: 'compliant',
    items: [
      { req: ['Úroveň due diligence (SDD/CDD/EDD)', 'Due diligence level (SDD/CDD/EDD)'], status: 'ok', note: ['due_diligence_level s constraint V2', 'due_diligence_level with constraint V2'] },
      ...
      { req: ['Plánování periodické revize', 'Periodic review scheduling'], status: 'ok', note: ['next_review_date V2 (HIGH=1 rok, MED=2 roky, LOW=3 roky)', 'next_review_date V2 (HIGH=1yr, MED=2yr, LOW=3yr)'] },
  ...
  },
  {
    ...
    status: 'partial',
    items: [
      { req: ['Časové razítko explicitního souhlasu', 'Explicit consent timestamp'], status: 'ok', note: ['gdpr_consent_at + gdpr_consent_version V2', 'gdpr_consent_at + gdpr_consent_version V2'] },
      { req: ['Marketingový souhlas', 'Marketing consent'], status: 'ok', note: ['marketing_consent boolean V2', 'marketing_consent boolean V2'] },
      { req: ['Právo na výmaz (soft delete)', 'Right to erasure (soft delete)'], status: 'ok', note: ['deleted_at + deletion_reason V2', 'deleted_at + deletion_reason V2'] },
      { req: ['Politika uchovávání dat', 'Data retention policy'], status: 'ok', note: ['data_retention_until v accounts + parties V2', 'data_retention_until in accounts + parties V2'] },
      { req: ['Klasifikace citlivosti dat', 'Data sensitivity classification'], status: 'ok', note: ['data_sensitivity v audit_entries V2', 'data_sensitivity in audit_entries V2'] },
      ...
      { req: ['Export přenositelnosti dat', 'Data portability export'], status: 'warn', note: ['API endpoint pro export dat zákazníka chybí', 'API endpoint for customer data export is missing'] },
    ],
  },
  {
    id: 'eba-ict',
    ...
    items: [
      ...
      { req: ['10letá retence auditu', '10-year audit retention'], status: 'ok', note: ['retention_until = occurred_at + 10 let V2', 'retention_until = occurred_at + 10 years V2'] },
      { req: ['Označování bezpečnostních událostí', 'Security event flagging'], status: 'ok', note: ['is_security_event + SIEM index V2', 'is_security_event + SIEM index V2'] },
  ...
  },
  {
    ...
    items: [
      ...
      { req: ['Výkaznický kód ČNB', 'CNB reporting code'], status: 'ok', note: ['cnb_reporting_code pro přeshraniční platby V2', 'cnb_reporting_code for cross-border payments V2'] },
  ...
  },
  {
    ...
    status: 'compliant',
    items: [
      { req: ['FATCA status zákazníka', 'Customer FATCA status'], status: 'ok', note: ['fatca_status v parties V2', 'fatca_status in parties V2'] },
      { req: ['CRS status zákazníka', 'Customer CRS status'], status: 'ok', note: ['crs_status v parties V2', 'crs_status in parties V2'] },
      { req: ['Identifikace US person', 'US person identification'], status: 'ok', note: ['fatca_status = US_PERSON/NON_US/EXEMPT', 'fatca_status = US_PERSON/NON_US/EXEMPT'] },
