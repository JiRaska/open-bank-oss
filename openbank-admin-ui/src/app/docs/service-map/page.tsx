...
const INFRA_META: Record<string, TierMeta> = {
  ...
  'infra:opa': { color: 'var(--success-text)', Icon: ShieldCheck, labelCs: 'OPA (autorizace)', labelEn: 'OPA (authz)', descCs: 'Rozhodovací bod autorizace (ADR-0034). Sidecar vyhodnocuje rest.rego politiky per požadavek.', descEn: 'Authorization decision point (ADR-0034). Sidecar evaluates rest.rego per request.' },
}
const EXT_META: Record<string, TierMeta> = {
  ...
  'ext:firebase-fcm': { color: 'var(--warning-text)', Icon: Send, labelCs: 'Firebase FCM', labelEn: 'Firebase FCM', descCs: 'Push notifikace přes Google FCM.', descEn: 'Push notifications via Google FCM.' },
          ...
          {[['all', t('Vše', 'All')], ...Object.entries(GROUP_LABELS).map(([k]) => [k, groupLabel(k)])].map(([key, label]) => (
            <button key={key} onClick={() => setFilter(key)}
              ...
              style={{
                ...
                color: filter === key ? 'var(--accent-strong-fg)' : 'var(--text-secondary)',
