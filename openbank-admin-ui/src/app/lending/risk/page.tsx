      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 12, flexWrap: 'wrap' }}>
        {(['decisions', 'policy', 'portfolio'] as const).map(id => (
          <button key={id} type="button" onClick={() => setTab(id)} aria-pressed={tab === id}
            ...
            style={{ padding: '6px 12px', fontSize: 12, fontWeight: 600, borderRadius: 6, border: 'none', cursor: 'pointer', background: tab === id ? 'var(--accent-strong)' : 'var(--surface-3)', color: tab === id ? 'var(--text-inverse)' : 'var(--text-secondary)' }}>
