    ...
    <div className="space-y-6">
      ...
      {!loading && !unavailable && items.length > 0 && <section className="grid gap-4 xl:grid-cols-2" aria-label={t('Katalog MGM programů', 'MGM program catalogue')}>
        {items.map(program => <article key={program.id} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm" data-referral-program={`${program.name}@${program.version}`}>
          <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.12em] text-[var(--success-text)]">{t('Publikováno', 'Published')} · v{program.version}</p><h2 className="mt-1 text-lg font-semibold">{program.name}</h2></div><span className="rounded-xl bg-[var(--warning-bg)] px-3 py-2 text-lg font-bold text-[var(--warning-text)]">{money(program)}</span></div>
          <dl className="mt-5 grid gap-3 rounded-xl bg-[var(--surface-2)] p-4 text-sm sm:grid-cols-2"><div><dt className="text-xs text-[var(--text-tertiary)]">{t('Kvalifikace', 'Qualification')}</dt><dd className="mt-1 font-medium">{program.qualifyingEvent}</dd></div><div><dt className="text-xs text-[var(--text-tertiary)]">{t('Platí do', 'Window ends')}</dt><dd className="mt-1 font-medium">{new Intl.DateTimeFormat(language === 'cs' ? 'cs-CZ' : 'en-GB', { dateStyle: 'medium' }).format(new Date(program.attributionWindowEndsAt))}</dd></div></dl>
