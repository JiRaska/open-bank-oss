...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const upstream = await fetch(contextServiceUrl(`/api/v1/context/lending-loans/${loanId}/shared-guarantors`), {
    ...
    let relationships
    try {
      relationships = parseSharedGuarantorRelationships(await readBoundedOwnershipJson(upstream, 256 * 1024))
      if (relationships.rootLoanId !== loanId) throw new Error('Scope mismatch')
