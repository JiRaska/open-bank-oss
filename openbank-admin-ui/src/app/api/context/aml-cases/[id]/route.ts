...
import { amlTimestampNanos, parseAmlCaseEvidence } from '@/lib/context/amlCaseEvidence'
...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const response = await fetch(contextServiceUrl(`/api/v1/context/aml-cases/${rootId}?${query}`), {
    ...
    let history
    try {
      history = parseAmlCaseEvidence(await response.json())
      if (history.root !== `aml-case:${rootId}`) throw new Error('Scope mismatch')
      for (const key of ['effectiveAt', 'knownAt'] as const) {
        const requested = query.get(key)
        if (requested && amlTimestampNanos(history[key]) !== amlTimestampNanos(requested)) throw new Error('Snapshot mismatch')
      }
