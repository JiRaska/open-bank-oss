...
type InboxItem = {
  id: string
  domain: 'lending' | 'sanctions' | 'transaction' | 'domestic-payment' | 'clearing' | 'sepaPayment' | 'agent'
  action: string
...
export async function GET() {
  ...
  const [lending, sanctions, transaction, domesticPayment, clearing, sepaPayment, agent] = await Promise.all([
  ...
  const items = [...lending.items, ...sanctions.items, ...transaction.items, ...domesticPayment.items, ...clearing.items, ...sepaPayment.items, ...agent.items]
