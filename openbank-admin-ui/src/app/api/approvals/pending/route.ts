...
// The inbox is read-only; every decision remains subject to the existing money-path controls.
...
// consent-service serves the shared PendingApproval shape for gated grant/revoke actions.
// Listing is read-only; consent lifecycle decisions remain entirely service-owned.
...
export async function GET() {
  ...
  const [lending, sanctions, transaction, domesticPayment, clearing, fx, ledger, swift, sepaPayment, sepaInstant, notification, party, account, balance, billing, consent, agent] = await Promise.all([
  ...
  const items = [...lending.items, ...sanctions.items, ...transaction.items, ...domesticPayment.items, ...clearing.items, ...fx.items, ...ledger.items, ...swift.items, ...sepaPayment.items, ...sepaInstant.items, ...notification.items, ...party.items, ...account.items, ...balance.items, ...billing.items, ...consent.items, ...agent.items]
