...
type InboxItem = {
  id: string
  domain: 'lending' | 'sanctions' | 'transaction' | 'ledger' | 'agent'
  action: string
...
export async function GET() {
  ...
  const [lending, sanctions, transaction, ledger, agent] = await Promise.all([
  ...
  const items = [...lending.items, ...sanctions.items, ...transaction.items, ...ledger.items, ...agent.items]
