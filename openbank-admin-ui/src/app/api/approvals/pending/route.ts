...
type InboxItem = {
  id: string
  domain: 'lending' | 'sanctions' | 'domestic-payment' | 'agent'
  action: string
...
export async function GET() {
  ...
  const [lending, sanctions, domesticPayment, agent] = await Promise.all([
  ...
  const items = [...lending.items, ...sanctions.items, ...domesticPayment.items, ...agent.items]
