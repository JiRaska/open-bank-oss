...
type InboxItem = {
  id: string
  domain: 'lending' | 'sanctions' | 'transaction' | 'sepaPayment' | 'agent'
  action: string
...
export async function GET() {
  ...
  const [lending, sanctions, transaction, sepaPayment, agent] = await Promise.all([
  ...
  const items = [...lending.items, ...sanctions.items, ...transaction.items, ...sepaPayment.items, ...agent.items]
    .sort((a, b) => (a.proposedAt ?? '').localeCompare(b.proposedAt ?? ''))
  return NextResponse.json({
    items,
    sources: { lending: lending.state, sanctions: sanctions.state, transaction: transaction.state, sepaPayment: sepaPayment.state, agent: agent.state },
  })
}
