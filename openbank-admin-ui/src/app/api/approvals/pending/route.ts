...
type InboxItem = {
  id: string
  domain: 'lending' | 'sanctions' | 'ledger' | 'agent'
  action: string
...
export async function GET() {
  ...
  const [lending, sanctions, ledger, agent] = await Promise.all([
  ...
  const items = [...lending.items, ...sanctions.items, ...ledger.items, ...agent.items]
    .sort((a, b) => (a.proposedAt ?? '').localeCompare(b.proposedAt ?? ''))
  return NextResponse.json({
    items,
    sources: { lending: lending.state, sanctions: sanctions.state, ledger: ledger.state, agent: agent.state },
  })
}
