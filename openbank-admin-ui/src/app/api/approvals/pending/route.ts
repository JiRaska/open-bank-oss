...
type InboxItem = {
  id: string
  domain: 'lending' | 'sanctions' | 'transaction' | 'domestic-payment' | 'sepaPayment' | 'agent'
  action: string
...
export async function GET() {
  ...
  const [lending, sanctions, transaction, domesticPayment, sepaPayment, agent] = await Promise.all([
  ...
  const items = [...lending.items, ...sanctions.items, ...transaction.items, ...domesticPayment.items, ...sepaPayment.items, ...agent.items]
