  ...
  return <AuthGuard permission="campaign:create">
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <section className="grid gap-5 lg:grid-cols-[1fr_.72fr]">
        <form noValidate onSubmit={e => { e.preventDefault(); void create() }} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <label htmlFor="segment-name" className="mt-6 block text-sm font-semibold text-slate-700">{t('Název', 'Name')}<input id="segment-name" value={name} onChange={e => { setName(e.target.value); setError(null) }} onBlur={() => setNameTouched(true)} aria-invalid={nameTouched && !validName} aria-describedby="segment-name-help segment-name-error" placeholder="new-savers" className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 font-mono text-sm outline-none transition focus:border-violet-500 focus:ring-4 focus:ring-violet-100" /></label>
          <p id="segment-name-help" className="mt-1 text-xs text-slate-500">{t('Malá písmena, čísla a pomlčky. Po schválení dostane tento název neměnnou verzi.', 'Lowercase letters, digits and hyphens. Approval creates an immutable version under this name.')}</p>
          <p id="segment-name-error" aria-live="polite" className="mt-1 min-h-4 text-xs font-medium text-rose-700">{nameTouched && !validName ? t('Začněte písmenem nebo číslicí a použijte jen malá písmena, číslice a pomlčky.', 'Start with a letter or number and use only lowercase letters, digits, and hyphens.') : ''}</p>
          <fieldset className="mt-6 space-y-4"><legend className="text-sm font-semibold text-slate-700">{t('Pravidla výběru', 'Selection rules')}</legend>
            ...
            <label htmlFor="segment-min-days" className="block rounded-xl border border-slate-200 p-4 text-sm text-slate-700"><span className="font-semibold">{t('Minimální délka vztahu (volitelné)', 'Minimum relationship age (optional)')}</span><input id="segment-min-days" inputMode="numeric" value={minDays} onChange={e => { setMinDays(e.target.value); setError(null) }} onBlur={() => setTenureTouched(true)} aria-invalid={tenureTouched && !validTenure} aria-describedby="segment-tenure-help segment-tenure-error" placeholder="30" className="mt-3 block w-full rounded-lg border border-slate-200 px-3 py-2" /><span id="segment-tenure-help" className="mt-2 block text-xs text-slate-500">{t('Prázdné = bez omezení podle délky vztahu.', 'Blank = no relationship-age restriction.')}</span><span id="segment-tenure-error" aria-live="polite" className="mt-1 block min-h-4 text-xs font-medium text-rose-700">{tenureTouched && !validTenure ? t('Zadejte celé nezáporné číslo dnů, například 30.', 'Enter a whole non-negative number of days, such as 30.') : ''}</span></label>
          </fieldset>
