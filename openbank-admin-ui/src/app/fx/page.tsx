                        ...
                        {ALL_DAYS.map(day => {
                          ...
                          return (
                            <button type="button" disabled={!FX_CONFIGURATION_WRITABLE} key={day}
                              ...
                              style={{ padding: '4px 8px', fontSize: '11px', fontWeight: 700, borderRadius: '5px', cursor: 'pointer', border: `1px solid ${active ? 'var(--accent-strong)' : 'var(--border)'}`, background: active ? 'var(--accent-strong)' : 'var(--surface-2)', color: active ? 'var(--text-inverse)' : 'var(--text-tertiary)', transition: 'all 0.1s' }}>
                  ...
                  {history.map((h, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td style={{ padding: '6px 16px', fontSize: '11px', fontWeight: 600 }}>
                        <span style={{ padding: '2px 6px', borderRadius: '4px', background: h.source === 'CNB' ? 'var(--accent-strong)' : h.source.includes('Override') ? 'var(--warning)' : h.source.includes('Margin') ? 'var(--info)' : 'var(--info)', color: 'var(--text-inverse)', opacity: 0.85 }}>{h.source}</span>
                      </td>
