    ...
    <div className={styles.workspace}>
      <section className={`card ${styles.panel}`}>
        ...
        <div className={styles.panelBody}>
          ...
          <Can permission="catalog:author">
            ...
            <div style={{ display: 'flex', gap: 7, marginTop: 7 }}><input id="studio-new-spec-code" className="input" aria-label={t('Kód nové specifikace', 'New specification code')} value={newSpecCode} onChange={e => setNewSpecCode(e.target.value)} placeholder="TERM_LIFE" /><button className="btn btn-secondary" type="button" onClick={createSpecification} disabled={busy} aria-busy={flight.activeKey === 'spec:create'} aria-label={flight.activeKey === 'spec:create' ? t('Vytvářím specifikaci…', 'Creating specification…') : t('Vytvořit specifikaci', 'Create specification')}><Plus size={13} aria-hidden="true" /></button></div>
          </Can>
      ...
      <section className={`card ${styles.panel}`}>
        ...
        <div className={styles.panelBody}>
          ...
          <Can permission="catalog:author">
            <div style={{ display: 'flex', gap: 7, marginTop: 8 }}><input id="studio-new-offering-code" className="input" aria-label={t('Kód nové nabídky', 'New offer code')} value={newOfferingCode} onChange={e => setNewOfferingCode(e.target.value)} placeholder="TERM_LIFE_CZ_WEB" /><button className="btn btn-secondary" type="button" onClick={createOffering} disabled={busy} aria-busy={flight.activeKey === 'offering:create'} aria-label={flight.activeKey === 'offering:create' ? t('Vytvářím nabídku…', 'Creating offering…') : t('Vytvořit nabídku', 'Create offering')}><Plus size={13} aria-hidden="true" /></button></div>
            ...
            <button className="btn btn-primary" type="button" style={{ width: '100%', marginTop: 8 }} disabled={!selectedOffering || busy} aria-busy={flight.activeKey === 'revision:create'} onClick={createDraft}><Plus size={13} aria-hidden="true" />{flight.activeKey === 'revision:create' ? t('Zakládám…', 'Creating…') : t('Založit novou revizi', 'Create a new revision')}</button>
          </Can>
      ...
      <section className={`card ${styles.panel}`}>
        ...
        <div className={styles.panelBody}>
          ...
          <Can permission="catalog:author" fallback={<textarea className={`input ${styles.editor}`} value={draftText} disabled />}>
            ...
            <div className={styles.actions}><button className="btn btn-secondary" disabled={!selectedRevision} onClick={() => void validateDraft()}><CheckCircle2 size={13} aria-hidden="true" />{t('Ověřit schéma', 'Validate schema')}</button><button className="btn btn-primary" type="button" disabled={!selectedRevision || selectedRevision.state !== 'DRAFT' || busy} aria-busy={flight.activeKey === 'revision:save'} onClick={saveDraft}><Send size={13} aria-hidden="true" />{flight.activeKey === 'revision:save' ? t('Ukládám…', 'Saving…') : t('Uložit draft', 'Save draft')}</button></div>
          </Can>
          <Can permission="catalog:publish"><div className={styles.approvalPanel}><div className={styles.approvalHead}><ShieldCheck size={15} aria-hidden="true" /><span>{t('Nezávislé schválení', 'Independent approval')}</span></div><p>{t('Publikace je nevratné rozhodnutí. Služba ověří, že autor a schvalovatel jsou rozdílné identity — tento formulář to nemůže obejít.', 'Publication is an irreversible decision. The service verifies that maker and checker are different identities — this form cannot bypass it.')}</p><div className={styles.approvalMeta}><span>{t('Autor draftu', 'Draft maker')}: <b>{selectedRevision?.makerId ?? '—'}</b></span><span>{t('Stav ověření', 'Validation')}: <b>{validationState === 'valid' ? t('ověřeno', 'verified') : t('čeká na ověření', 'awaiting validation')}</b></span></div><div style={{ display: 'flex', gap: 7 }}><label className="sr-only" htmlFor="studio-publish-reason">{t('Důvod schválení', 'Approval reason')}</label><input id="studio-publish-reason" className="input" value={publishReason} onChange={e => setPublishReason(e.target.value)} placeholder={t('Důvod schválení', 'Approval reason')} /><button className="btn btn-primary" type="button" disabled={!selectedRevision || selectedRevision.state !== 'DRAFT' || !publishReason.trim() || busy} aria-busy={flight.activeKey === 'revision:publish'} onClick={publish}><ShieldCheck size={13} aria-hidden="true" />{flight.activeKey === 'revision:publish' ? t('Publikuji…', 'Publishing…') : t('Publikovat', 'Publish')}</button></div></div></Can>
        </div>
      </section>
    </div>
