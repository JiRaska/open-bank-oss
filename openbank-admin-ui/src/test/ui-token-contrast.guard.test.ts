...
describe('admin UI token contrast', () => {
  ...
  it('the fixed strong accent fill has readable foreground in both themes', () => {
    const root = declarations(':root')
    for (const tokens of [root, { ...root, ...declarations('\\.dark') }]) {
      expect(contrast(resolve(tokens, '--accent-strong-fg'), resolve(tokens, '--accent-strong'))).toBeGreaterThanOrEqual(AA)
    }
  })
