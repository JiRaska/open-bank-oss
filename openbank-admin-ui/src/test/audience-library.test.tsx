...
describe('audience library', () => {
  ...
  it('rejects malformed reach evidence and offers an explicit retry', async () => {
    let previews = 0
    vi.stubGlobal('fetch', vi.fn(async (url: string) => {
      if (url.includes('/preview')) {
        previews += 1
        return previews === 1
          ? { ok: true, json: async () => ({ state: 'ok', name: 'actives', version: 1, size: -1, asOf: 'not-a-date' }) }
          : { ok: true, json: async () => ({ state: 'ok', name: 'actives', version: 1, size: 42, asOf: '2026-09-09T08:00:00Z' }) }
      }
      return { ok: true, json: async () => ({ state: 'ok', items: [{ name: 'actives', version: 1, rules: ['party status is ACTIVE'] }] }) }
    }))
    ...
    const retry = await screen.findByRole('button', { name: 'Retry reach for Active customers' })
    expect(container.querySelector('[data-audience-size="actives@1"]')).toBeNull()
    fireEvent.click(retry)
    await waitFor(() => expect(container.querySelector('[data-audience-size="actives@1"]')?.textContent).toContain('42'))
  })
