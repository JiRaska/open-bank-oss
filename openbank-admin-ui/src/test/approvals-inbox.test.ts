...
describe('federated approvals inbox (ADR-0227 D2)', () => {
  ...
  it('merges lending, sanctions, transaction, domestic-payment, swift and agent queues into canonical items, sorted by proposedAt', async () => {
    ...
    expect(body.items.map((i: { id: string }) => i.id)).toEqual(['L-1', 'D-1', 'W-1', 'S-1', 'P-1', 'T-1', 'L-2'])
    ...
    expect(body.items[2]).toMatchObject({ domain: 'swift', action: 'swift.send', maker: 'operator.d' })
