...
describe('federated approvals inbox (ADR-0227 D2)', () => {
  ...
  it('merges lending, sanctions, transaction, ledger and agent queues into canonical items, sorted by proposedAt', async () => {
    ...
    expect(body.items.map((i: { id: string }) => i.id)).toEqual(['L-1', 'J-1', 'S-1', 'P-1', 'T-1', 'L-2'])
    expect(body.items[0]).toMatchObject({ domain: 'lending', action: 'lending.writeoff', maker: 'officer.a' })
    expect(body.items[1]).toMatchObject({ domain: 'ledger', action: 'ledger.reverse', maker: 'operator.d' })
