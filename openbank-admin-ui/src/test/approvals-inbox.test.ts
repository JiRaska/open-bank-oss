...
describe('federated approvals inbox (ADR-0227 D2)', () => {
  ...
  it('merges lending, sanctions, transaction, fx and agent queues into canonical items, sorted by proposedAt', async () => {
    const mock = vi.fn().mockImplementation((url: string) => {
      ...
      if (url.includes('fx/approvals')) {
        return Promise.resolve(new Response(JSON.stringify([
          { id: 'F-1', action: 'fx.convert', resourceId: 'conv-3', makerId: 'trader.d', createdAt: '2026-07-29T11:00:00Z' },
        ]), { status: 200 }))
      }
      return Promise.resolve(new Response(JSON.stringify([
    ...
    expect(body.items.map((i: { id: string }) => i.id)).toEqual(['L-1', 'F-1', 'S-1', 'P-1', 'T-1', 'L-2'])
    expect(body.items[0]).toMatchObject({ domain: 'lending', action: 'lending.writeoff', maker: 'officer.a' })
    expect(body.items[1]).toMatchObject({ domain: 'fx', action: 'fx.convert', maker: 'trader.d' })
