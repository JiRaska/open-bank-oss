...
describe('federated approvals inbox (ADR-0227 D2)', () => {
  ...
  it('merges lending, sanctions, transaction, domestic-payment, clearing, fx, ledger and agent queues into canonical items, sorted by proposedAt', async () => {
    ...
    // C-1 and J-1 both sit at 11:00, so their relative order is the stable-sort's concat order
    // (clearing before ledger in route.ts); F-1 is 11:30 and orders on its own, strictly after
    // the 11:00 tied group regardless of array position.
    expect(body.items.map((i: { id: string }) => i.id)).toEqual(['L-1', 'D-1', 'C-1', 'J-1', 'F-1', 'S-1', 'P-1', 'T-1', 'L-2'])
