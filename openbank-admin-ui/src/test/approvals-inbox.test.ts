...
describe('federated approvals inbox (ADR-0227 D2)', () => {
  ...
  it('merges lending, sanctions, transaction, domestic-payment, clearing, fx, ledger, swift, sepaPayment, sepa-instant, party and agent queues into canonical items, sorted by proposedAt', async () => {
    const mock = vi.fn().mockImplementation((url: string) => {
      ...
      if (url.includes('parties/approvals')) {
        // 11:45, deliberately after every other 11:xx carrier so it orders on its own.
        return Promise.resolve(new Response(JSON.stringify([
          { id: 'PT-1', action: 'party.merge', resourceId: 'party-4', makerId: 'operator.f', createdAt: '2026-07-29T11:45:00Z' },
        ]), { status: 200 }))
      }
      return Promise.resolve(new Response(JSON.stringify([
    ...
    // by the stable-sort's concat order in route.ts. PT-1 is 11:45 and orders on its own.
    expect(body.items.map((i: { id: string }) => i.id)).toEqual(['L-1', 'D-1', 'C-1', 'J-1', 'W-1', 'SP-1', 'F-1', 'I-1', 'PT-1', 'S-1', 'P-1', 'T-1', 'L-2'])
    ...
    expect(body.items[8]).toMatchObject({ domain: 'party', action: 'party.merge', maker: 'operator.f' })
  ...
  // Same regression, party side (issue #5679): party-service has served ApprovalStore.decide
  // since ADR-0179 but never the pending list, so a parked `party.merge` four-eyes decision was
  // invisible on the one screen built to show parked decisions.
