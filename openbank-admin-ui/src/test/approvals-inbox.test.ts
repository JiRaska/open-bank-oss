...
describe('federated approvals inbox (ADR-0227 D2)', () => {
  ...
  it('merges lending, sanctions, transaction, domestic-payment, clearing, fx, ledger, swift, sepaPayment, sepa-instant, party, notification and agent queues into canonical items, sorted by proposedAt', async () => {
    ...
    // 11:00 tie. PT-1 is 11:45 and orders on its own.
    ...
    expect(body.items[9]).toMatchObject({ domain: 'party', action: 'party.merge', maker: 'operator.f' })
