...
describe('federated approvals inbox (ADR-0227 D2)', () => {
  ...
  it('merges lending, sanctions, transaction, domestic-payment, clearing, fx, swift and agent queues into canonical items, sorted by proposedAt', async () => {
    ...
    // D-1, C-1 and W-1 all sit at 11:00, so their relative order is the stable-sort's concat
    // order (domestic-payment, clearing, ..., swift in route.ts); F-1 is 11:30 and orders on
    // its own, strictly after the 11:00 tied group regardless of array position.
    expect(body.items.map((i: { id: string }) => i.id)).toEqual(['L-1', 'D-1', 'C-1', 'W-1', 'F-1', 'S-1', 'P-1', 'T-1', 'L-2'])
    ...
    expect(body.items[3]).toMatchObject({ domain: 'swift', action: 'swift.send', maker: 'operator.d' })
