  ...
  it('merges every configured domain queue into canonical items, sorted by proposedAt', async () => {
    const mock = vi.fn().mockImplementation((url: string) => {
      ...
      if (url.includes('consents/approvals') || url.includes('consent-service')) {
        return Promise.resolve(new Response(JSON.stringify([
          { id: 'CO-1', action: 'consent.grant', resourceId: 'consent-7', makerId: 'operator.k', createdAt: '2026-07-29T11:58:00Z' },
        ]), { status: 200 }))
      }
      return Promise.resolve(new Response(JSON.stringify([
    ...
    // 11:00 tie.
    expect(body.items.map((i: { id: string }) => i.id)).toEqual(['L-1', 'N-1', 'D-1', 'C-1', 'J-1', 'W-1', 'SP-1', 'F-1', 'I-1', 'PT-1', 'A-1', 'B-1', 'BL-1', 'CO-1', 'S-1', 'P-1', 'T-1', 'L-2'])
    ...
    expect(body.items[11]).toMatchObject({ domain: 'balance', action: 'balance.debit', maker: 'operator.i' })
    expect(body.items[12]).toMatchObject({ domain: 'billing', action: 'billing.post', maker: 'operator.j' })
    expect(body.items[13]).toMatchObject({ domain: 'consent', action: 'consent.grant', maker: 'operator.k' })
