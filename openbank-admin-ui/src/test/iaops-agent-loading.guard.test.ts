...
describe('IAOPS agent loading contract', () => {
  it('announces agent detail loading without changing probe flow', () => {
    ...
    expect(source).not.toContain('triggerBoundedCheck')
    expect(source).toContain('href="/iaops/flaky-test-hunter"')
    expect(source).toContain("t('Otevřít řízené spuštění', 'Open governed check')")
  })
})
