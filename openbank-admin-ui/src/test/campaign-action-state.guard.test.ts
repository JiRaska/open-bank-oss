...
describe('campaign action state accessibility guard', () => {
  it('exposes truthful progress for draft and detail mutations', () => {
    ...
    expect(detail).toContain("import * as Dialog from '@radix-ui/react-dialog'")
    expect(detail).toContain("if (await runAction(actionIntent)) {")
    expect(detail).toContain("actionCloseFocusOverrideRef.current = actionIntent === 'enrol' ? null : campaignWorkspaceRef.current")
    expect(detail).toContain('const target = override?.isConnected ? override : trigger?.isConnected ? trigger : null')
    expect(detail).not.toContain('trapDialogFocus')
