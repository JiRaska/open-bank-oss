...
test('reviews the exact second identity vote and retains a failed decision for retry', async ({ page }) => {
  ...
  const trigger = page.getByRole('button', { name: /Confirm & decide|Potvrdit a rozhodnout/ })
  await trigger.click()
  ...
  await expect(dialog.getByRole('button', { name: /Back to review|Zpět ke kontrole/ })).toBeFocused()
