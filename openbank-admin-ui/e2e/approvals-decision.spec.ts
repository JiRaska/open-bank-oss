...
test.describe('approval workbench', () => {
  ...
  test('shows incomplete domain evidence and records one human AI-proposal decision', async ({ page }) => {
    ...
    const approvalTrigger = page.getByRole('button', { name: new RegExp(`Review and approve proposal ${proposal.title}|Zkontrolovat a schválit návrh ${proposal.title}`) })
    ...
    await expect(dialog.getByLabel(/Decision reason|Důvod rozhodnutí/)).toBeFocused()
    ...
    await expect(dialog).toBeHidden()
    await expect(page.getByLabel(/Approval workspace|Pracovní prostor schvalování/)).toBeFocused()
