...
test('registers a party responsively and retries the same command safely', async ({ page }) => {
  ...
  await expect(page.locator('#party-legal-name')).toHaveAttribute('aria-invalid', 'true')
  await expect(page.locator('#party-legal-name-error')).toBeVisible()
  await expect(page.locator('#party-email')).toHaveAttribute('aria-invalid', 'true')
  await expect(page.locator('#party-email-error')).toBeVisible()
  expect(idempotencyKeys).toHaveLength(0)

  await page.locator('#party-legal-name').fill('Alena Nováková')
  await expect(page.locator('#party-legal-name')).toHaveAttribute('aria-invalid', 'false')
  await page.locator('#party-email').fill('not-an-email')
  await submit.click()
  await expect(page.locator('#party-email')).toBeFocused()
  await expect(page.locator('#party-email-error')).toContainText(/platnou|valid/i)
  expect(idempotencyKeys).toHaveLength(0)
