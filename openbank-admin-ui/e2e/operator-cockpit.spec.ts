...
test('Temporal and approvals show live source-backed state without inventing human provenance', async ({ page }) => {
  ...
  await expect(page.getByText('Alice Nováková')).toHaveCount(0)
  await expect(page.getByText('alice@openbank.test').first()).toBeVisible()
  await expect(page.locator('[data-proposer-kind="unverified"]')).toBeVisible()
  await expect(page.getByText(/authorship cannot be verified|Původ tohoto návrhu nelze ověřit/)).toBeVisible()
