...
test('renders the animated evidence system and consolidates test dimensions', async ({ page }) => {
  ...
  const historyChart = page.getByRole('region', { name: /Posuvný graf historie fleet evidence|Scrollable fleet evidence history chart/ })
  await expect(historyChart).toBeVisible()
  await expect(page.getByText(/Posuňte graf vodorovně|Scroll the chart horizontally/)).toBeVisible()
