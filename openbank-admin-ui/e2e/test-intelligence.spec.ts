...
test.beforeEach(async ({ context, baseURL, page }) => {
  ...
  await page.route('**/api/test-intelligence', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({
    ...
    contracts: [], mutations: [], performance: [{ id: 'money-path-smoke', component: null, state: 'passed', observedAt: '2026-08-22T10:00:00.000Z', source: 'perf/k6/money-path-smoke.js', thresholds: 1, metrics: { p95Ms: 220.4, errorRatePercent: 0.2, checkPassRatePercent: 99.8 } }],
    ...
    clientExperiences: [{ id: 'openbank-app', title: 'OpenBank customer app', surface: 'mobile', platforms: ['android', 'ios'], evidence: [{ kind: 'visual', state: 'passed', observedAt: '2026-08-22T11:00:00.000Z', source: 'openbank-app CI', environment: 'ci', counts: { discovered: 1, executed: 1, passed: 1, failed: 0, skipped: 0, errors: 0 }, detail: 'Roborazzi committed-golden verification' }], rum: { state: 'passed', policy: 'consent-gated', detail: 'Mobile RUM is opt-in and its runtime signal is independent of CI.', observedAt: '2026-08-22T12:00:00.000Z', source: 'prometheus', sampledSpansLast7d: 12, errorSpansLast7d: 2 }, blocker: null }],
...
test('renders the animated evidence system and consolidates test dimensions', async ({ page }) => {
  ...
  await expect(page.getByText('12 sampled spans · 2 error spans')).toBeVisible()
