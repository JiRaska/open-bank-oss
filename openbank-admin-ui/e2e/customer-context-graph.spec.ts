...
test('Customer 360 combines its supplied projection with one bounded live snapshot', async ({ page, context, baseURL }, testInfo) => {
  ...
  await expect(graph.getByLabel('Selected node evidence').getByText('State: REVOKED')).toBeVisible()
  ...
  await expect(graph.getByLabel('Selected node evidence').getByText('State: REVOKED')).toHaveCount(0)
  ...
  await expect(page.getByText('State: REVOKED')).toHaveCount(0)
})
