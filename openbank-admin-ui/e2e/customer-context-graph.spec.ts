...
test('Customer 360 combines its supplied projection with one bounded live snapshot', async ({ page, context, baseURL }, testInfo) => {
  ...
  await expect(graph.getByLabel('Selected node evidence').getByText('Projected state: REVOKED')).toBeVisible()
  ...
  await expect(graph.getByLabel('Selected node evidence').getByText('Projected state: REVOKED')).toHaveCount(0)
