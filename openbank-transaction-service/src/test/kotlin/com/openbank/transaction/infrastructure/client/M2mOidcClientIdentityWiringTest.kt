 ...
 * `service-account-openbank-transaction` (ROLE_API only), not the shared `openbank-services` one. Two
 ...
 * `openbank-transaction` and its own secret variable (a missing named client fails the first call, not
 * the build).
 */
class M2mOidcClientIdentityWiringTest {
    ...
    fun `the money-path rest-clients select the named m2m oidc-client and not the default one`() {
        listOf(LedgerRestClient::class.java, BalanceCoverRestClient::class.java).forEach { client ->
    ...
    fun `the not-yet-migrated rest-clients keep the default (shared) client`() {
        listOf(FxServiceRestClient::class.java).forEach { client ->
            assertThat(providers(client))
    ...
    @Test
    fun `the named m2m oidc-client authenticates as openbank-transaction with its own secret`() {
        val m2m = oidcClient[M2M] as Map<*, *>
        assertThat(m2m["client-id"]).isEqualTo("openbank-transaction")
