...
/**
 * Produces the singleton [WorkflowClient]. The client is built lazily so that with the feature flag
 * disabled (the default) the bean exists for CDI wiring but never dials Temporal's frontend on
 * `:7233` at boot — the gRPC stub is only created on first access, which the disabled path never does.
 */
@ApplicationScoped
