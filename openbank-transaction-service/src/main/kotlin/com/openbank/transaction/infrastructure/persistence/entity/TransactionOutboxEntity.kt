...
/**
 * `claimed_at` is transaction-only — added straight on this entity, not the shared
 ...
 * `TransactionOutboxRepositoryImpl.claimProcessable`'s atomic claim query on DISPATCHING; read
 * back by the same query to decide if a DISPATCHING row is stale enough to reclaim.
 */
@Entity
@Table(name = "transaction_outbox")
class TransactionOutboxEntity : PanacheOutboxEntity() {
