...
// See LICENSE in the repository root for https://www.apache.org/licenses/LICENSE-2.0 for details.
...
/**
 * Drains the transaction transactional outbox (ADR-0050 / ADR-0049 D3).
 *
 ...
 * `TransactionOutboxRepositoryImpl` therefore implements [OutboxRepository.claimProcessable] as
 * an atomic `FOR UPDATE SKIP LOCKED` claim, not the unclaimed-peek default, so two concurrently
 * running pods can never both select and publish the same row.
 */
@ApplicationScoped
class TransactionOutboxDispatcher(
    private val repo: TransactionOutboxRepository,
    ...
    @Scheduled(
        ...
        identity = "transaction-outbox-dispatcher",
    )
    @Bulkhead(1)
