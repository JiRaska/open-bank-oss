...
package com.openbank.billing.it
...
/**
 * Isolated Postgres + Valkey(Redis) per test JVM (mirrors `openbank-standing-order-service`'s
 * `PostgresRedisTestResource`) — billing needs both: Postgres for `billing_cycle_assessment` /
 * `assessed_fee` / `billing_outbox`, Redis for the four-eyes `ApprovalStore`.
 */
