...
internal object AnnualFeeSummaryOutboxPayloads {
     ...
     *
     * Declared on [AnnualFeeSummaryReadyPayload] rather than here: `check-event-contract-code-agreement`
     * looks for `const val EVENT_TYPE` inside the BODY of the data class it is matching, so a copy
     * living in this object is invisible to it and the contract's message reads as having no producer.
     */
    const val EVENT_TYPE: String = AnnualFeeSummaryReadyPayload.EVENT_TYPE
...
internal data class AnnualFeeSummaryReadyPayload(
    ...
    companion object {
        /** The event type this payload IS. Read by the contract-agreement gate from this body. */
