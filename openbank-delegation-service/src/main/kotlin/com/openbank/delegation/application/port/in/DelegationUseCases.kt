...
data class IssueExternalDisclosureCommand(
    val delegationId: UUID,
    val documentId: UUID,
...
/**
 * Secrets are returned only at creation for a future secure delivery adapter. This intentionally
 * is not a data class: autogenerated `toString()` is an accidental secret log sink.
 */
class IssuedExternalDisclosure(val disclosure: ExternalDisclosure, val linkSecret: String, val otp: String) {
    override fun toString(): String = "IssuedExternalDisclosure(disclosureId=${disclosure.id}, secrets=redacted)"
}
