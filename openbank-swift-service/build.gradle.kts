...
tasks.withType<Test> {
    ...
    if (System.getenv("CI") == "true") {
        ...
        // Re-enable once the pact-publish CI step is wired per #2319.
        ...
        // Cost of the two exclusions above: no CI job can run either class, so neither
        // pacts/openbank-transaction-service-openbank-swift-service.json nor
        // pacts/openbank-swift-service-openbank-clearing-simulator.json is regenerated by
        // pact-drift-check.yml — both are outside the ADR-0063 drift gate until #2319 lands.
        // .github/scripts/derive-pact-drift-scope.sh names them in UNGATED_PACTS and fails if
        // that declaration ever stops matching what the exclusions actually strand.
