 ...
 * defaults.
 *
 * Both are needed for [com.openbank.swift.integration.SwiftBootSmokeIT] to reach a green
 ...
 * Valkey is needed too. The single `@Channel("swift-events-out")` Kafka emitter is switched to
...
class PostgresRedisTestResource : QuarkusTestResourceLifecycleManager {
    ...
    override fun start(): Map<String, String> {
        val pg = PostgreSQLContainer(DockerImageName.parse("postgres:16.3-alpine"))
            ...
            .withDatabaseName("openbank_swift_it")
        pg.start()
        postgres = pg
        ...
        return mapOf(
            "quarkus.datasource.reactive.url" to "vertx-reactive:postgresql://$pgHost:$pgPort/openbank_swift_it",
            "quarkus.datasource.jdbc.url" to "jdbc:postgresql://$pgHost:$pgPort/openbank_swift_it",
