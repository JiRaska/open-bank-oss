 ...
 */
class SwiftOutboxBacklogGaugeTest {

    @Test
    fun `registers a swift backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<SwiftOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("swift"), capture(supplier)) } returns Unit

        val gauge = SwiftOutboxBacklogGauge(repo, metrics)
        gauge.register()
