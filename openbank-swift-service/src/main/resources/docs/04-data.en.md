...
Events are drained from `swift_outbox` to Kafka topic `openbank.payments.swift.event` (channel `swift-events-out`, String key + String value). The `payload` column holds the serialized event; `event_type` carries the type label. The concrete domain-event schema and the wiring that writes outbox rows from the send/ack/reject path are not yet present in the use-case code (TBD — see [02 — Architecture](./02-architecture.md)). Event schemas must be versioned backward-compatibly when added.
