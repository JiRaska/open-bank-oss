...
Události jsou drénovány z `swift_outbox` do Kafka topicu `openbank.payments.swift.event` (kanál `swift-events-out`, String klíč + String hodnota). Sloupec `payload` drží serializovanou událost; `event_type` nese typový štítek. Konkrétní schéma doménové události a zapojení, které zapisuje outbox řádky ze send/ack/reject cesty, zatím nejsou v kódu use case (TBD — viz [02 — Architektura](./02-architecture.md)). Schémata událostí musí být při přidání verzována zpětně kompatibilně.
