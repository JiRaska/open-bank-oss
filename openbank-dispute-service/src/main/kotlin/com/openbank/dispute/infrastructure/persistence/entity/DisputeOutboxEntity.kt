...
/**
 * `claimed_at` is dispute-only — added straight on this entity, not the shared
 ...
 * `DisputeOutboxRepositoryImpl.claimProcessable`'s atomic claim query on DISPATCHING; read back
 * by the same query to decide if a DISPATCHING row is stale enough to reclaim.
 */
@Entity
@Table(name = "dispute_outbox")
class DisputeOutboxEntity : PanacheOutboxEntity() {
