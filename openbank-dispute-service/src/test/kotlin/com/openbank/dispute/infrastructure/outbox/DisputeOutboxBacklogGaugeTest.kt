 ...
 */
class DisputeOutboxBacklogGaugeTest {

    @Test
    fun `registers a dispute backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<DisputeOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("dispute"), capture(supplier)) } returns Unit

        val gauge = DisputeOutboxBacklogGauge(repo, metrics)
        gauge.register()
