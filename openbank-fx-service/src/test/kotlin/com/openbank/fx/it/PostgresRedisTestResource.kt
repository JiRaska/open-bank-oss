 ...
 * defaults.
 *
 * Both are needed for [com.openbank.fx.integration.FxBootSmokeIT] to reach a green
 ...
 * Valkey is needed too. The single `@Channel("fx-events-out")` Kafka emitter is switched to the
...
class PostgresRedisTestResource : QuarkusTestResourceLifecycleManager {
    ...
    override fun start(): Map<String, String> {
        ...
        val pg = PostgreSQLContainer(DockerImageName.parse("postgres:16.3-alpine"))
            ...
            .withDatabaseName("openbank_fx_it")
        pg.start()
        postgres = pg
        ...
        return mapOf(
            "quarkus.datasource.reactive.url" to
                "vertx-reactive:postgresql://$pgHost:$pgPort/openbank_fx_it",
            "quarkus.datasource.jdbc.url" to
                "jdbc:postgresql://$pgHost:$pgPort/openbank_fx_it",
