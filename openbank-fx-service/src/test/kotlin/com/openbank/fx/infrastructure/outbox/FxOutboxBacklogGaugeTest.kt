 ...
 */
class FxOutboxBacklogGaugeTest {

    @Test
    fun `registers an fx backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<FxOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("fx"), capture(supplier)) } returns Unit

        val gauge = FxOutboxBacklogGauge(repo, metrics)
        gauge.register()
