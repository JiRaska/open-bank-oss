 ...
 *
 * fx-service is a money-path service (version.txt) that previously had zero @QuarkusTest, so
 ...
 *
 * The lone `@Channel("fx-events-out")` Kafka emitter is swapped to the in-memory connector so no
...
class FxBootSmokeIT {
    ...
    fun `the service-info endpoint answers on the configured HTTP port`() {
        ...
        assertThat(body).contains("openbank-fx-service")
    }
}
