...
import com.openbank.fx.infrastructure.persistence.entity.FxOutboxEntity
...
@ApplicationScoped
class FxOutboxRepositoryImpl(private val clock: Clock) :
    FxOutboxRepository,
    PanacheRepository<FxOutboxEntity> {
    ...
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, FxOutboxEntity::class.java)
