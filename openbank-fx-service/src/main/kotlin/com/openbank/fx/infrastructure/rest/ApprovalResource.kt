...
/**
 * Checker-facing endpoint for the four-eyes gate on `fx.convert` (ADR-0155). A maker's
 * `POST /fx/convert` call is paused by [com.openbank.libs.authz.AuthorizeInterceptor] with
 * HTTP 202 and a `PendingApproval` id when OPA flags the action `four_eyes_required`; a
 * DIFFERENT operator decides it here, then the maker retries the original call with an
 * `X-Approval-Id` header. Mirrors `openbank-billing-service`'s `ApprovalResource` exactly.
 */
@Path("/api/v1/fx/approvals")
@Produces(MediaType.APPLICATION_JSON)
@Tag(name = "FX Approvals", description = "Four-eyes decisions for gated FX-conversion actions")
class ApprovalResource(private val approvalStore: ApprovalStore) {
    ...
    /**
     * The checker's queue (issue #5679, mirroring sanctions #3472, lending, ledger,
     * domestic-payment and sepa-instant). Without it a parked decision is invisible: the maker
     * gets a 202 with an approval id and no way to hand it over except out of band, so the
     * four-eyes ceremony on `fx.convert` only completed if the two operators were already
     * talking. The Redis TTL (24h) then expired the request silently.
     *
    ...
    @Authorize(action = "fx.approval.read", resource = "")
    ...
    private companion object {
        // Same ceiling as sanctions/lending/ledger/domestic-payment/sepa-instant's queue. The
        // read is a Redis scan, and an unbounded `limit` from a query parameter is a trivially
        // reachable amplification.
...
data class ApprovalResponse(
    ...
    // sanctions/lending/ledger/domestic-payment/sepa-instant's shape.
    val makerId: String?,
