...
/**
 * Regression guard (C7/ADR-0030): no onboarding endpoint must be opened without role-gating.
 * Pure reflection — no Quarkus boot needed.
 */
class OnboardingSecurityTest {

    @Test
    fun `no OnboardingResource endpoint is @PermitAll`() {
        val methods = OnboardingResource::class.java.declaredMethods.filter { m ->
