...
package com.openbank.tpp.infrastructure.persistence.repository
...
import com.openbank.tpp.infrastructure.persistence.entity.TppOutboxEntity
...
@ApplicationScoped
class TppOutboxRepositoryImpl(private val clock: Clock) :
    TppOutboxRepository,
    PanacheRepository<TppOutboxEntity> {
     ...
     * lifetime of this one statement/transaction — it must not span the network publish call
     * that follows.
     */
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, TppOutboxEntity::class.java)
