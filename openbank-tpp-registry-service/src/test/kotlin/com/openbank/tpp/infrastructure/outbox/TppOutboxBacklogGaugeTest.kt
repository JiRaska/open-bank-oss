 ...
 */
class TppOutboxBacklogGaugeTest {

    @Test
    fun `registers a tpp-registry backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<TppOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("tpp-registry"), capture(supplier)) } returns Unit

        val gauge = TppOutboxBacklogGauge(repo, metrics)
        gauge.register()
