...
/**
 * Regression guard (C7/ADR-0030): no TPP registry endpoint must be opened without role-gating.
 * Pure reflection — no Quarkus boot needed.
 */
class TppRegistrySecurityTest {

    @Test
    fun `no TppRegistryResource endpoint is @PermitAll`() {
        val methods = TppRegistryResource::class.java.declaredMethods.filter { m ->
