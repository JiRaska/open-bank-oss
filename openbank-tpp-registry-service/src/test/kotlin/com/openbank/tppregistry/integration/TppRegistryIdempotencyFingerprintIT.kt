 ...
 * canonicalised body) so the same key with a DIFFERENT request is refused (422
 * IDEMPOTENCY_KEY_REUSED) instead of replaying the first response — which would otherwise answer
 * a second, different registration with the first TPP's entry. `registerTpp` is the endpoint under
 * test; the count of `tpp_entries` rows for the reused key is the side-effect assertion (a mocked
 * use case can't see whether a SECOND registration actually happened).
 */
@QuarkusTest
