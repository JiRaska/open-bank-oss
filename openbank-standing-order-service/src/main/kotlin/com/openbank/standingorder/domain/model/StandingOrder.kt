...
data class StandingOrder(
    ...
    val partyId: UUID, val debitAccountId: UUID,
    ...
    fun cancel(now: Instant) = also {
        require(status !in setOf(StandingOrderStatus.CANCELLED, StandingOrderStatus.COMPLETED)) { "Cannot cancel order in status $status" }
    }.copy(status = StandingOrderStatus.CANCELLED, updatedAt = now)

    fun recordExecution(nextDate: LocalDate, now: Instant) = copy(
        lastExecutionDate = nextExecutionDate, nextExecutionDate = nextDate,
        executionCount = executionCount + 1, updatedAt = now,
