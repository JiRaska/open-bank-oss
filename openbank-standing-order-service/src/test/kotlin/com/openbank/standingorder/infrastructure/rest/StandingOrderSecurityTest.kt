...
/**
 * Regression guard (C7/ADR-0030): no standing-order endpoint must be opened without role-gating.
 * Pure reflection — no Quarkus boot needed.
 */
class StandingOrderSecurityTest {

    @Test
    fun `no StandingOrderResource endpoint is @PermitAll`() {
        val methods = StandingOrderResource::class.java.declaredMethods.filter { m ->
