 ...
 * `service-account-openbank-standing-order` (ROLE_API only), not the shared `openbank-services` one. Two
 ...
 * `openbank-standing-order` and its own secret variable (a missing named client fails the first call, not
 * the build).
 */
class M2mOidcClientIdentityWiringTest {
    ...
    fun `the money-path rest-clients select the named m2m oidc-client and not the default one`() {
        listOf(TransactionServiceClient::class.java, SepaPaymentClient::class.java).forEach { client ->
    ...
    @Test
    fun `the named m2m oidc-client authenticates as openbank-standing-order with its own secret`() {
        val m2m = oidcClient[M2M] as Map<*, *>
        assertThat(m2m["client-id"]).isEqualTo("openbank-standing-order")
