 ...
 */
class StandingOrderOutboxBacklogGaugeTest {

    @Test
    fun `registers a standing-order backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<StandingOrderOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("standing-order"), capture(supplier)) } returns Unit

        val gauge = StandingOrderOutboxBacklogGauge(repo, metrics)
        gauge.register()
