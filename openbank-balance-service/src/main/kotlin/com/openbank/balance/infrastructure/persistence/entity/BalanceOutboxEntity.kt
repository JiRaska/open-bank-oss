...
/**
 * `claimed_at` is balance-only — added straight on this entity, not the shared
 ...
 * `BalanceOutboxRepositoryImpl.claimProcessable`'s atomic claim query on DISPATCHING; read back
 * by the same query to decide if a DISPATCHING row is stale enough to reclaim.
 */
@Entity
@Table(name = "balance_outbox")
class BalanceOutboxEntity : PanacheOutboxEntity() {
