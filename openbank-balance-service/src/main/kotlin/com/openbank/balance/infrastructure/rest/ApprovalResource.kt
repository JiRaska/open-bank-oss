...
class ApprovalResource(private val approvalStore: ApprovalStore) {
    ...
    /**
     * The checker's queue (issue #5679, the balance-service slice of #3472's shape). Without it a
     * parked decision on `balance.credit`/`balance.debit` is invisible: the maker gets a 202 with
     * an approval id and no way to hand it over except out of band, so the four-eyes ceremony only
     * completes if the two operators are already talking, and the Redis TTL (24h) then expires the
     * request silently. Read-only, and deliberately NOT filtered to "approvals someone else made":
     * the self-approval guard lives in `RedisApprovalStore.decide`, and refusing at read time would
     * only hide a maker's own request from them while still letting them attempt it.
     */
    @GET
    ...
    private companion object {
        // Same ceiling as lending's and sanctions' queues. The read is a Redis scan, and an
...
data class ApprovalResponse(
    ...
    // 24h Redis TTL. Additive, so no major bump (ADR-0048); matches sanctions/lending's shape.
    val makerId: String?,
