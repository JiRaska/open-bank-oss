...
import com.openbank.balance.infrastructure.persistence.entity.BalanceOutboxEntity
...
@ApplicationScoped
class BalanceOutboxRepositoryImpl(private val clock: Clock) :
    BalanceOutboxRepository,
    PanacheRepository<BalanceOutboxEntity> {
    ...
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, BalanceOutboxEntity::class.java)
