 ...
 */
class BalanceOutboxBacklogGaugeTest {

    @Test
    fun `registers a balance backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<BalanceOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("balance"), capture(supplier)) } returns Unit

        val gauge = BalanceOutboxBacklogGauge(repo, metrics)
        gauge.register()
