...
dependencies {
    ...
    // Micrometer + Prometheus registry: balance-service had no MeterRegistry, so the shared
    ...
    // Property-based testing of balance/overdraft invariants (ADR-0011 L1 — Kotest property).
    ...
    // Consumer-driven contract tests (ADR-0063); provider dep needed for BalancePactProviderVerificationTest
    testImplementation(libs.pact.consumer)
    testImplementation(libs.pact.provider)
    // CI infra sweep (#578): isolated PostgreSQL + Redpanda per test JVM for *ApiIT.
...
pitest {
    ...
    targetClasses = setOf("com.openbank.balance.domain.*")
    targetTests = setOf("com.openbank.balance.domain.*", "com.openbank.balance.application.usecase.*")
    ...
    threads = 4
    excludedClasses = setOf("com.openbank.balance.domain.*Kt")
}
