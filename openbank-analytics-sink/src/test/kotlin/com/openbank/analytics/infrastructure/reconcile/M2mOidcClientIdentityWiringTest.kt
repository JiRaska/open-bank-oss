...
/**
 * #10486 batch 5: analytics-sink's registry sweep lists accounts (`GET /api/v1/accounts/active`,
 * `account.list`) as its OWN principal, `service-account-openbank-analytics-sink` (ROLE_API only).
 ...
 * `quarkus.oidc-client.m2m` must exist with client `openbank-analytics-sink` and its own secret variable (a
...
class M2mOidcClientIdentityWiringTest {
    ...
    fun `the migrated rest-clients select the named oidc-client and not the default one`() {
        listOf(
            AccountRegistryClient::class.java,
        ).forEach { client ->
    ...
    @Test
    fun `the named oidc-client authenticates as openbank-analytics-sink with its own secret`() {
        val named = oidcClient[NAMED] as Map<*, *>
        assertThat(named["client-id"]).isEqualTo("openbank-analytics-sink")
