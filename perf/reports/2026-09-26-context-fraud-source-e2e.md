...
The probe initially timed out while waiting for complete page loading after authentication; waiting for the document instead of all ancillary requests allowed the real flow to complete. This was a probe correction, not a production authentication change. A subsequent probe also received HTTP 200 from the review-queue BFF in the same authenticated browser. Visual inspection confirmed the actual synthetic queue rows and score summaries. The isolated fixture used a loopback-only relay for the existing review-queue path because the generic local service mapping targets a different port; the relay forwarded the actual bearer token to the actual source and did not fabricate data. This adds local queue/render evidence, not complete scenario acceptance or deployed service-discovery proof. Browser evidence and synthetic screenshots remain machine-local.
