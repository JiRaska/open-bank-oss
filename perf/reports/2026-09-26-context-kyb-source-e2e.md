...
The actual Party API created a synthetic initiator (201, `SYNTHETIC`). A staff API call set its KYC status for fixture identity matching; this is not proof of an external KYC verification workflow. The actual KYB API created one `REGISTRY_VERIFIED` case. Initiator matching and declarations returned 200, with the case reaching `READY_TO_SIGN`. One UBO observation was persisted; its reference outbox and the registry-verification outbox both reached `SENT`. Context contained one KYB observation reference. A subsequent read-only comparison confirmed that its observation ID, revision and source SHA-256 exactly matched the source observation; this comparison does not prove an authorized API read.
...
`perf/scripts/context-kyb-history-e2e.py` now encodes the real maker/checker assignment, assigned history and source comparison, denied reads and owned-assignment cleanup. Verified HTTPS and private distinct-identity token inputs are required. Permanent source restriction and historical suppression checks are explicitly opt-in. Syntax, help and offline input boundaries passed. Live execution with freshly issued real OAuth tokens and verified HTTPS returned `unexpected-http-503` at the maker request; cleanup passed. An earlier token request itself timed out, while the same live identity service subsequently served discovery successfully. A read-only assignment-administration request also returned 503 with policy-decision-point unavailability and OPA timeout markers, narrowing the failure beyond the assignment-write path. Neither attempt proves authorized history acceptance. If approval commits but its response is lost, the available APIs cannot safely recover the owned assignment ID: cleanup fails visibly rather than inferring ownership or retrying approval.
