...
A no-network one-iteration k6 summary-export fixture confirmed that tagged status counters collapse in the summary format used by this baseline. The diagnostic now uses fixed counters per bounded HTTP status bucket, preserving separate counts in exported summaries without altering workload, authentication or acceptance thresholds.
