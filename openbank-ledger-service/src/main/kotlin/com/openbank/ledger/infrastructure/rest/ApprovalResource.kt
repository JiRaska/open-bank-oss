...
class ApprovalResource(private val approvalStore: ApprovalStore) {
     ...
     * parked decision is invisible: the maker gets a 202 with an approval id and has no way to
     * hand it over except out of band, so the four-eyes ceremony on `ledger.reverse` only
     * completed if the two operators were already talking. The Redis TTL (24h) then expired the
     * request silently.
     *
    ...
    @Authorize(action = "ledger.approval.read", resource = "")
    ...
    private companion object {
        // Same ceiling as sanctions/lending's queue. The read is a Redis scan, and an unbounded
        // `limit` from a query parameter is a trivially reachable amplification.
