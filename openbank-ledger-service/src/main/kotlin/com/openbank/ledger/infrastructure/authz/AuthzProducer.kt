...
package com.openbank.ledger.infrastructure.authz
...
/**
 * Wires the ledger-service's [PolicyDecisionPoint] bean (ADR-0034 Phase 5, issue #266).
 * Without this producer, [com.openbank.libs.authz.AuthorizeInterceptor] finds no PDP bean and
 * either proceeds with a WARN (advisory, `authz.enforce=false` — the state before this change)
 * or fails closed with a 503 (enforce=true, no bean — never the intended state). The sca/consent
 * pattern; same defaults (`opa.url`/`opa.path`/`opa.timeout-ms`) as every other OPA-backed service.
 */
@ApplicationScoped
class AuthzProducer {
