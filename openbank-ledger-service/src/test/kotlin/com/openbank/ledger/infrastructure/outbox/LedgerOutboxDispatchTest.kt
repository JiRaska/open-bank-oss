 ...
 * These are pure functions, so no Quarkus/DB/Kafka context is needed.
 */
class LedgerOutboxDispatchTest {

    private fun entry(
        eventId: UUID = UUID.randomUUID(),
        aggregateId: UUID = UUID.randomUUID(),
        eventType: String = "ledger.journal.posted",
    ) = OutboxEntry(
        eventId = eventId,
    ...
    fun `failure below the cap keeps the row retryable (FAILED)`() {
        assertThat(LedgerOutboxRepositoryImpl.statusAfterFailure(1))
            .isEqualTo(OutboxStatus.FAILED)
        assertThat(LedgerOutboxRepositoryImpl.statusAfterFailure(LedgerOutboxRepositoryImpl.MAX_ATTEMPTS - 1))
            .isEqualTo(OutboxStatus.FAILED)
    }

    @Test
    fun `reaching the attempt cap parks the row as terminal DEAD`() {
        assertThat(LedgerOutboxRepositoryImpl.statusAfterFailure(LedgerOutboxRepositoryImpl.MAX_ATTEMPTS))
            .isEqualTo(OutboxStatus.DEAD)
        assertThat(LedgerOutboxRepositoryImpl.statusAfterFailure(LedgerOutboxRepositoryImpl.MAX_ATTEMPTS + 5))
            .isEqualTo(OutboxStatus.DEAD)
    }
    ...
    @Test
    fun `MAX_ATTEMPTS constant aligns with libs OutboxFailurePolicy`() {
        assertThat(LedgerOutboxRepositoryImpl.MAX_ATTEMPTS)
            .isEqualTo(OutboxFailurePolicy.DEFAULT_MAX_ATTEMPTS)
    }
}
