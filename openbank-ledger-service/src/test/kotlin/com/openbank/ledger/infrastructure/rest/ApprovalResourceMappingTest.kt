 ...
 * `openbank-sanctions-service`'s `ApprovalResourceMappingTest` (#3472).
 */
class ApprovalResourceMappingTest {
    ...
    fun `toResponse maps every field including a decided approval`() {
        ...
        assertThat(response.action).isEqualTo("ledger.reverse")
        assertThat(response.resourceId).isEqualTo("journal-1")
