...
/**
 * Postgres-backed finding repository (ADR-0168). Replaces the in-memory store so findings — and their
...
class PostgresFindingRepository(private val sf: Mutiny.SessionFactory) : FindingRepository {
    ...
    override suspend fun update(finding: FlakyTestFinding): FlakyTestFinding = sf.withTransaction { s ->
    ...
    override suspend fun findActive(): List<FlakyTestFinding> = sf.withSession { s ->
        s.createQuery(
    ...
    override suspend fun findById(id: String): FlakyTestFinding? =
...
private fun FindingEntity.applyFrom(f: FlakyTestFinding): FindingEntity {
...
private fun FindingEntity.toDomain(): FlakyTestFinding = FlakyTestFinding(
    id = id.toString(),
    checkType = checkType,
    severity = severity,
