 ...
 *
 * Deliberately not a direct call to [FlakyTestCheckScheduler.runScheduledCheck]. A direct call proves the method
...
@TestProfile(FlakyTestCheckSchedulerCronIT.EverySecondProfile::class)
class FlakyTestCheckSchedulerCronIT {

    class EverySecondProfile : QuarkusTestProfile {
        override fun getConfigOverrides(): Map<String, String> = mapOf(
            // Literals only — see the class KDoc.
            "openbank.flaky-test-hunter.check-cron" to "*/1 * * * * ?",
            "openbank.temporal.enabled" to "false",
        )
    }

    @Inject
    lateinit var scheduler: FlakyTestCheckScheduler
    ...
    fun `the real cron fires the scheduled sweep`() {
        ...
        assertTrue(
            ...
            "The scheduled flaky-test-hunter sweep never fired within $TIMEOUT_SECONDS s. The cron " +
