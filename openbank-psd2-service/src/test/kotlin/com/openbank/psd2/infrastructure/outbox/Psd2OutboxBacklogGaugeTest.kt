 ...
 */
class Psd2OutboxBacklogGaugeTest {

    @Test
    fun `registers a psd2 backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<Psd2OutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("psd2"), capture(supplier)) } returns Unit

        val gauge = Psd2OutboxBacklogGauge(repo, metrics)
        gauge.register()
