...
/**
 * Regression guard (C7/ADR-0030): no sanctions list endpoint must be opened without role-gating.
 * Pure reflection — no Quarkus boot needed.
 */
class SanctionsListSecurityTest {

    @Test
    fun `no SanctionsListResource endpoint is @PermitAll`() {
        val methods = SanctionsListResource::class.java.declaredMethods.filter { m ->
