...
/**
 * `claimed_at` is sanctions-only — added straight on this entity, not the shared
 ...
 * `SanctionsOutboxRepositoryImpl.claimProcessable`'s atomic claim query on DISPATCHING; read
 * back by the same query to decide if a DISPATCHING row is stale enough to reclaim.
 */
@Entity
@Table(name = "sanctions_outbox")
class SanctionsOutboxEntity : PanacheOutboxEntity() {
