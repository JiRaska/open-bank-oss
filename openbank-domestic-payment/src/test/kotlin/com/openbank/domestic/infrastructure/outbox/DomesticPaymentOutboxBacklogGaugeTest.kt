 ...
 */
class DomesticPaymentOutboxBacklogGaugeTest {

    @Test
    fun `registers a domestic backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<DomesticPaymentOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("domestic"), capture(supplier)) } returns Unit

        val gauge = DomesticPaymentOutboxBacklogGauge(repo, metrics)
        gauge.register()
