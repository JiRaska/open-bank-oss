 ...
 */
@Path("/api/v1/domestic-payments/approvals")
@Produces(MediaType.APPLICATION_JSON)
@Tag(name = "Domestic Payment Approvals", description = "Four-eyes decisions for gated domestic payment actions")
class ApprovalResource(private val approvalStore: ApprovalStore) {
    ...
    /**
     * The checker's queue (issue #5679, mirroring sanctions #3472, lending and ledger). Without it
     * a parked decision is invisible: the maker gets a 202 with an approval id and has no way to
     * hand it over except out of band, so the four-eyes ceremony on `domestic-payment.transitionStatus`
     * only completed if the two operators were already talking. The Redis TTL (24h) then expired
     * the request silently.
     *
     * Read-only, and deliberately NOT filtered to "approvals someone else made": the self-approval
     * guard lives in `RedisApprovalStore.decide`, and refusing at read time would only hide a
     * maker's own request from them while still letting them attempt it. Seeing the queue is not
     * authority over it.
     */
    @GET
    @RolesAllowed("ROLE_OPERATOR", "ROLE_ADMIN", "ROLE_PAYMENTS")
    @Authorize(action = "domestic-payment.approval.read", resource = "")
    ...
    private companion object {
        // Same ceiling as sanctions/lending/ledger's queue. The read is a Redis scan, and an
...
data class ApprovalResponse(
    ...
    // 24h Redis TTL. Additive, so no major bump (ADR-0048); matches sanctions/lending/ledger's shape.
    val makerId: String?,
