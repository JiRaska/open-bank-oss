...
class FinOpsWorkflowLivenessTest {
    ...
    fun `scheduler registers gauge at startup and records success after workflow dispatch`(): Unit = runBlocking {
        ...
        val runAnalysis = mockk<RunFinOpsAnalysisUseCase>()
        val temporalConfig = mockk<TemporalConfig>()
        every { temporalConfig.enabled() } returns true
        coEvery { runAnalysis.startDetached(RunTrigger.SCHEDULED) } returns "wf-1"

        val scheduler = FinOpsAnalysisScheduler().apply {
    ...
    fun `failed dispatch leaves liveness old`() {
        ...
        val temporalConfig = mockk<TemporalConfig>()
        every { temporalConfig.enabled() } returns true
        coEvery { runAnalysis.startDetached(RunTrigger.SCHEDULED) } throws IllegalStateException("temporal down")
        ...
        assertThatThrownBy { runBlocking { scheduler.runScheduledAnalysis() } }
    ...
    private companion object {
        const val WORKFLOW = "finops-analysis"
