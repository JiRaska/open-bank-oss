...
/**
 * `claimed_at` is party-only — added straight on this entity, not the shared
 ...
 * `PartyOutboxRepositoryImpl.claimProcessable`'s atomic claim query on DISPATCHING; read back
 * by the same query to decide if a DISPATCHING row is stale enough to reclaim.
 */
@Entity
@Table(name = "party_outbox")
class PartyOutboxEntity : PanacheOutboxEntity() {
