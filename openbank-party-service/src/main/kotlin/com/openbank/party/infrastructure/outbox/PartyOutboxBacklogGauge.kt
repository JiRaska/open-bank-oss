...
/**
 * Publishes the party outbox backlog as `openbank.outbox.backlog{service="party"}`.
 * All the logic lives in [AbstractOutboxBacklogGauge] (ADR-0049 consolidation); this bean
 * only binds the party repository + the CDI/scheduler annotations (which, per ADR-0013,
 * must sit on the concrete bean).
 */
@Startup
@ApplicationScoped
class PartyOutboxBacklogGauge : AbstractOutboxBacklogGauge {
    private lateinit var outboxRepository: PartyOutboxRepository

    @Inject
    constructor(outboxRepository: PartyOutboxRepository, metrics: DomainMetrics) : super(metrics) {
