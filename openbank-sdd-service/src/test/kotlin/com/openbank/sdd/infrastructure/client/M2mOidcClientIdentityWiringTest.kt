 ...
 * `service-account-openbank-sdd` (ROLE_API only), not the shared `openbank-services` one. Two
 ...
 * `openbank-sdd` and its own secret variable (a missing named client fails the first call, not
 * the build).
 */
class M2mOidcClientIdentityWiringTest {
    ...
    @Test
    fun `the named m2m oidc-client authenticates as openbank-sdd with its own secret`() {
        val m2m = oidcClient[M2M] as Map<*, *>
        assertThat(m2m["client-id"]).isEqualTo("openbank-sdd")
