...
import com.openbank.pid.infrastructure.persistence.entity.PidOutboxEntity
...
@ApplicationScoped
class PidOutboxRepositoryImpl(private val clock: Clock) :
    PidOutboxRepository,
    PanacheRepository<PidOutboxEntity> {
    ...
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, PidOutboxEntity::class.java)
