...
/**
 * `claimed_at` is pid-only — added straight on this entity, not the shared
 ...
 * `PidOutboxRepositoryImpl.claimProcessable`'s atomic claim query on DISPATCHING; read back
 * by the same query to decide if a DISPATCHING row is stale enough to reclaim.
 */
@Entity
@Table(name = "pid_outbox")
class PidOutboxEntity : PanacheOutboxEntity() {
