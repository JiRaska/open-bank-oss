...
/**
 * Regression coverage for #1201: two dispatcher instances racing [PidOutboxRepositoryImpl]'s
...
class PidOutboxClaimIT {

    @Inject
    lateinit var repository: PidOutboxRepositoryImpl

    private fun <T> onEventLoop(block: suspend () -> T): T =
        VertxContextSupport.subscribeAndAwait { uni(CoroutineScope(Dispatchers.Unconfined)) { block() } }

    // The @QuarkusTest Postgres is shared across test classes (same convention as the ledger
    // sibling test) — without this, backlog rows other IT classes left behind get scooped up by
    // this test's claimProcessable calls too.
