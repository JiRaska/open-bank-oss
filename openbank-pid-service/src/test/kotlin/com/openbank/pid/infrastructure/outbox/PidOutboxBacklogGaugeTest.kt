 ...
 */
class PidOutboxBacklogGaugeTest {

    @Test
    fun `registers a pid backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<PidOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("pid"), capture(supplier)) } returns Unit

        val gauge = PidOutboxBacklogGauge(repo, metrics)
        gauge.register()
