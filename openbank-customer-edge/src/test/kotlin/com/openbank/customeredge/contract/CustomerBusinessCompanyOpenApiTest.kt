...
class CustomerBusinessCompanyOpenApiTest {
    ...
    fun `the schema carries exactly the company fields`() {
        ...
        assertThat(props.path("accounts").path("items").path("\$ref").asText())
            .isEqualTo("#/components/schemas/BusinessCompanyAccount")
        val businessAccount = spec.path("components").path("schemas").path("BusinessCompanyAccount")
        assertThat(businessAccount.path("properties").keys())
    ...
    fun `company account pages publish the mandate header and next cursor`() {
        ...
        assertThat(pageSchema).isEqualTo("#/components/schemas/BusinessCompanyAccountPage")
        val page = spec.path("components").path("schemas").path("BusinessCompanyAccountPage")
        assertThat(page.path("properties").path("data").path("items").path("\$ref").asText())
            .isEqualTo("#/components/schemas/BusinessCompanyAccount")
        assertThat(page.path("properties").path("pagination").path("\$ref").asText())
            .isEqualTo("#/components/schemas/Pagination")
    }
}
