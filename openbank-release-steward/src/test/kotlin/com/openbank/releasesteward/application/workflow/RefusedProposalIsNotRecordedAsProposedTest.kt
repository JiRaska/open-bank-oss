...
import com.openbank.releasesteward.infrastructure.adapter.GitHubProposalAdapter
 ...
 * `ReleaseStewardWorkflowImpl` reports — the two places a fabricated URL was read as success.
 */
class RefusedProposalIsNotRecordedAsProposedTest {

    private class RecordingRepository : FindingRepository {
        var updated: ReleaseStewardFinding? = null
        override suspend fun save(finding: ReleaseStewardFinding): ReleaseStewardFinding = finding
    ...
    private fun finding() = ReleaseStewardFinding(
        ...
        title = "openbank-audit-service application.yaml sets quarkus.application.version explicitly",
    ...
    private fun proposeWith(fixDiff: String?): Pair<ReleaseStewardFinding, RecordingRepository> {
    ...
    fun `a refused fix-diff PR also leaves the finding DIAGNOSED`() {
        val (result, repository) = proposeWith(
            fixDiff = "--- a/openbank-audit-service/src/main/resources/application.yaml\n" +
    ...
    /**
     * The consumer assertion: `ReleaseStewardWorkflowImpl` reports `findingsProposed` as
     * `count { it.status == PROPOSED }`. A refusal must contribute zero to that number — this is
     * the exact count the fabricated `pending-release-steward-<id>` URL used to inflate.
     */
    @Test
