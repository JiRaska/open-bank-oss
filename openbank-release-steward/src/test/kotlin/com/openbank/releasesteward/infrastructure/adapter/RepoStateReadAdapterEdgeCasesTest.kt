...
class RepoStateReadAdapterEdgeCasesTest {
    ...
    @Test
    fun `malformed registry JSON degrades to an empty set rather than aborting the sweep`(@TempDir root: Path): Unit =
        runBlocking {
    ...
    @Test
    fun `a directory not prefixed openbank- is never a module, even with a version-txt`(@TempDir root: Path): Unit =
        runBlocking {
    ...
    @Test
    fun `an openbank- directory WITHOUT a version-txt is not a released component`(@TempDir root: Path): Unit =
        runBlocking {
    ...
    @Test
    fun `a top-level version-txt FILE named like a module is not mistaken for a directory`(@TempDir root: Path): Unit =
        runBlocking {
    ...
    @Test
    fun `a malformed admin-ui package-json degrades to null instead of throwing`(@TempDir root: Path): Unit =
        runBlocking {
            Files.createDirectories(root.resolve("openbank-admin-ui"))
    ...
    @Test
    fun `the admin-ui version-txt read is trimmed of its trailing newline`(@TempDir root: Path): Unit = runBlocking {
        Files.createDirectories(root.resolve("openbank-admin-ui"))
    ...
    @Test
    fun `a module with no application-yaml at all is not scanned as an override`(@TempDir root: Path): Unit =
        runBlocking {
            Files.createDirectories(root.resolve("openbank-ledger-service"))
            Files.writeString(root.resolve("openbank-ledger-service/version.txt"), "1.0.0\n")
    ...
    @Test
    fun `only the services that actually override are listed, in sorted order`(@TempDir root: Path): Unit =
        runBlocking {
