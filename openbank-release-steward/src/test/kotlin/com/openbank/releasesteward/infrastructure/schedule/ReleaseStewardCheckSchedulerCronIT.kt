...
import com.openbank.releasesteward.PostgresTestResource
 ...
 *
 * Deliberately not a direct call to [ReleaseStewardCheckScheduler.runScheduledCheck]. A direct call proves the method
...
@TestProfile(ReleaseStewardCheckSchedulerCronIT.EverySecondProfile::class)
class ReleaseStewardCheckSchedulerCronIT {

    class EverySecondProfile : QuarkusTestProfile {
        override fun getConfigOverrides(): Map<String, String> = mapOf(
            // Literals only — see the class KDoc.
            "openbank.release-steward.check-cron" to "*/1 * * * * ?",
            "openbank.temporal.enabled" to "false",
        )
    }

    @Inject
    lateinit var scheduler: ReleaseStewardCheckScheduler
    ...
    fun `the real cron fires the scheduled sweep`() {
        ...
        assertTrue(
            ...
            "The scheduled release-steward sweep never fired within $TIMEOUT_SECONDS s. The cron " +
