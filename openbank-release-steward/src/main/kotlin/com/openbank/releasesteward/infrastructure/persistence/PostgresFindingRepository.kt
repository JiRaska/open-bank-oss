...
/**
 * Postgres-backed finding repository (ADR-0165). Replaces the in-memory store so findings — and their
...
class PostgresFindingRepository(private val sf: Mutiny.SessionFactory) : FindingRepository {
    ...
    override suspend fun update(finding: ReleaseStewardFinding): ReleaseStewardFinding = sf.withTransaction { s ->
    ...
    override suspend fun findActive(): List<ReleaseStewardFinding> = sf.withSession { s ->
        s.createQuery(
    ...
    override suspend fun findById(id: String): ReleaseStewardFinding? =
...
private fun FindingEntity.applyFrom(f: ReleaseStewardFinding): FindingEntity {
...
private fun FindingEntity.toDomain(): ReleaseStewardFinding = ReleaseStewardFinding(
    id = id.toString(),
    checkType = checkType,
    severity = severity,
