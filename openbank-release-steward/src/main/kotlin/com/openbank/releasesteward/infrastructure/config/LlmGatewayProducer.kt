...
/**
 * Produces the shared [LlmGatewayPort] (ADR-0174) for release-steward, wired to the same LiteLLM
...
class LlmGatewayProducer {
    ...
    @Produces
    @ApplicationScoped
    fun llmGateway(config: ReleaseStewardConfig, metrics: LlmCallMetrics): LlmGatewayPort {
        val apiKey = ConfigProvider.getConfig()
            .getOptionalValue("release-steward.model.api-key", String::class.java)
            .orElse("")
