 ...
 * `relation "<table>_seq" does not exist`. This defect shipped repeatedly across services
 * (party V5/V6, notification V4/V5) because nothing failed the build for it.
 *
 * This test fails when any `: PanacheEntity()` table in this service lacks a matching, unquoted,
 * lowercase `<table>_seq` sequence in the Flyway migrations. A quoted `"<table>_SEQ"` does NOT
 * satisfy it — Postgres treats that as a distinct case-sensitive identifier (exactly the party V5
 * mistake that V6 had to undo).
 */
class HibernateSequenceGuardTest {
    ...
    /**
     * Returns the set of sequence identifiers as Postgres would store them: an unquoted name is
     * folded to lower case, a "quoted" name keeps its exact casing (and therefore will not match a
