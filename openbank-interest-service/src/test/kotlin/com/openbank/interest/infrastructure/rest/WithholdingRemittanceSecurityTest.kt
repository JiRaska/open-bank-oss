...
/**
 * Regression guard (C7/ADR-0030): no interest withholding endpoint must be opened without role-gating.
 * Pure reflection — no Quarkus boot needed.
 */
class WithholdingRemittanceSecurityTest {

    @Test
    fun `no WithholdingRemittanceResource endpoint is @PermitAll`() {
        val methods = WithholdingRemittanceResource::class.java.declaredMethods.filter { m ->
