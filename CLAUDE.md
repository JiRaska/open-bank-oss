...
- **ARC runner tool cache (JDK/Node) re-downloaded on every job if `RUNNER_TOOL_CACHE` not on hostPath.** `actions/setup-java` downloads JDK 21 + JDK 25 (~380 MB total) from GitHub CDN (`185.199.x.x`, `57.150.x.x`) per pod per job if `RUNNER_TOOL_CACHE` is not on NVMe. With 30 services × 60 CI runs/day = 600+ GB/day NAT ($27+/day). Fix: add `RUNNER_TOOL_CACHE=/mnt/k8s-disks/0/runner-tool-cache` env + hostPath volume to ARC runner spec (same pattern as `GRADLE_USER_HOME` — `arc-runners.tf`). Symptom in VPC Flow Logs: `BytesOutToDestination` dominated by `185.199.x.x`/`57.150.x.x`. The EC2 VPC endpoint, docker.io prefix, and ECR pull-through fixes do NOT address this — they cover image pulls, not runner tool downloads. Enforced by CloudWatch alarm `openbank-nat-egress-20gb-per-hour` + daily NAT budget `openbank-nat-egress-daily` (both in `sandbox-substrate/finops-budget.tf`).
