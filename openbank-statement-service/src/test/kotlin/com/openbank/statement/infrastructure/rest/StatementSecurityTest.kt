...
/**
 * Regression guard (C7/ADR-0030): no statement endpoint must be opened without role-gating.
 * Pure reflection — no Quarkus boot needed.
 */
class StatementSecurityTest {

    @Test
    fun `no StatementResource endpoint is @PermitAll`() {
        val methods = StatementResource::class.java.declaredMethods.filter { m ->
