 ...
 */
class StatementOutboxBacklogGaugeTest {

    @Test
    fun `registers a statement backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<StatementOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("statement"), capture(supplier)) } returns Unit

        val gauge = StatementOutboxBacklogGauge(repo, metrics)
        gauge.register()
