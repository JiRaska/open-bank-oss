...
@ApplicationScoped
class ContextOwnershipAccessAdapter(
    @RestClient private val client: ContextOwnershipAccessRestClient,
    @ConfigProperty(name = "openbank.kyb.data-free-context-access-enabled", defaultValue = "false")
    private val dataFreeAccessEnabled: Boolean,
) : UboObservationAccess {

    @Timeout(ASSIGNMENT_TIMEOUT_MS)
    override suspend fun check(caseId: UUID, bearer: String): UboObservationAccessDecision = try {
        val response = if (dataFreeAccessEnabled) {
            client.access(caseId, bearer, caseId.toString(), "KYB_OWNERSHIP_REVIEW")
        } else {
            client.history(caseId, bearer, caseId.toString(), "KYB_OWNERSHIP_REVIEW")
        }
        val allowedStatus = if (dataFreeAccessEnabled) HTTP_NO_CONTENT else HTTP_OK
        response.use {
            when (response.status) {
                allowedStatus -> UboObservationAccessDecision.ALLOWED
