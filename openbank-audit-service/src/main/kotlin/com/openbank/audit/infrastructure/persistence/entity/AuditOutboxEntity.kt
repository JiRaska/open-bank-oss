...
package com.openbank.audit.infrastructure.persistence.entity
...
/**
 * `claimed_at` is audit-only — added straight on this entity, not the shared
 ...
 * `AuditOutboxRepositoryImpl.claimProcessable`'s atomic claim query on DISPATCHING; read back
 * by the same query to decide if a DISPATCHING row is stale enough to reclaim.
 */
@Entity
@Table(name = "audit_outbox")
class AuditOutboxEntity : PanacheOutboxEntity() {
