...
package com.openbank.audit.infrastructure.persistence.repository

import com.openbank.audit.application.port.out.AuditOutboxRepository
import com.openbank.audit.infrastructure.persistence.entity.AuditOutboxEntity
...
@ApplicationScoped
class AuditOutboxRepositoryImpl(private val clock: Clock) :
    AuditOutboxRepository,
    PanacheRepository<AuditOutboxEntity> {
    ...
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, AuditOutboxEntity::class.java)
