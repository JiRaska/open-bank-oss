...
| [02 — Architecture](./02-architecture.md) | Engineering, tech leads | C4 diagramy, hexagonální vrstvy, consume + outbox flow |
...
- **Persistence:** PostgreSQL databáze `openbank_audit`, tabulka `audit_entries` (append-only, schema `public`), Flyway migrace V1..V4
...
- **Outbox:** tabulka `audit_outbox` + plánovaný dispatcher (re-emit cesta, viz [02](./02-architecture.md))
