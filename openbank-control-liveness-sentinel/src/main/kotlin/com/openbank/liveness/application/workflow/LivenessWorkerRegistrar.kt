...
class LivenessWorkerRegistrar(
    ...
    private val log = Logger.getLogger(LivenessWorkerRegistrar::class.java)
    ...
    fun onStart(@Observes event: StartupEvent) {
        ...
        log.infof(
            "Registering Temporal control-liveness-sentinel worker on task queue '%s'",
            temporalConfig.taskQueue(),
        )
        val factory = WorkerFactory.newInstance(workflowClient)
        val worker = factory.newWorker(temporalConfig.taskQueue())
        worker.registerWorkflowImplementationTypes(LivenessCheckWorkflowImpl::class.java)
        worker.registerActivitiesImplementations(collectSignals, detectFindings, diagnoseAndPropose)
        factory.start()
        log.infof("Temporal control-liveness-sentinel worker started on task queue '%s'", temporalConfig.taskQueue())
    }
}
