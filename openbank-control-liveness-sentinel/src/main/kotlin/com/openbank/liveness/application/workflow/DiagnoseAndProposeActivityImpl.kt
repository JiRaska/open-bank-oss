...
open class DiagnoseAndProposeActivityImpl(
    ...
    override fun diagnose(finding: LivenessFinding, contextMetrics: Map<String, Double>): LivenessFinding =
        runOnVertxContext {
            log.infof("Diagnosing finding %s (%s)", finding.id, finding.mechanism)
    ...
    override fun propose(finding: LivenessFinding): LivenessFinding = runOnVertxContext {
        log.infof("Proposing fix for finding %s (%s)", finding.id, finding.mechanism)
