...
/**
 * Outbound port for a capability the assistant may invoke on the customer's behalf (ADR-0089).
 *
 * Adapters live in `infrastructure.tool`; the chat loop reaches them only through
 * `CopilotToolRegistry`, which discovers every implementation via CDI. Phase-1 tools are
