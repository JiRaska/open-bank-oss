 ...
 */
class ClearingOutboxBacklogGaugeTest {

    @Test
    fun `registers a clearing backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<ClearingOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("clearing"), capture(supplier)) } returns Unit

        val gauge = ClearingOutboxBacklogGauge(repo, metrics)
        gauge.register()
