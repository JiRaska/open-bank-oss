...
/**
 * `claimed_at` is clearing-only — added straight on this entity, not the shared
 ...
 * `ClearingOutboxRepositoryImpl.claimProcessable`'s atomic claim query on DISPATCHING; read back
 * by the same query to decide if a DISPATCHING row is stale enough to reclaim.
 */
@Entity
@Table(name = "clearing_outbox")
class ClearingOutboxEntity : PanacheOutboxEntity() {
