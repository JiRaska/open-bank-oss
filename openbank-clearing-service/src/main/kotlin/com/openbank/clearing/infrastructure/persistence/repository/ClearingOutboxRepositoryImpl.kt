...
@ApplicationScoped
class ClearingOutboxRepositoryImpl(private val clock: Clock) :
    ...
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, ClearingOutboxEntity::class.java)
    ...
    companion object {
        ...
        private const val CLAIM_SQL = """
            UPDATE clearing_outbox
            SET status = :dispatching, claimed_at = :now, updated_at = :now
            WHERE id IN (
                SELECT id FROM clearing_outbox
