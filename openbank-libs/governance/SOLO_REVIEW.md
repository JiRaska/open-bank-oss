...
remaining provider spending controls. Authentication is explicitly selected:
`AGENT_REVIEW_AUTH_MODE=api` (the default) requires `AGENT_REVIEW_API_ENABLED=true`
and the dedicated `AGENT_REVIEW_ANTHROPIC_API_KEY` secret, with a provider-enforced
spending limit. The owner-selected `subscription` mode instead requires
`AGENT_REVIEW_SUBSCRIPTION_ENABLED=true` and `CLAUDE_CODE_OAUTH_TOKEN`.
A missing credential or disabled mode fails before provider invocation, without
fallback. Only the selected credential reaches the model subprocess. Temporary
HOME and config paths prevent discovery of any other local login.
Subscription mode bounds dispatches, turns, input, output and runtime; CLI dollar
estimates do not reserve or cap the shared subscription allowance. Other uses of
the subscription remain outside this workflow's control.
...
unknown usage must be reconciled before another paid dispatch. Automatic mutating
workers remain parked without provider credentials. The new
