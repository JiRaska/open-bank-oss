...
/**
 * #10486 pattern (ADR-0315 D5): this service's money-path writes are made as its OWN principal,
 * `service-account-openbank-treasury` (ROLE_API only), not the shared `openbank-services` one. Two
 ...
 * `openbank-treasury` and its own secret variable (a missing named client fails the first call, not
 * the build).
 */
class M2mOidcClientIdentityWiringTest {
    ...
    fun `the money-path rest-clients select the named m2m oidc-client and not the default one`() {
        listOf(LedgerRestClient::class.java).forEach { client ->
    ...
    @Test
    fun `the named m2m oidc-client authenticates as openbank-treasury with its own secret`() {
        val m2m = oidcClient[M2M] as Map<*, *>
        assertThat(m2m["client-id"]).isEqualTo("openbank-treasury")
