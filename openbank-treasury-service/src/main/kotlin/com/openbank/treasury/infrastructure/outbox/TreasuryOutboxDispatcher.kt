...
package com.openbank.treasury.infrastructure.outbox
...
/**
 * Relays `treasury_outbox` rows to Kafka.
 *
 * `dispatch` is a **suspend** function on purpose. A plain `@Scheduled` method carries no Vert.x
 * context — Quarkus invokes it on a bare executor thread — so a body wrapping a reactive Panache
 * call in `runBlocking` throws `HR000068` and the job aborts having done nothing, silently. Five
 * schedulers in this fleet had never run for that reason (#2148, #2187), which is why
 * `check-no-runblocking-in-scheduled.py` now enforces the shape.
 */
@ApplicationScoped
class TreasuryOutboxDispatcher(
    private val repo: TreasuryOutboxRepository,
    ...
    @Scheduled(
        ...
        identity = "treasury-outbox-dispatcher",
    )
    @Bulkhead(1)
