...
class TreasuryOutboxRepositoryImpl(private val clock: Clock) :
    ...
    /**
     * Atomic claim (#1201 pattern): two dispatcher pods can never both take one row. Needed even
     * at `replicas: 1`, because an Argo Rollouts canary runs the old and new pod at once and both
     * run every `@Scheduled` bean regardless of traffic weight.
     */
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, TreasuryOutboxEntity::class.java)
    ...
    /** Returns the status actually PERSISTED, never one predicted before the write (#5128). */
    ...
    private fun OutboxMessage.toEntity() = TreasuryOutboxEntity().also {
        it.eventId = eventId
    ...
    private fun TreasuryOutboxEntity.toEntry() = OutboxEntry(
        eventId = eventId,
    ...
    companion object {
        private val CLAIM_SQL = """
            UPDATE treasury_outbox AS claimed
            SET status = :dispatching, claimed_at = :now, updated_at = :now
            WHERE claimed.id IN (
                SELECT candidate.id FROM treasury_outbox AS candidate
