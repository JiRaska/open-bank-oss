...
Those four rules are structurally incapable of firing. `TransactionSagaStuck` is a
critical money-path alert whose whole job is to page someone when a saga wedges, and it
has never been able to do that. Nothing anywhere disagrees: the rule loads cleanly,
...
# Alert rules that are dead TODAY, declared rather than hidden (issue #5733). Keyed by
# "<alert>#<metric>" so fixing one alert does not silently excuse another on the same
# metric. Each stays only until that alert is instrumented, renamed or deleted -- the list
# is checked BOTH ways below, so an entry that stops reproducing is itself reported and it
...
KNOWN_DEAD: dict[str, str] = {
    ...
    "SwiftMtMessageProcessingStalled#openbank_swift_messages_processed_total":
        "#5733 -- same `== 0` shape.",
    "LendingRepaymentProcessingStalled#openbank_lending_repayments_processed_total":
        "#5733 -- same `== 0` shape.",
}
