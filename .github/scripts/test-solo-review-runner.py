...
class DriverTest(unittest.TestCase):
    def setUp(self):
        credentials = patch.dict(os.environ, REVIEW_AUTH_MODE="api", AGENT_REVIEW_API_ENABLED="true", ANTHROPIC_API_KEY="api-fixture")
        credentials.start()
        self.addCleanup(credentials.stop)

    def test_subscription_cli_retains_isolation_and_limits(self):
        with patch.dict(os.environ, REVIEW_AUTH_MODE="subscription", AGENT_REVIEW_SUBSCRIPTION_ENABLED="true"):
            self.test_cli_isolated_and_has_no_repository_token()

    def test_subscription_requires_own_opt_in_and_credential(self):
        for enabled, token in (("false", "fixture"), ("", "fixture"), ("true", "")):
            with self.subTest(enabled=enabled, token=bool(token)), patch.dict(
                    os.environ, REVIEW_AUTH_MODE="subscription", AGENT_REVIEW_SUBSCRIPTION_ENABLED=enabled,
                    CLAUDE_CODE_OAUTH_TOKEN=token), patch.object(runner, "anchored") as anchor:
                with self.assertRaisesRegex(ValueError, "no provider invocation"):
                    runner.review("missing.json", "unused.json", "correctness", "/trusted/claude")
    ...
    def test_cli_isolated_and_has_no_repository_token(self, policy_check):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            if os.environ.get("REVIEW_AUTH_MODE") == "subscription":
...
class SourceAndAcceptanceTest(unittest.TestCase):
    ...
    def test_workflow_enforces_readonly_models_and_acceptance_order(self):
        ...
        self.assertIn("vars.AGENT_REVIEW_SUBSCRIPTION_ENABLED == 'true'", workflow.read_text())
