...
def selected_credential():
    mode = os.environ.get("REVIEW_AUTH_MODE", "api")
    require(mode in ("api", "subscription"), "unsupported review authentication mode")
    flag, credential = (("AGENT_REVIEW_API_ENABLED", "ANTHROPIC_API_KEY") if mode == "api" else
                        ("AGENT_REVIEW_SUBSCRIPTION_ENABLED", "CLAUDE_CODE_OAUTH_TOKEN"))
    require(os.environ.get(flag) == "true", "selected review mode is disabled; no provider invocation")
    require(bool(os.environ.get(credential, "").strip()),
            "selected review credential is missing; no provider invocation")
    return credential
...
def review(input_path, output, slot, cli):
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="solo-review-") as directory:
            # Only the explicitly selected credential is inherited; never discover local logins.
            env["HOME"] = directory
