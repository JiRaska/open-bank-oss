...
def render_rca(records: list[dict], now: dt.datetime) -> str:
...
def self_test() -> None:
    ...
    assert "P90 = " in render_rca(fine, now)
    flagged_doc = render_rca(rec("Recurring", [10, 15, 19]), now)
    assert "**`Recurring`** — 3 fires" in flagged_doc, flagged_doc
    assert "No alert reached the recurrence threshold" in render_rca(rec("Twice", [18, 19]), now)
...
def main() -> int:
    ...
    if args.rca_output:
        with open(args.rca_output, "w", encoding="utf-8") as stream:
            stream.write(render_rca(records, now))
    ...
    mttr = mttr_p90(records)
    print(json.dumps({"mttr": mttr, "recurrences": flagged}, indent=2, sort_keys=True))
    return 0
