...
class DocsTruthService(
    ...
    override suspend fun startDetached(trigger: RunTrigger): String {
        ...
        val options = WorkflowOptions.newBuilder()
            ...
            .build()
        val stub = workflowClient.newWorkflowStub(DocsTruthWorkflow::class.java, options)
        try {
            WorkflowClient.start({ stub.runCheck(trigger) })
            log.infof("Started docs-truth-agent sweep workflow %s (trigger=%s)", workflowId, trigger)
    ...
    companion object {
        ...
        fun scheduledWorkflowId(trigger: RunTrigger, at: Instant): String =
            "docs-truth-agent-check-${trigger.name.lowercase()}-${DAY.format(at)}"
    }
}
