...
package com.openbank.docstruth.infrastructure.schedule
...
/**
 * The periodic sweep ADR-0166 decided and nothing ever built.
 *
 * ADR-0166 chose the cadence and not the hour: "Runs on a periodic sweep (weekly) rather than a reactive webhook"
 * — an ADR's status going stale has no natural trigger moment, so the periodic sweep is not a backstop here, it is
 * the PRIMARY path. It was never built, so only the reactive ADR-file-changed trigger existed.
 *
 * `RunTrigger.SCHEDULED` has existed in this agent's domain model since the service was written, and nothing ever
 * emitted it. The agent was trigger-only (`POST /api/v1/docs-truth-agent/check/trigger`), so it had checked
 ...
 */
@ApplicationScoped
class DocsTruthCheckScheduler {

    @Inject
    lateinit var runCheck: RunDocsTruthCheckUseCase
    ...
    private val log = Logger.getLogger(DocsTruthCheckScheduler::class.java)
    ...
    @Scheduled(
        cron = "{openbank.docs-truth-agent.check-cron}",
        concurrentExecution = Scheduled.ConcurrentExecution.SKIP,
    )
    suspend fun runScheduledCheck() {
        fires.incrementAndGet()
        if (!temporalConfig.enabled()) {
            log.info("Temporal disabled; skipping the scheduled docs-truth-agent sweep")
            return
        }
        val workflowId = runCheck.startDetached(RunTrigger.SCHEDULED)
        liveness?.recordSuccess()
        log.infof("Scheduled docs-truth-agent sweep dispatched as workflow %s", workflowId)
    }

    private companion object {
        const val WORKFLOW_NAME = "docs-truth-check"
