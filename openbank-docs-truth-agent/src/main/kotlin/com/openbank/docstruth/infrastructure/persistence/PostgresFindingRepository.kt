...
/**
 * Postgres-backed finding repository (ADR-0166). Replaces the in-memory store so findings — and their
...
class PostgresFindingRepository(private val sf: Mutiny.SessionFactory) : FindingRepository {
    ...
    override suspend fun update(finding: DocsTruthFinding): DocsTruthFinding = sf.withTransaction { s ->
    ...
    override suspend fun findActive(): List<DocsTruthFinding> = sf.withSession { s ->
        s.createQuery(
    ...
    override suspend fun findById(id: String): DocsTruthFinding? =
...
private fun FindingEntity.applyFrom(f: DocsTruthFinding): FindingEntity {
...
private fun FindingEntity.toDomain(): DocsTruthFinding = DocsTruthFinding(
    id = id.toString(),
    checkType = checkType,
    severity = severity,
