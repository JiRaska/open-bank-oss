...
import com.openbank.docstruth.PostgresTestResource
 ...
 *
 * Deliberately not a direct call to [DocsTruthCheckScheduler.runScheduledCheck]. A direct call proves the method
 * body works and says nothing about whether anything ever invokes it — it passes identically
 * against the service as it shipped, which had no schedule at all and had therefore checked
...
@TestProfile(DocsTruthCheckSchedulerCronIT.EverySecondProfile::class)
class DocsTruthCheckSchedulerCronIT {

    class EverySecondProfile : QuarkusTestProfile {
        override fun getConfigOverrides(): Map<String, String> = mapOf(
            // Literals only — see the class KDoc.
            "openbank.docs-truth-agent.check-cron" to "*/1 * * * * ?",
            "openbank.temporal.enabled" to "false",
        )
    }

    @Inject
    lateinit var scheduler: DocsTruthCheckScheduler
    ...
    fun `the real cron fires the scheduled sweep`() {
        ...
        assertTrue(
            ...
            "The scheduled docs-truth-agent sweep never fired within $TIMEOUT_SECONDS s. The cron " +
                "expression is not wired to the scheduler, which is the defect this test exists " +
                "for: the agent then checks nothing while its empty findings table reads as " +
                "'nothing to act on'.",
        )
    }
