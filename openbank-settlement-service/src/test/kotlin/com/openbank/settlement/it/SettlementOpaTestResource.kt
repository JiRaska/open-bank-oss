...
class SettlementOpaTestResource : QuarkusTestResourceLifecycleManager {
    ...
    override fun start(): Map<String, String> {
        ...
        val mounts = mapOf(
            ...
            "settlement_rest_ext.rego" to "settlement_rest_ext.rego",
    ...
    private fun imageFromDeployment(): String {
        val yaml = Path.of(System.getProperty("openbank.test.settlement-deployment")).readText()
        val rollout = Yaml().loadAll(yaml).filterIsInstance<Map<*, *>>().single {
            it["kind"] == "Rollout" &&
                it.child("metadata")["name"] == "settlement-service"
        }
