...
AUDIT_REST_EXT=$REPO/openbank-infra/gitops/components/audit/audit_rest_ext.rego
...
OUT=$REPO/openbank-infra/gitops/components/audit/audit-opa-bundle.yaml

{
  echo "# GENERATED by gen-audit-opa-bundle.sh — do not hand-edit."
  echo "# Source: rest.rego + audit_rest_ext.rego + agents.rego + agents-opa-data.yaml + rules-opa-data.yaml + bundle.manifest"
  echo "apiVersion: v1"
  echo "kind: ConfigMap"
  echo "metadata:"
  echo "  name: audit-opa-bundle"
  echo "  namespace: audit"
  echo "  labels:"
  echo "    app.kubernetes.io/name: audit-service"
  echo "    app.kubernetes.io/part-of: audit"
  echo "  annotations:"
  echo "    openbank.tech/policy-checksum: \"$CHECKSUM\""
  echo "data:"
  echo "  rest.rego: |"
  sed 's/^/    /' "$REST_REGO" | sed 's/[[:space:]]*$//'
  echo "  audit_rest_ext.rego: |"
  sed 's/^/    /' "$AUDIT_REST_EXT" | sed 's/[[:space:]]*$//'
  echo "  agents.rego: |"
...
DEPLOY=$REPO/openbank-infra/gitops/components/audit/audit-service.yaml
if [ -f "$DEPLOY" ]; then
