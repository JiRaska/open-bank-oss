...
STATEMENT_REST_EXT=$REPO/openbank-infra/gitops/components/statements/statement_rest_ext.rego
...
OUT=$REPO/openbank-infra/gitops/components/statements/statement-opa-bundle.yaml

{
  echo "# GENERATED by gen-statement-opa-bundle.sh — do not hand-edit."
  echo "# Source: rest.rego + statement_rest_ext.rego + agents.rego + agents-opa-data.yaml + rules-opa-data.yaml + bundle.manifest"
  echo "apiVersion: v1"
  echo "kind: ConfigMap"
  echo "metadata:"
  echo "  name: statement-opa-bundle"
  echo "  namespace: statements"
  echo "  labels:"
  echo "    app.kubernetes.io/name: statement-service"
  echo "    app.kubernetes.io/part-of: statements"
  echo "  annotations:"
  echo "    openbank.tech/policy-checksum: \"$CHECKSUM\""
  echo "data:"
  echo "  rest.rego: |"
  sed 's/^/    /' "$REST_REGO" | sed 's/[[:space:]]*$//'
  echo "  statement_rest_ext.rego: |"
  sed 's/^/    /' "$STATEMENT_REST_EXT" | sed 's/[[:space:]]*$//'
  echo "  agents.rego: |"
...
# (subPath mounts do NOT hot-reload — same pattern as gen-party-opa-bundle.sh).
ROLLOUT=$REPO/openbank-infra/gitops/components/statements/statement-service.yaml
if [ -f "$ROLLOUT" ]; then
