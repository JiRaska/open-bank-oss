...
KYC_REST_EXT=$REPO/openbank-infra/gitops/components/kyc/kyc_rest_ext.rego
...
OUT=$REPO/openbank-infra/gitops/components/kyc/kyc-opa-bundle.yaml

{
  echo "# GENERATED by gen-kyc-opa-bundle.sh — do not hand-edit."
  echo "# Source: rest.rego + kyc_rest_ext.rego + agents.rego + agents-opa-data.yaml + rules-opa-data.yaml + bundle.manifest"
  echo "apiVersion: v1"
  echo "kind: ConfigMap"
  echo "metadata:"
  echo "  name: kyc-opa-bundle"
  echo "  namespace: kyc"
  echo "  labels:"
  echo "    app.kubernetes.io/name: kyc-service"
  echo "    app.kubernetes.io/part-of: kyc"
  echo "  annotations:"
  echo "    openbank.tech/policy-checksum: \"$CHECKSUM\""
  echo "data:"
  echo "  rest.rego: |"
  sed 's/^/    /' "$REST_REGO" | sed 's/[[:space:]]*$//'
  echo "  kyc_rest_ext.rego: |"
  sed 's/^/    /' "$KYC_REST_EXT" | sed 's/[[:space:]]*$//'
  echo "  agents.rego: |"
...
DEPLOY=$REPO/openbank-infra/gitops/components/kyc/kyc-service.yaml
if [ -f "$DEPLOY" ]; then
