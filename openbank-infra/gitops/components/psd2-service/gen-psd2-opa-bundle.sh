...
PSD2_REST_EXT=$REPO/openbank-infra/gitops/components/psd2-service/psd2_rest_ext.rego
...
OUT=$REPO/openbank-infra/gitops/components/psd2-service/psd2-opa-bundle.yaml

{
  echo "# GENERATED by gen-psd2-opa-bundle.sh — do not hand-edit."
  echo "# Source: rest.rego + psd2_rest_ext.rego + agents.rego + agents-opa-data.yaml + rules-opa-data.yaml + bundle.manifest"
  echo "apiVersion: v1"
  echo "kind: ConfigMap"
  echo "metadata:"
  echo "  name: psd2-opa-bundle"
  echo "  namespace: psd2"
  echo "  labels:"
  echo "    app.kubernetes.io/name: psd2-service"
  echo "    app.kubernetes.io/part-of: psd2"
  echo "  annotations:"
  echo "    openbank.tech/policy-checksum: \"$CHECKSUM\""
  echo "data:"
  echo "  rest.rego: |"
  sed 's/^/    /' "$REST_REGO" | sed 's/[[:space:]]*$//'
  echo "  psd2_rest_ext.rego: |"
  sed 's/^/    /' "$PSD2_REST_EXT" | sed 's/[[:space:]]*$//'
  echo "  agents.rego: |"
...
ROLLOUT=$REPO/openbank-infra/gitops/components/psd2-service/psd2-service.yaml
if [ -f "$ROLLOUT" ]; then
