...
CAMPAIGN_REST_EXT=$REPO/openbank-infra/gitops/components/campaign/campaign_rest_ext.rego
...
OUT=$REPO/openbank-infra/gitops/components/campaign/campaign-opa-bundle.yaml

{
  echo "# GENERATED by gen-campaign-opa-bundle.sh — do not hand-edit."
  echo "# Source: rest.rego + campaign_rest_ext.rego + agents.rego + agents-opa-data.yaml + rules-opa-data.yaml + bundle.manifest"
  echo "apiVersion: v1"
  echo "kind: ConfigMap"
  echo "metadata:"
  echo "  name: campaign-opa-bundle"
  echo "  namespace: campaign"
  echo "  labels:"
  echo "    app.kubernetes.io/name: campaign-service"
  echo "    app.kubernetes.io/part-of: campaign"
  echo "  annotations:"
  echo "    openbank.tech/policy-checksum: \"$CHECKSUM\""
  echo "data:"
  echo "  rest.rego: |"
  sed 's/^/    /' "$REST_REGO" | sed 's/[[:space:]]*$//'
  echo "  campaign_rest_ext.rego: |"
  sed 's/^/    /' "$CAMPAIGN_REST_EXT" | sed 's/[[:space:]]*$//'
  echo "  agents.rego: |"
...
# (subPath mounts do NOT hot-reload — same pattern as every other gen-*-opa-bundle.sh).
ROLLOUT=$REPO/openbank-infra/gitops/components/campaign/campaign-service.yaml
if [ -f "$ROLLOUT" ]; then
