...
# Standalone ledger_rest_ext.rego (matches delegation/consent/kyc/psd2/aml/interest/balance/
# fraud), so `opa test` can load it directly rather than a heredoc trapped inside this
...

{
  ...
  echo "# Source: rest.rego + ledger_rest_ext.rego + agents.rego + agents-opa-data.yaml + rules-opa-data.yaml + bundle.manifest"
  echo "apiVersion: v1"
  echo "kind: ConfigMap"
  echo "metadata:"
  ...
  sed 's/^/    /' "$LEDGER_REST_EXT" | sed 's/[[:space:]]*$//'
  echo "  agents.rego: |"
