...
DISPUTE_REST_EXT=$REPO/openbank-infra/gitops/components/dispute-service/dispute_rest_ext.rego
...
OUT=$REPO/openbank-infra/gitops/components/dispute-service/dispute-opa-bundle.yaml

{
  echo "# GENERATED by gen-dispute-opa-bundle.sh — do not hand-edit."
  echo "# Source: rest.rego + dispute_rest_ext.rego + agents.rego + agents-opa-data.yaml + rules-opa-data.yaml + bundle.manifest"
  echo "apiVersion: v1"
  echo "kind: ConfigMap"
  echo "metadata:"
  echo "  name: dispute-opa-bundle"
  echo "  namespace: dispute"
  echo "  labels:"
  echo "    app.kubernetes.io/name: dispute-service"
  echo "    app.kubernetes.io/part-of: dispute"
  echo "  annotations:"
  echo "    openbank.tech/policy-checksum: \"$CHECKSUM\""
  echo "data:"
  echo "  rest.rego: |"
  sed 's/^/    /' "$REST_REGO" | sed 's/[[:space:]]*$//'
  echo "  dispute_rest_ext.rego: |"
  sed 's/^/    /' "$DISPUTE_REST_EXT" | sed 's/[[:space:]]*$//'
  echo "  agents.rego: |"
...
ROLLOUT=$REPO/openbank-infra/gitops/components/dispute-service/dispute-service.yaml
if [ -f "$ROLLOUT" ]; then
