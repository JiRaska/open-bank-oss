...
package com.openbank.kyc.infrastructure.persistence.repository

import com.openbank.kyc.application.port.out.KycOutboxRepository
import com.openbank.kyc.infrastructure.persistence.entity.KycOutboxEntity
...
@ApplicationScoped
class KycOutboxRepositoryImpl(private val clock: Clock) :
    KycOutboxRepository,
    PanacheRepository<KycOutboxEntity> {
    ...
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, KycOutboxEntity::class.java)
