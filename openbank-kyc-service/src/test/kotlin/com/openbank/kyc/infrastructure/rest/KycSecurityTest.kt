...
/**
 * Regression guard (C7/ADR-0030): no KYC endpoint must be opened without role-gating.
 * Pure reflection — no Quarkus boot needed.
 */
class KycSecurityTest {

    @Test
    fun `no KycResource endpoint is @PermitAll`() {
        val methods = KycResource::class.java.declaredMethods.filter { m ->
