 ...
 */
class KycOutboxBacklogGaugeTest {

    @Test
    fun `registers a kyc backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<KycOutboxRepository>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("kyc"), capture(supplier)) } returns Unit

        val gauge = KycOutboxBacklogGauge(repo, metrics)
        gauge.register()
