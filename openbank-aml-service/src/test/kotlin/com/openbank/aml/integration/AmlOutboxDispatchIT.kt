 ...
 * tests in [com.openbank.aml.infrastructure.outbox.AmlOutboxDispatchTest] cannot reach: a real
 * reactive Panache session drives [AmlOutboxDispatcher.dispatchForTest] against the dedicated
 * IT Postgres, while the Kafka leg is swapped to the in-memory connector so we can assert exactly
 * what was produced.
 *
 * What this proves that the unit tests can't:
 *  - N1: the whole coroutine dispatch chain runs on the event loop without the HR000068
 ...
 * and never races the assertions. [AmlOutboxDispatcher.dispatchForTest] bypasses the FT interceptors
 * on [AmlOutboxDispatcher.dispatch] to avoid a SmallRyeConfigProviderResolver ClassLoader clash that
 * occurs when @Timeout/@Retry run on a non-Quarkus thread in the test JVM.
 */
@QuarkusTest
@QuarkusTestResource(AmlOutboxDispatchIT.InMemoryKafkaResource::class)
@QuarkusTestResource(com.openbank.aml.it.PostgresRedisTestResource::class)
class AmlOutboxDispatchIT {
    ...
    private fun entityFor(eventId: UUID): AmlOutboxEntity? = onVertxContext {
        Panache.withSession { repository.find("eventId", eventId).firstResult() }.awaitSuspending()
    }
    ...
    fun `dispatch publishes pending rows with N2 key plus N3 headers and marks them SENT`() {
        ...
        val eventType = "aml.case.created.v1"
        ...
        // Drive the batch dispatch without FT interceptors (avoids SmallRyeConfigProviderResolver
        // ClassLoader issue when @Timeout/@Retry interceptors run on a non-Quarkus thread).
        onVertxContext { dispatcher.dispatchForTest() }

        val sink = connector.sink<String>("aml-events-out")
