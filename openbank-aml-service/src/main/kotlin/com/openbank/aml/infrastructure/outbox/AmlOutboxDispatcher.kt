...
import com.openbank.aml.infrastructure.kafka.KafkaAmlOutboxEventPublisher
...
/**
 * Drains the AML transactional outbox to Kafka (ADR-0050 / ADR-0049 D3).
 *
 * Extends [AbstractOutboxDispatcher] which owns the dispatch loop. This class only contributes
 * the service-level wiring plus the CDI-proxied annotations required for MicroProfile Fault
...
class AmlOutboxDispatcher(
    private val repo: AmlOutboxRepositoryImpl,
    private val publisher: KafkaAmlOutboxEventPublisher,
    ...
    @Scheduled(
        ...
        identity = "aml-outbox-dispatcher",
    )
    @Bulkhead(1)
    ...
    /** Bypasses [dispatch] FT interceptors for integration tests — same batch logic, no FT overhead. */
