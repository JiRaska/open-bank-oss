...
- **2026-09-17** — **New automatic opening path: business current account at digital onboarding**,
  no new route, caller, edge or privilege. `PartyEventConsumer` now opens one `PENDING_ACTIVATION`
  CURRENT account on the Business Current Account product (EUR) for a `PARTY_CREATED` whose
  `partyType` is COMPANY or SOLE_TRADER. **Who can trigger it:** only a party-service
  `PARTY_CREATED` on the existing `party-events-in` channel; nothing reachable over HTTP. It is
  behind `openbank.account.onboarding.open-business-accounts`, **default false** — production keeps
  operator-opened business accounts; only the sandbox manifest turns it on. The account is inert
  until the existing party-ACTIVE reconcile (KYC+AML two-key gate) activates it, exactly like
