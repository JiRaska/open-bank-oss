...
| T28 | A concurrent retry substitutes a weaker representation rule or an altered offer under the same request key; a later writer rewrites a co-signature | The expand-only V28 ledger serializes proposals by unique `(principal_party_id, request_key)`. The repository returns the original operation only when immutable payload, rule snapshot, policy identity/revision, source case and initiator match; otherwise it refuses the replay. Database triggers reject evidence rewrites and decision updates/deletes; unique operation/actor and SCA-session constraints prevent duplicate signers and session reuse. V29 permits a null SCA session only for REJECT, while requiring one for APPROVE. Real-Postgres tests prove exact decision replay and no grant. **Not yet an issuance control:** quorum execution and the grant/outbox transaction do not exist; existing JOINT offers remain denied. Deploy V25–V28 before V29; rollback the application while retaining evidence tables, never a destructive down-migration. |
...
| T30 | A customer forges a company signer, enumerates pending offers, or changes the draft after one signature | The edge derives the human actor from the customer token and the selected company through the live acting-for check; proposal and decision APIs accept only the edge M2M identity, scope operations to the authenticated company, and recheck the current full JOINT roster. The client cannot supply an actor header directly or turn generic SCA into a decision. Canonical sorted payload and rule snapshots are immutable; exact-key proposal replays require identical actor, payload and rule. A changed live rule or expired PENDING offer is not signable. **Residual:** decisions are evidence, not grant authority; quorum execution and grant/outbox issuance remain absent. Rollback APIs and OPA actions together, retaining V28–V29 evidence rows for audit. |
| T31 | An SCA session for a frozen proposal is reused for the opposite business verdict, or SCA is consumed but the decision row is lost | `requestHash` covers the grant draft, not a verdict. The approval-intent hash is versioned and includes operation id, company, payload hash, rule hash, human actor and APPROVE; the decision writer compares it with SCA's write-once binding before consuming. Device `DENIED` makes a challenge FAILED, so an authenticated business REJECT is immutable evidence with no SCA session (V29); it cannot be misreported as a consumed approval. An external SCA consume and local SQL insert are not atomic: on timeout, the writer reads SCA again and accepts recovery only for the same party, statutory purpose, exact operation/binding, COMPLETED state and non-null `consumedAt`. An old SCA provider lacking these fields fails closed. The SQL insert checks PENDING and expiry again and exact actor/session retries return the original row. **Residual:** the decision API still issues no grant; quorum and outbox execution must re-read the live rule and decisions atomically. |
