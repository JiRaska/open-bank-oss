...
| T24 | A caller forges an entity profile, reuses a revoked mandate, or treats a JOINT representative as a sole signer | The edge derives the human actor from the authenticated token; it is not accepted from the app. delegation-service re-checks the selected principal and the actor's currently active mandate at issuance and acceptance time against party-service, before consuming SCA. An individual may issue or accept a grant for an entity only when the mandate explicitly says `SOLE` with `requiredSignatures=1`; JOINT, missing/unknown or inconsistent quorum metadata is denied rather than interpreted as unilateral authority. The challenge belongs to that human, while the grant belongs to the selected entity. Unknown, inactive, malformed and unavailable results fail closed. The east-west call uses party-service's private-CA mTLS listener on 8443 with hostname verification and TLS 1.3; OIDC and namespace/port NetworkPolicy remain independent controls. Residual: a JOINT representative needs a separate co-signed flow before they can issue or accept a grant; this check does not establish an employee's delegated `delegation.manage` capability. |
...
| T25 | A compromised client changes an approval-group member or threshold after the owner completes SCA, or manages an organization's roster through a stale/forged profile | The edge first requests a versioned, server-canonical SHA-256 reference over operation, owner, group id/revision, trimmed name, sorted unique members and threshold. The approving device signs that reference in `DynamicLinkingData`; sca-service compares it exactly in the atomic consume gate for create and revise. The edge forwards the authenticated human separately from the selected owner, and delegation-service revalidates that actor's current organization mandate before every create, revise or deactivate. Create and revise consume the human actor's SCA—not the entity's. Deactivation is deliberately immediate without SCA so an authorized owner can stop future group use during an incident; it still rechecks current authority, increments the revision, and publishes the inactive roster, while existing operation snapshots remain unchanged. Changed create/revise fields, revoked authority or actor mismatch refuse before persistence. Group revisions are monotonic and every full roster is stored and published transactionally. Cross-tenant reads collapse to 404. Residual: groups are configuration only; non-SOLO execution remains refused until resource-policy binding, immutable per-operation snapshots and an atomic distinct-actor decision ledger land. |
...
- **LEGAL_ENTITY grantors are bound to a human actor** (ADR-0232 D5 / ADR-0284): customer-edge
  ...
  and requires that human to appear in its active `acting-for` mandate set with explicit `SOLE`
  authority and one required signature. A JOINT mandate is visible in the profile switcher but
  cannot be used by one human to issue a delegation. The same human owns
  ...
  sub-administration and co-signed JOINT issuance remain separate workflows. Rollback must keep
