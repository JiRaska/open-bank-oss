...
| T24 | A caller forges an entity profile, reuses a revoked mandate, or intercepts the authority lookup | The edge derives the human actor from the authenticated token; it is not accepted from the app. delegation-service re-checks the selected principal and the actor's currently active mandate at issuance time against party-service, before consuming SCA. It admits an independent delegation only for an explicit `SOLE` mandate with `requiredSignatures=1`; one JOINT signatory and a response missing authority metadata are denied. Unknown, inactive, malformed and unavailable results fail closed. The east-west call uses party-service's parallel private-CA mTLS listener on 8443: hostname verification authenticates the server, a namespace-local cert identifies delegation-service, and TLS 1.3 is pinned; OIDC and namespace/port NetworkPolicy remain independent controls. An event-only projection was rejected for this admission decision because bootstrap/replay lag and a revocation race would trade authorization freshness for availability. Residual: this establishes statutory/owner representation, not a delegated employee capability such as `delegation.manage`; JOINT issuance requires a separately proven multi-actor operation ledger and must remain unavailable until then. |
...
| T28 | A concurrent retry substitutes a weaker representation rule or an altered offer under the same request key; a later writer rewrites a co-signature | The expand-only V28 ledger serializes proposals by unique `(principal_party_id, request_key)`. The repository returns the original operation only when the immutable payload, rule snapshot, policy identity/revision, source case and initiator match exactly; otherwise it refuses the replay. Database triggers reject evidence rewrites and decision updates/deletes; unique operation/actor and SCA-session constraints prevent duplicate signers and session reuse. Real-Postgres tests cover these invariants and prove proposal insertion creates no grant. **Not yet an issuance control:** there is no proposal or decision API consuming the attestation-fresh roster, SCA-linked decision writer, quorum execution, or grant/outbox transaction in this slice. Existing JOINT offers remain denied. Deploy V25–V27 before V28; rollback the application and retain the evidence tables, not a destructive down-migration. |
| T29 | One JOINT mandate or a stale statutory roster is mistaken for authority to open an issuance operation | `StatutoryRuleClient` reads the signed Party policy and the entity's full mandate list, verifies an ACTIVE COMPANY, a JOINT quorum and office-capable roster, the requesting human's membership, and a live same-KYB-case JOINT mandate with the exact quorum for every listed representative. It then calls KYB's live-registry attestation-status endpoint and requires the exact attestation ID, current flag, rule-text hash, signer count and office list to match Party's snapshot. Missing metadata, a revoked/expired mandate, an amended rule, a mismatched case/count/office or either upstream failure all fail closed. `RosterMatched` permits only an inert proposal after ordinary draft/ownership checks; it is **not grant authority** and must be rechecked at quorum execution to close the decision-to-commit race. KYB provider + OPA + ingress policy deploy before this reader; rollback the reader with the provider or it denies all new JOINT proposals. |
| T30 | A customer forges a company signer, enumerates pending offers, or changes the draft after one signature | The edge derives the human actor from the customer token and the selected company through the live acting-for check; the proposal API accepts only the edge M2M identity, scopes operations to the authenticated company, and rechecks the current full JOINT roster before revealing an offer. The client cannot supply an actor header directly to this path or turn a generic SCA session into a decision. Canonical sorted payload and rule snapshots are hashed, immutable in V28, and exact-key replays return the original only for identical actor, payload and rule. A changed live rule or expired PENDING offer is unreadable for signing and returns a conflict. **Residual:** no co-signer decision, quorum execution, grant or outbox issuance is enabled; PENDING must not be interpreted as usable access by any consumer. Rollback the API and OPA action together, retaining additive V28 evidence rows for audit. |
| T31 | An SCA session for a frozen proposal is reused for the opposite business verdict, or SCA is consumed but the decision row is lost | The existing `requestHash` covers the proposed grant, not the signer's APPROVE/REJECT verdict; `DELEGATION_STATUTORY_APPROVAL` binds only operation id/hash. Therefore a decision writer must derive a versioned fingerprint over operation id, payload hash, rule hash, actor and explicit verdict, and compare that fingerprint when spending SCA. Device `DENIED` makes the challenge FAILED rather than COMPLETED, so a business REJECT cannot be represented as a consumed successful approval session; it needs its own authenticated, immutable rejection evidence and a compatible nullable-session migration. An external SCA consume and local SQL insert are not atomic: on an ambiguous retry, recovery may accept a previously consumed challenge only after SCA attests the same party, purpose, exact decision binding and consumed state. SCA's read response now exposes the write-once binding and `consumedAt` for that future reconciliation; older provider versions return null and must fail closed. No approval/decision API or quorum execution is exposed yet; PENDING remains inert. |
