...
- **2026-09-21** — **Own machine identity for the settlement journal and balance legs (#10486 batch 2).** `LedgerRestClient` and `BalanceRestClient` now mint their bearer from the NAMED oidc-client `m2m`, Keycloak client `openbank-settlement` (`ROLE_API` only). ledger-service grants it `ledger.create` plus `ledger.read` (`service-settlement-ledger-post` — the read is `LedgerJournalLookupAdapter`'s already-posted check on the same interface) and never `ledger.reverse`; balance-service grants it `balance.debit` + `balance.credit` (`service-settlement-balance-move`) and never a hold. **STRIDE-S:** a new credential. Its secret is generated by Keycloak in the live realm, stored by the owner's provisioning script at Vault KV `keycloak/settlement-service` (`client_secret`), projected by the `settlement-service-m2m-oidc` ExternalSecret and never seen by the repo; the env ref is `optional: false`, so an unseeded entry blocks the new pod loudly (CreateContainerConfigError) rather than calling with an empty credential. Compromise of this secret reaches only `ledger.create`, `ledger.read`, `balance.debit` and `balance.credit`, against the shared secret's 54 money-path writes. **Repudiation improves:** the upstream's OPA decision reason and principal now name this service instead of "some caller on the shared client". The service's other rest-clients stay on the shared `openbank-services` client until their own edges migrate (asserted by `M2mOidcClientIdentityWiringTest`). Rollback: revert the commit (the clients return to the shared token).
