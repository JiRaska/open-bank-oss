...
- **2026-09-26** — Pillar 1 credit-risk capital read (ADR-0313 phase 2, #10618): `GET /api/v1/risk/snapshots/{id}/capital` on the existing read action `risk.snapshot.read` (RISK / FINANCE / ADMIN, no role widening). Derived on request from a TIED_OUT run, never stored; UNTIED is 409. No caller input beyond the run id. New configuration only (`openbank.risk.capital.sa.*`, a versioned BCBS d424 parameter set bound by a config mapping, validated at startup: missing / unknown / out-of-range risk weights or minima fail the boot rather than defaulting). A mis-mapped GL account could UNDERSTATE RWA (and so overstate the ratios), so only explicitly mapped accounts are weighted, every other asset balance is returned as unclassified, the defaults are the conservative readings (SCRA Grade C, other retail, stage 3 at 150%), and the response states that ratios on credit-risk RWA alone are an upper bound. Own funds are read only from the explicitly mapped 6000-6060 accounts and never supplied or guessed. No new edge, data store or write path. Rollback: revert.
- **2026-09-25** — IRRBB read (ADR-0313 phase 1, #10618): `GET /api/v1/risk/snapshots/{id}/irrbb?curveSetId=&tier1Capital=` on the existing read action `risk.snapshot.read` (RISK / FINANCE / ADMIN, no method-level role widening). Derived on request from a TIED_OUT run and a curve set, never stored; an UNTIED run is 409 like every other derived read. `tier1Capital` is caller-supplied, validated as a positive decimal (400 otherwise) and used only to compute the displayed outlier ratio; it is not persisted or logged. New configuration only (`openbank.risk.irrbb.*`: shock sizes, shock source, optional post-shock floor) — a missing currency is reported "not configured", never defaulted. No new edge, data store or write path. Rollback: revert.
- **2026-09-25** — LCR / NSFR read (ADR-0313 phase 1, #10618): `GET /api/v1/risk/snapshots/{id}/liquidity` on the existing read action `risk.snapshot.read` (RISK / FINANCE / ADMIN, no role widening). Derived on request from a TIED_OUT run, never stored; UNTIED is 409. No caller input beyond the run id. New configuration only (`openbank.risk.liquidity.*`, a versioned parameter set bound by a config mapping, validated at startup: missing / unknown / out-of-range factors fail the boot rather than defaulting). A mis-mapped GL account could overstate HQLA, so only explicitly mapped accounts count and every other balance is returned as unclassified. No new edge, data store or write path. Rollback: revert.
