...
| **Tampering** (replay-key reuse, #10916) | Mitigated | The `Idempotency-Key` / `X-Request-ID` on both application-submit endpoints is bound to a request fingerprint (method + path + canonical body); the same key with a different body is refused 422 `IDEMPOTENCY_KEY_REUSED` instead of replaying the first application. |
...
- **2026-09-26** — **Replay key bound to a request fingerprint on `POST /api/v1/lending/applications` and `POST /api/v1/lending/intake/applications` (#10945).** **Tampering / repudiation:** previously the same `Idempotency-Key` with a DIFFERENT body replayed the first request's response, so a second, different loan application was answered as the first and silently never happened. Now the key is bound to a SHA-256 fingerprint of method + path + the canonicalised request DTO (#10916, libs #10922) and a mismatch is refused **422 `IDEMPOTENCY_KEY_REUSED`** before any use case runs. Records stored before this deploy carry no fingerprint and are treated as a match for one TTL window. No new endpoint, caller, privilege, event or migration. Rollback: revert. Intake fingerprints the customer's `CustomerIntakeRequest`, not the clock-derived application built from it.
