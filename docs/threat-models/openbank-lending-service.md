...
| **Tampering** (replay-key reuse, #10916) | Mitigated | The `Idempotency-Key` / `X-Request-ID` on both application-submit endpoints is bound to a request fingerprint (method + path + canonical body); the key is claimed atomically in Redis (`reserve`) before the use case runs; the same key with a different body is refused 409 `IDEMPOTENCY_KEY_REUSED`, a concurrent duplicate 409 `IDEMPOTENCY_REQUEST_IN_PROGRESS`, and a refused/failed submit releases the claim. Redis-only: no DB-level check, and pre-fingerprint records replay unchecked for one 300 s TTL. |
...
- **2026-09-26** — **Replay key bound to a request fingerprint on `POST /api/v1/lending/applications` and `POST /api/v1/lending/intake/applications` (#10945).** **Tampering / repudiation:** previously the same `Idempotency-Key` with a DIFFERENT body replayed the first request's response, so a second, different loan application was answered as the first and silently never happened. Now the key is claimed atomically (`IdempotencyStore.reserve`) together with a SHA-256 fingerprint of method + path + the canonicalised request DTO (libs `RequestFingerprints`, #10916/#10922) before any use case runs: a mismatch is refused **409 `IDEMPOTENCY_KEY_REUSED`**, a concurrent duplicate **409 `IDEMPOTENCY_REQUEST_IN_PROGRESS`**, and a refused or failed submit releases the claim. The binding lives only in Redis (no DB-level check). Records stored before this deploy carry no fingerprint and are treated as a match for one TTL window. No new endpoint, caller, privilege, event or migration. Rollback: revert. Intake fingerprints the customer's `CustomerIntakeRequest`, not the clock-derived application built from it.
