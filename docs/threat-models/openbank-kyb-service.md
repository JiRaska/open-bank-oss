...
- KYB receives register data and document-service's ceremony result, calls party-service to grant
  mandates, then writes its case transition and transactional outbox event. Party-service projects
  the signed statutory rule from that event.
...
| One of several mandate grants fails after earlier grants succeeded | None yet: the existing REST loop is not atomic with the KYB case/outbox transaction. | **Hard rollout gate:** move multi-signer mandate creation and rule projection into one party-service transaction with idempotent replay, or implement an equivalent compensating protocol and prove it under partial failure. A selected business profile is not JOINT authorization. |
| An agreement event is duplicated or delivered late | KYB writes through its outbox; party-service identifies a policy by source case, compares repeated evidence and uses effective time for latest-evidence reads. | Do not authorize from the event alone: verify current attestation and live mandates before opening an operation. |
