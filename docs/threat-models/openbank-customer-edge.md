...
- **2026-09-21** — Standing orders and SDD mandates follow the business signing policy (#10281,
  ...
  under `X-Acting-For` now go through `holdForApproval` like the four payment rails: a policy
  needing more than one signature consumes the initiator's SCA (optional `X-SCA-Challenge-Id`,
  required only then), holds the exact upstream body and answers 202; a policy that cannot be
  evaluated answers 503 rather than creating the instruction on one signature. The release targets
...
- **2026-09-19** — **Business multi-signature payments and signing routes (#10281, E-8).** New
  ...
  trusted payees list, add and remove, and approvals list, detail, sign and reject. There is also
  `/me/approvals/pending`, which is always the human's. Payment POSTs can answer 202 PENDING_APPROVAL.
  A new edge→delegation-service call path uses the existing declared edge, so the network policy
  does not change. A new edge→rail call happens on release, with the same rails, party header and
  service token as before. The hold ships disabled
  (`BUSINESS_APPROVALS_ENFORCE=false`) until delegation-service's internal signing API is deployed.
  The APPROVAL dynamic-linking bytes are sca-service's:
  `challengeId|decision|amount|currency|creditor|reference|approvalRequestId|payloadSha256`.
- **2026-09-18** — **SCA under `X-Acting-For` is the human's; company profile route.** Before this,
  the profile-switch header re-scoped the SCA routes to the ENTITY: challenges were raised for the
  company, `/sca/pending` showed every mandate holder the approvals raised by every other one, a
  device could be enrolled to the company, and a decision was forwarded and audited as the company,
  so sca-service's device-party binding accepted any co-representative's company-enrolled device and
  the audit row could not say who approved. All SCA routes and both `consume` call sites now bind to
  the human (E-7). New read-only `GET /customer/v1/business/company` requires `X-Acting-For` with an
  active mandate (403 otherwise, and 403 without the header) and reads party-service, kyb-service's
  cached register extract and account-service as that entity; it writes nothing. Rollback: revert the
  `customer.human` call sites — SCA returns to entity binding.
