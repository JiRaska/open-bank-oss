...
| **T/E** | A joint statutory signer approves a different delegation proposal, or another customer enumerates pending corporate approvals | `DELEGATION_STATUTORY_APPROVAL` requires a canonical operation UUID and exact SHA-256 payload hash before issuance. Both values are appended to the enrolled-device signed bytes, persisted with the challenge and compared at atomic consume; old payment/document/card signature bytes stay unchanged. The pending list repeats the authenticated customer-party check inside sca-service even when OPA is advisory. Delegation-service must independently verify the purpose, display the immutable proposal, revalidate the live roster and atomically issue only after distinct signed decisions reach quorum. |
| **T/E** | A signature for issuing a grant is presented as consent for a JOINT company to accept one | `DELEGATION_STATUTORY_ACCEPTANCE` is a distinct closed SCA purpose with different customer wording. It requires the same complete, unmixed operation UUID plus SHA-256 binding, and the purpose participates in the challenge idempotency key. A consumer must compare the stored purpose and exact binding before single-use consume; the purpose alone never activates a grant. Deploy this expanded SCA vocabulary before the separate acceptance ledger and executor. Rollback may disable acceptance initiation but must not reinterpret old approval challenges as acceptance. |
| **T**ampering | A legacy string-encoded dynamic link is interpreted differently from the object form | The v1 request accepts a string only when it contains a JSON object. Both forms deserialize into the same `DynamicLinkingData` model before existing purpose-specific checks, persistence and device-signature binding; malformed JSON, arrays and scalars fail at ingress. The object form remains the preferred wire shape. Deploy the parser and additive `oneOf` contract together; rollback restores the earlier object-only runtime without changing stored challenge evidence. |
