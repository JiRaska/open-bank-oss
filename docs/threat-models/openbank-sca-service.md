...
| **T/E** | A joint statutory signer approves a different delegation proposal, or another customer enumerates pending corporate approvals | `DELEGATION_STATUTORY_APPROVAL` requires a canonical operation UUID and exact SHA-256 payload hash before issuance. Both values are appended to the enrolled-device signed bytes only for this purpose, persisted with the challenge and compared at atomic consume; old payment/document/card signature bytes stay unchanged. The pending list repeats the authenticated customer-party check inside sca-service even when OPA is advisory. Residual: the client must fetch and display the immutable proposal details before signing, and delegation-service must revalidate the roster and atomically issue a grant only after distinct signed decisions reach quorum. Until that workflow exists, JOINT issuance remains denied. |
