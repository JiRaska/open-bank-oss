...
## 10. Stav realizace (2026-09-04)

### C — Bezpečnost — HOTOVO
- **#8359 four-eyes sdd+interest** — MERGED PR [#8611](https://github.com/JiRaska/open-bank-oss/pull/8611).
- **#8360 SCA four-eyes stalemate** — MERGED PR [#8626](https://github.com/JiRaska/open-bank-oss/pull/8626)
  (ADR-0280: caller-aware four-eyes exemptions; `device.enroll`/`scaChallenge.consume` nově gated
  pro lidské cestičky, ověřená SCA automatika teče dál).

### D — Produktová zralost — HOTOVO (1× blocked, 1× čeká na lidský trial)
- **#8361 net-settlement ledger leg** — MERGED PR [#8723](https://github.com/JiRaska/open-bank-oss/pull/8723)
  (ADR-0281: `settleBatch` commitne v jedné transakci batch SETTLED + items + `batch.settled` +
  outbox komand `openbank.clearing.net_settlement.post`; self-consumer postne balanced journal do
  ledger-service s idempotency klíčem `clearing-net-settlement-{batchId}`; DLQ komplet v téže změně;
  atomicita dokázána `ClearingSettleOutboxAtomicityIT` na reálné DB; ledger V26 seed účty 1110–1113).
- **#8362 EU FSF adapter** — MERGED PR [#8689](https://github.com/JiRaska/open-bank-oss/pull/8689)
  (`ListImportOutcome` + metrika `openbank.sanctions.list.imports`).
- **#8364 PD/LGD kalibrace** — MERGED PR [#8681](https://github.com/JiRaska/open-bank-oss/pull/8681)
  (IFRS 9 risk parametry verzované end-to-end včetně kalibračního replaye).
- **#8365 onboarding path** — MERGED PR [#8632](https://github.com/JiRaska/open-bank-oss/pull/8632);
  issue zůstává OPEN do dokončení lidského trialu.
- **#8366 deployment reference** — MERGED PR [#8628](https://github.com/JiRaska/open-bank-oss/pull/8628).
- **#8363 customer app GA** — OPEN, blocked na přístup k repu `openbank-app` (KMP). Dossier
  `/docs/customer-app` derivuje fakta z app kódu, takže po merge v app repu se přepne derivací
  — rozbor a unblock postup v komentáři issue.

### E — Provoz & infra — HOTOVO (46, 47)
- E46 (temporal worker switch naming) — MERGED PR [#8374](https://github.com/JiRaska/open-bank-oss/pull/8374).
- E47 (deploy-drift badge + e2e release-exclusion) — MERGED PR [#8369](https://github.com/JiRaska/open-bank-oss/pull/8369).

### F — Hygiena & tech dluh
- **F48 / #8370 (11 auto.offset.reset baseline položek)** — OPEN, owner decisions. Per-entry
  klasifikace všech 11 consumerů z kódu + doporučení (prakticky všude platné option 2, enforce
  `earliest` přes msg-override ConfigMap; replay riziko se týká jen chybějících offsetů) je
  decision-ready v komentáři issue. Multi-service scope → po schválení per-service PR.
- **F49 (gotchas→gates parity)** — MERGED PR [#8725](https://github.com/JiRaska/open-bank-oss/pull/8725):
  nový enforced gate `jaxrs-path-not-hijacked-by-toplevel-fun` (`check-jaxrs-path-hijack.py`,
  self-test oběma směry, 2187 Kotlin main sources čistých, budget_seconds + min_subjects +
  rationale + review_after dle lifecycle meta-gate). Gotcha z #3371 (@Path ukradený top-level
  funkcí) tak přestala být jen prozaickou poučkou.

### Poznatky z realizace (2026-09-04)
- **Race push↔PR-body-edit:** `security-checklist-money-path` gate čte PR body ze synchronize
  event payloadu — edit body pár sekund PO pushi se do běhu nedostane. Zafixováno prázdným
  retrigger commitem; poučka: nejdřív body, pak push (nebo počkat na re-run).
- **Partial `git add` je tichá chyba:** threat-model changelog zůstal ve working tree, protože
  `git add` vyjmenoval jen code paths; gate `threat-model-updated-on-trust-boundary-change` to
  v CI správně chytil. Poučka: stage-ovat explicitně i docs artefakty, které gates vyžadují
  ve stejném diffu (ADR-0030 D2).
- Strojová zátěž (load ~165 z paralelních buildů) prodloužila lokální Gradle běhy nad 5 min —
  `--configure-on-demand --offline` + per-suite `--tests` filtry drží iteraci použitelnou.
