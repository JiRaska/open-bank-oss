...
# Runbook — openbank-aml-service

> Operational runbook for the `aml` service. Data domain **compliance**,
...
triaging an incident that starts on `aml`.
...
> (`openbank-libs/governance/attestations.yaml: aml.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `aml.oncall` attestation — until that is live, escalate via the team channel).
