...
# Runbook — openbank-notification-service

> Operational runbook for the `notification` service. Data domain **platform**,
...
- **Downstream (depends on this service):** `consent-service`

A failure here propagates to the downstream services above — check them when
triaging an incident that starts on `notification`.
...
> (`openbank-libs/governance/attestations.yaml: notification.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `notification.oncall` attestation — until that is live, escalate via the team channel).
