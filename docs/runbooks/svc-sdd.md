...
# Runbook — openbank-sdd-service

> Operational runbook for the `sdd` service. Data domain **payments**,
...
triaging an incident that starts on `sdd`.
...
- **Scale:** `kubectl scale deploy/sdd-service -n sdd --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: sdd.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `sdd.oncall` attestation — until that is live, escalate via the team channel).
