...
# Runbook — openbank-context-service
...
| Field | Value |
|---|---|
| Service | `openbank-context-service` |
| HTTP port | `8149` |
| Data domain | compliance |
| Datastore | PostgreSQL (database `openbank_context`) |
| Classification | restricted |
| Retention | source-aligned; read audit 7 years |
| Lineage role | both |
...
- Metrics: scraped by the fleet PodMonitor (namespace `context`); dashboards in Grafana.
...
> (`openbank-libs/governance/attestations.yaml: context.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `context.oncall` attestation — until that is live, escalate via the team channel).
