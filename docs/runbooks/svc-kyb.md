...
# Runbook — openbank-kyb-service

> Operational runbook for the `kyb` service. Data domain **identity**,
...
| Field | Value |
|---|---|
| Service | `openbank-kyb-service` |
| HTTP port | `8157` |
| Data domain | identity |
| Datastore | PostgreSQL (database `openbank_kyb`) |
...
- **Upstream (this service consumes):** `party-service`
...
triaging an incident that starts on `kyb`.

## Health & probes

- Readiness: `GET :8090/q/health/ready` · Liveness: `GET :8090/q/health/live`
- Metrics: scraped by the fleet PodMonitor (namespace `kyb`); dashboards in Grafana.
- Logs: `kubectl logs -n kyb deploy/kyb-service -f`, or Loki
  `{namespace="kyb"}`.

## Routine operations

- **Restart:** `kubectl rollout restart deploy/kyb-service -n kyb` (rolling, zero-downtime at >1 replica).
- **Scale:** `kubectl scale deploy/kyb-service -n kyb --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: kyb.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `kyb.oncall` attestation — until that is live, escalate via the team channel).
