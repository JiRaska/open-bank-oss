...
# Runbook — openbank-card-processing-service

> Operational runbook for the `card-processing` service. Data domain **payments**,
...
| Field | Value |
|---|---|
| Service | `openbank-card-processing-service` |
| HTTP port | `8157` |
| Data domain | payments |
| Datastore | PostgreSQL (database `openbank_card_processing`) |
...
triaging an incident that starts on `card-processing`.
...
- Logs: `kubectl logs -n payments deploy/card-processing-service -f`, or Loki
  `{namespace="payments"}`.

## Routine operations

- **Restart:** `kubectl rollout restart deploy/card-processing-service -n payments` (rolling, zero-downtime at >1 replica).
- **Scale:** `kubectl scale deploy/card-processing-service -n payments --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: card-processing.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `card-processing.oncall` attestation — until that is live, escalate via the team channel).
