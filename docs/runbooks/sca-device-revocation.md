...
money-path four-eyes requirement applies when authorization is enforced. Obtain the
required approval through the established flow; do not disable policy enforcement.
...
HTTP tests use advisory authorization; the OPA suite independently checks owner scope
and four-eyes. A live approval-flow drill remains necessary.
