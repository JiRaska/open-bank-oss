...
# Runbook — openbank-transaction-service

> Operational runbook for the `transaction` service. Data domain **core**,
...
| HTTP port | `8102` |
| Data domain | core |
| Datastore | PostgreSQL (database `openbank_transactions`) |
...
- Readiness: `TCP :8102` · Liveness: `GET :8085/q/health/live`
...
- **Scale:** `kubectl scale rollout/transaction-service -n payments --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: transaction.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `transaction.oncall` attestation — until that is live, escalate via the team channel).
