...
# Runbook — openbank-statement-service

> Operational runbook for the `statement` service. Data domain **compliance**,
...
| Datastore | PostgreSQL (database `openbank_statement`) |
...
triaging an incident that starts on `statement`.
...
> (`openbank-libs/governance/attestations.yaml: statement.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `statement.oncall` attestation — until that is live, escalate via the team channel).
