...
# Runbook — openbank-loyalty-service

> Operational runbook for the `loyalty` service. Data domain **open-banking**,
...
| Field | Value |
|---|---|
| Service | `openbank-loyalty-service` |
| HTTP port | `8157` |
| Data domain | open-banking |
| Datastore | PostgreSQL (database `openbank_loyalty`) |
...
triaging an incident that starts on `loyalty`.
...
- **Restart:** `kubectl rollout restart deploy/loyalty-service -n loyalty` (rolling, zero-downtime at >1 replica).
- **Scale:** `kubectl scale deploy/loyalty-service -n loyalty --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: loyalty.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `loyalty.oncall` attestation — until that is live, escalate via the team channel).
