...
# Runbook — openbank-swift-service

> Operational runbook for the `swift` service. Data domain **payments**,
...
- Readiness: `TCP :8122` · Liveness: `GET :8085/q/health/live`
...
- **Scale:** `kubectl scale rollout/swift-service -n payments --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: swift.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `swift.oncall` attestation — until that is live, escalate via the team channel).
