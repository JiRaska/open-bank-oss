...
# Runbook — openbank-kyc-service

> Operational runbook for the `kyc` service. Data domain **compliance**,
...
- Readiness: `TCP :8114` · Liveness: `GET :8085/q/health/live`
...
- **Scale:** `kubectl scale rollout/kyc-service -n kyc --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: kyc.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `kyc.oncall` attestation — until that is live, escalate via the team channel).
