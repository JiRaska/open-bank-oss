...
- **Upstream (this service consumes):** `account-service`, `transaction-service`, `party-service`, `sepa-payment`
...
triaging an incident that starts on `audit`.
...
- **Scale:** `kubectl scale deploy/audit-service -n audit --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: audit.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `audit.oncall` attestation — until that is live, escalate via the team channel).
