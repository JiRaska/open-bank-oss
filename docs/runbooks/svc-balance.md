...
| HTTP port | `8103` |
| Data domain | core |
| Datastore | PostgreSQL (database `openbank_balances`) |
...
- **Scale:** `kubectl scale rollout/balance-service -n balances --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth; a later ArgoCD sync reconciles manual changes).
