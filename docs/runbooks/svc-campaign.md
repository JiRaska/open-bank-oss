...
triaging an incident that starts on `campaign`.
...
> (`openbank-libs/governance/attestations.yaml: campaign.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `campaign.oncall` attestation — until that is live, escalate via the team channel).
