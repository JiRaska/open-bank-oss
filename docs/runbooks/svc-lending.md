...
# Runbook — openbank-lending-service

> Operational runbook for the `lending` service. Data domain **payments**,
...
triaging an incident that starts on `lending`.
...
> (`openbank-libs/governance/attestations.yaml: lending.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `lending.oncall` attestation — until that is live, escalate via the team channel).
