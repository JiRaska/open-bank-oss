...
| Datastore | PostgreSQL (database `openbank_fx`) |
...
- **Scale:** `kubectl scale rollout/fx-service -n fx --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
