...
# Runbook — openbank-agent-service

> Operational runbook for the `agent` service. Data domain **platform**,
...
| Field | Value |
|---|---|
| Service | `openbank-agent-service` |
| HTTP port | `8109` |
| Data domain | platform |
| Datastore | PostgreSQL (database `openbank_agent`) |
...
- **Downstream (depends on this service):** `account-service`, `transaction-service`, `balance-service`

A failure here propagates to the downstream services above — check them when
triaging an incident that starts on `agent`.

## Health & probes

- Readiness: `TCP :8109` · Liveness: `GET :8085/q/health/live`
...
> (`openbank-libs/governance/attestations.yaml: agent.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `agent.oncall` attestation — until that is live, escalate via the team channel).
