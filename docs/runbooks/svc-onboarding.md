...
# Runbook — openbank-onboarding-service

> Operational runbook for the `onboarding` service. Data domain **compliance**,
...
- **Scale:** `kubectl scale rollout/onboarding-service -n onboarding --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth; a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: onboarding.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `onboarding.oncall` attestation — until that is live, escalate via the team channel).
