...
# Runbook — openbank-party-service

> Operational runbook for the `party` service. Data domain **identity**,
...
- **Downstream (depends on this service):** `account-service`, `kyc-service`, `audit-service`, `account-service`
...
- **Scale:** `kubectl scale rollout/party-service -n party --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth; a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: party.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `party.oncall` attestation — until that is live, escalate via the team channel).
