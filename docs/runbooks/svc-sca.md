...
# Runbook — openbank-sca-service

> Operational runbook for the `sca` service. Data domain **identity**,
...
| Field | Value |
|---|---|
| Service | `openbank-sca-service` |
| HTTP port | `8110` |
| Data domain | identity |
| Datastore | PostgreSQL (database `openbank_sca`) |
...
triaging an incident that starts on `sca`.

## Health & probes

- Readiness: `TCP :8110` · Liveness: `GET :8085/q/health/live`
...
- **Scale:** `kubectl scale rollout/sca-service -n sca --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: sca.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `sca.oncall` attestation — until that is live, escalate via the team channel).
