...
# Runbook — openbank-clearing-service

> Operational runbook for the `clearing` service. Data domain **payments**,
...
- **Scale:** `kubectl scale rollout/clearing-service -n payments --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth; a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: clearing.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `clearing.oncall` attestation — until that is live, escalate via the team channel).
