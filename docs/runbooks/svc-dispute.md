...
# Runbook — openbank-dispute-service

> Operational runbook for the `dispute` service. Data domain **compliance**,
...
> (`openbank-libs/governance/attestations.yaml: dispute.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `dispute.oncall` attestation — until that is live, escalate via the team channel).
