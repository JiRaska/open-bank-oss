...
# Runbook — openbank-wealth-service

> Operational runbook for the `wealth` service. Data domain **open-banking**,
...
| Field | Value |
|---|---|
| Service | `openbank-wealth-service` |
| HTTP port | `8154` |
| Data domain | open-banking |
| Datastore | PostgreSQL (database `openbank_wealth`) |
...
- **Downstream (depends on this service):** `customer-edge`

A failure here propagates to the downstream services above — check them when
triaging an incident that starts on `wealth`.

## Health & probes

- Readiness: `GET :8090/q/health/ready` · Liveness: `GET :8090/q/health/live`
- Metrics: scraped by the fleet PodMonitor (namespace `wealth`); dashboards in Grafana.
- Logs: `kubectl logs -n wealth deploy/wealth-service -f`, or Loki
  `{namespace="wealth"}`.

## Routine operations

- **Restart:** `kubectl rollout restart deploy/wealth-service -n wealth` (rolling, zero-downtime at >1 replica).
- **Scale:** `kubectl scale deploy/wealth-service -n wealth --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: wealth.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `wealth.oncall` attestation — until that is live, escalate via the team channel).
