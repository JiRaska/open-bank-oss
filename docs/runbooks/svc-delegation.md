...
# Runbook — openbank-delegation-service

> Operational runbook for the `delegation` service. Data domain **open-banking**,
...
| Field | Value |
|---|---|
| Service | `openbank-delegation-service` |
| HTTP port | `8126` |
...
- **Downstream (depends on this service):** `sca-service`, `pid-service`

A failure here propagates to the downstream services above — check them when
triaging an incident that starts on `delegation`.

## Health & probes

- Readiness: `GET :8090/q/health/ready` · Liveness: `GET :8090/q/health/live`
- Metrics: scraped by the fleet PodMonitor (namespace `delegation`); dashboards in Grafana.
- Logs: `kubectl logs -n delegation deploy/delegation-service -f`, or Loki
  `{namespace="delegation"}`.

## Routine operations

- **Restart:** `kubectl rollout restart deploy/delegation-service -n delegation` (rolling, zero-downtime at >1 replica).
- **Scale:** `kubectl scale deploy/delegation-service -n delegation --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth; a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: delegation.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `delegation.oncall` attestation — until that is live, escalate via the team channel).
