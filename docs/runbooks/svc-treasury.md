...
# Runbook — openbank-treasury-service

> Operational runbook for the `treasury` service. Data domain **core**,
...
| HTTP port | `8160` |
| Data domain | core |
| Datastore | PostgreSQL (database `openbank_treasury`) |
...
- **Downstream (depends on this service):** `ledger-service`, `audit-service`

A failure here propagates to the downstream services above — check them when
triaging an incident that starts on `treasury`.
...
- **Restart:** `kubectl rollout restart deploy/treasury-service -n treasury` (rolling, zero-downtime at >1 replica).
- **Scale:** `kubectl scale deploy/treasury-service -n treasury --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: treasury.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `treasury.oncall` attestation — until that is live, escalate via the team channel).
