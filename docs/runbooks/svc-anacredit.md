...
# Runbook — openbank-anacredit-service

> Operational runbook for the `anacredit` service. Data domain **compliance**,
...
triaging an incident that starts on `anacredit`.
...
> (`openbank-libs/governance/attestations.yaml: anacredit.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `anacredit.oncall` attestation — until that is live, escalate via the team channel).
