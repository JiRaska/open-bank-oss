...
# Runbook — openbank-pid-service

> Operational runbook for the `pid` service. Data domain **identity**,
...
| Field | Value |
|---|---|
| Service | `openbank-pid-service` |
| HTTP port | `8105` |
| Data domain | identity |
| Datastore | PostgreSQL (database `openbank_pid`) |
...
triaging an incident that starts on `pid`.

## Health & probes

- Readiness: `TCP :8105` · Liveness: `GET :8085/q/health/live`
- Metrics: scraped by the fleet PodMonitor (namespace `pid`); dashboards in Grafana.
- Logs: `kubectl logs -n pid deploy/pid-service -f`, or Loki
  `{namespace="pid"}`.

## Routine operations

- **Restart:** `kubectl rollout restart deploy/pid-service -n pid` (rolling, zero-downtime at >1 replica).
- **Scale:** `kubectl scale deploy/pid-service -n pid --replicas=<n>` (or edit the GitOps manifest — GitOps is source of truth, a later ArgoCD sync reconciles manual changes).
...
> (`openbank-libs/governance/attestations.yaml: pid.dr_drill`).

## Escalation & break-glass

- First responder: the owning squad's on-call (rotation tracked as the
  `pid.oncall` attestation — until that is live, escalate via the team channel).
