...
**Delivery note (2026-08-19).** Live on `main` while this ADR read `delivery-status: planned`: **39 production Kotlin file(s)** across `openbank-campaign-service`, `openbank-customer-edge`, `openbank-dispute-service`, `openbank-engagement-service` and others cite this ADR, naming its decision clause(s) `D1`, `D2`, `D3`, `D4`, `D5`, `D6` rather than referencing it as prior art. Found by the code->ADR evidence read described in #5708 (the inverse of the ADR->path probe #5669 deliberately rejected). `decision-status` is unchanged; the unbuilt remainder is triaged in #5708.
