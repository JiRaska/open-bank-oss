...
- **Fleet rollout** — ✅ Shipped (the 2026-06-30 "pending" was stale): Phase 2 = **17 REST pacts** committed fleet-wide + `pact-drift-check.yml` live (regenerates 14 services' consumer pacts on every relevant PR, fails on diff). Phase 3 = **7 Kafka/message pacts** with message consumer + provider (`MessageTestTarget`) verification. Remaining (why the header stays Partial): pitest gate advisory→block flip and its 7-of-~13 coverage, money-path `can-i-deploy`, and the swift pact excluded from the drift gate (#2404).
