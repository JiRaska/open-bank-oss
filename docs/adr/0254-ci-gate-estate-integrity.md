...
- **`ruleset-context-parity`** (enforced). Reads the required-status-check contexts
  ...
  This needs **no permission grant at all** — `GET .../rulesets` is world-readable
  on a public repository (verified with a bare unauthenticated `curl`, 200 with no
  `Authorization` header). The first draft assumed the private-repo requirement
  ("Administration" read) applied here and declared `permissions: administration:
  read` on the job; `administration` is not a valid workflow-permissions key at
  all, and GitHub cannot parse a workflow declaring one — it broke `ci.yml`
  outright (every push produced a zero-job failed run, "workflow file issue")
  before this was caught on the open PR. Corrected before merge; see
  `check-ruleset-context-parity.py`'s own header for the full story.
...
**Negative**
- None from `ruleset-context-parity` itself — see Decision: it needs no permission
  grant, the endpoint is public. The negative that DID materialize was in building
  it: an invalid `permissions:` key reached an open PR and broke `ci.yml`'s ability
  to run at all before review caught it, which is its own small lesson about
  verifying a permission claim against GitHub's actual behavior rather than
  general docs before shipping it.
