...
- **D2 (threat-model CI gate)** — ✅ Shipped: `check-threat-models.py` runs as a required check in `Validate manifests`; all 13 money-path services have STRIDE/DFD threat models at `docs/threat-models/<service>.md`; adding a money-path service without a threat model blocks merge. **Diff-aware rule ✅ Shipped** (PR #389): `check-threat-model-diff.py` flags a money-path service whose PR moves a trust boundary (inbound REST, outbound client edge, authn/transport `application.yaml` keys, or its gitops NetworkPolicy/Deployment) without a matching `docs/threat-models/<service>.md` update — **advisory** until the ADR-0144 target enforce date (2026-09-15), then a hard gate. The gate's own parser is now unit-tested (`check_threat_model_diff_test.py`) and the previously-unrun `openbank-infra/scripts/*_test.py` suites execute in CI.
...
- **D3 (mutation testing + DAST)** — 🟡 Partial: pitest (`pitest.yml`, weekly, advisory) now runs across 9 money-path services' domain layers, not just lending; DAST is deployed — `dast-zap-baseline.yml` (OWASP ZAP baseline, weekly, advisory, ledger-service target) and `api-fuzz.yml` (schemathesis). What remains: raise pitest/DAST from advisory to blocking per service once kill-scores/scan-clean sustain, and widen DAST beyond the single ledger-service target.
- **D5 (runtime SBOM drift detection)** — ⬜ Pending: each service self-publishes its build-time SBOM at `/q/openbank/sbom` (`SbomResource`), but nothing yet compares a *running* image's actual dependency/OS layer against its attested release SBOM over time (needs a scheduled cluster-side rescan, ADR-0121 Axis 2 adjacent).

Remaining D3 (blocking flips + DAST breadth) / D5, and the D2/D4 enforce flips (date- and observation-gated), tracked in issue #265.
