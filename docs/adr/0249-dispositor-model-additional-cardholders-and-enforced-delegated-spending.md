...
- **D1 (additional cardholder is a card)** — IMPLEMENTED. `POST` delegated-card issuance lands in PR #4194 (`25bd63177`, 2026-08-13): `Card.delegationGrantId`/`delegated`, the SCA-gated `cards.issue.delegated` route in `CustomerEdgeResource.kt`, and `DelegatedCardIssuanceTest.kt` (249 lines) + `DelegatedCardTest.kt` (448 lines). Since that verification, `openbank-app` added `CardsApi.issueDelegated` and `CardsScreen`'s additional-cardholder sheet with `ISSUE_DELEGATED` SCA. `DelegationPreset.kt` says only that choosing a card-view grant does not itself issue a card; issuance is a separate card flow.
...
- **D4 (SCA + audit on every grant, and notify on first use)** — PARTIAL. SCA is real (`scaCardGate(... "ISSUE_DELEGATED" ...)` in `CustomerEdgeResource.kt`) and delegated card issuance emits `CUSTOMER_DELEGATED_CARD_ISSUED`. The delegation transactional outbox publishes lifecycle and `SpendReserved`/`SpendConfirmed`/`SpendReleased` events. `DelegationNotificationConsumer` subscribes to those events; a `SpendConfirmed` from delegation-service requests a grantor `DELEGATION_FIRST_USE` notification with the grant id as its deduplication key. This proves the domestic-payment first-use path exists, not that every card or future payment rail reports first use. Do not infer absent notification from the lack of a synchronous `notification-service` call: this integration is event-driven.
...
**Current boundary (2026-09-17).** `delivery-status: partial` remains accurate. D1 is now reachable in the app; D3's reservation operation types and edge call sites cover delegated domestic payments, while SEPA and instant initiation still require account ownership. D4's event-driven first-use notification covers confirmed delegated spend on that domestic path; delegated-card first use is not yet evidenced by a card-authorization event. A shared ceiling across new payment rails must be wired before those rails accept a delegate, and notification coverage must be proven per newly enabled authority. `decision-status` remains `proposed`, independently of delivery status.
