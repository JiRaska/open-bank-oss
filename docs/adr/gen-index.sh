...
#   DIGEST.md    the whole decision history as one-line summaries (~4k tokens).
#                THIS is the file a reviewer, an auditor or an AI agent reads: the
#                full fleet is ~225k words, so nobody -- human or model -- ever loads
#                it, which made a 170+-ADR registry effectively unsearchable. The
...
# =============================== DIGEST.md ===================================
{
  ...
  echo
  echo "The ADR fleet is ~225k words. This file is the same history in ~4k tokens, so it can"
  echo "be read in full — by a reviewer, an auditor, or an AI agent — before deciding which"
