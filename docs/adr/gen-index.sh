...
#   CURRENT.md   the decisions that STAND today, grouped by domain tag. The digest
#                is history: it lists a superseded decision next to the one that
#                replaced it, and an accepted one next to a proposal, and leaves the
#                reader to resolve the `supersedes` chain in their head. This view
#                does that resolution once — superseded, deprecated and rejected ADRs
#                are dropped, proposals are marked, and each ADR appears in full
#                under its FIRST tag (the primary domain) with a pointer from its
#                other tags. Same data, no new source of truth.
#
# DERIVED DATA: never hand-edit any of the four. Edit the ADRs (or this script)
...
# =============================== CURRENT.md =================================
# Tag order follows tags.txt (domain first, process last) so the file reads as a map
# of the platform rather than an alphabet. tags.txt is comments + one tag per line.
tag_order=$(grep -v '^[[:space:]]*#' tags.txt | grep -v '^[[:space:]]*$')

# Standing = decision still in force. `proposed` is included and marked: it is not yet
# a decision, but it is the live conversation, and hiding it would make this view
# disagree with what is being built right now.
standing_rows=$(printf '%s' "$TABLE" | awk -F"$SEP" \
  'NF && ($4 == "accepted" || $4 == "proposed")')
standing=$(printf '%s' "$standing_rows" | grep -c . || true)
proposed=$(printf '%s' "$standing_rows" | awk -F"$SEP" '$4 == "proposed"' | grep -c . || true)
retired=$((count - standing))

{
  echo "# ADR current — the decisions that stand today, by domain"
  echo
  echo "_Generated by \`docs/adr/gen-index.sh\` — do not hand-edit._"
  echo
  echo "[DIGEST.md](DIGEST.md) is the whole history, one line per ADR, in the order decisions"
  echo "were made. This file is the same lines with the history resolved: ${retired} superseded,"
  echo "deprecated or rejected ADRs are omitted, and the ${standing} that stand are grouped by"
  echo "domain tag — ${proposed} of them still \`proposed\`, marked as such. Each ADR appears in"
  echo "full under its FIRST tag; its other tags carry a pointer. Read this to learn what the"
  echo "platform's rules ARE; read the digest to learn how they got that way."
  echo
  echo "Format: **number** · title · \`delivery\` · summary · _(supersedes …)_"
  echo
  for tag in $tag_order; do
    primary=$(printf '%s' "$standing_rows" | awk -F"$SEP" -v t="$tag" \
      '{ split($7, a, ","); if (a[1] == t) print }')
    secondary=$(printf '%s' "$standing_rows" | awk -F"$SEP" -v t="$tag" \
      '{ n = split($7, a, ","); for (i = 2; i <= n; i++) if (a[i] == t) print $1 }' \
      | sort | paste -sd' ' -)
    [ -z "$primary" ] && [ -z "$secondary" ] && continue
    echo "## $tag"
    echo
    printf '%s\n' "$primary" | while IFS="$SEP" read -r num f title decision delivery date tags repos sup supby summary; do
      [ -z "$num" ] && continue
      mark=""; [ "$decision" = "proposed" ] && mark=" _(proposed)_"
      chain=""; [ -n "$sup" ] && chain=" · _(supersedes ${sup//,/, })_"
      printf -- '- **[%s](%s)** · %s%s · `%s` · %s%s\n' \
        "$num" "$f" "$title" "$mark" "$delivery" "$summary" "$chain"
    done
    if [ -n "$secondary" ]; then
      echo
      echo "_Also tagged \`$tag\`: ${secondary}_"
    fi
    echo
  done
} > CURRENT.md
...
echo "Wrote README.md, DIGEST.md, CURRENT.md, index.json ($count ADRs indexed, $standing standing)."
