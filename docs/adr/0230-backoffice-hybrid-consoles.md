...
**Delivery note (2026-08-19).** Live on `main` while this ADR read `delivery-status: planned`: **9 production Kotlin file(s)** across `openbank-fraud-service`, `openbank-lending-service`, `openbank-sdd-service` cite this ADR, naming its decision clause(s) `D1` rather than referencing it as prior art. Found by the code->ADR evidence read described in #5708 (the inverse of the ADR->path probe #5669 deliberately rejected). `decision-status` is unchanged; the unbuilt remainder is triaged in #5708.
