...
| Independent review of AI-authored changes | **Withdrawn 2026-08-09 (#2161).** The agent-review workflow could not execute: GitHub Models was retired 2026-07-30 and answers HTTP 410, which set `ghmodels_retired=true`, and the Claude fallback was gated on `retired != 'true'` — so the fallback was disabled precisely when the primary failed. Measured over 10 consecutive runs: both the review step and the step named "Verify the Claude fallback actually reviewed" were `skipped` in 10/10, every run concluding `success`. The "defer to humans" half is also unbacked: the `main-protection` ruleset has `required_approving_review_count: 0` (#2183). | ADR-0154 (superseded); #2161; #2183 | **gap — control withdrawn, not implemented** |
