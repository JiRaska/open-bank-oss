...
class ProductCatalogService @Inject constructor(private val clock: Clock) {
    ...
    fun findAll(): List<Product> = store.values.sortedBy { it.code }

    // ADR-0105 — unified product identity. account-service references a product by UUID
    // (accounts.product_id); the catalogue is the authority that resolves that UUID to its
    // product. The two products an account is opened with (PartyEventConsumer defaults)
    // adopt those UUIDs as their canonical id; any other product gets a deterministic
    // derived UUID. Lookup resolves by store key (prod-NNN), code, or canonical UUID.
    private val canonicalProductIds: Map<String, String> = mapOf(
        "prod-014" to "00000000-0000-0000-0000-0000000000c2",
        "prod-010" to "00000000-0000-0000-0000-0000000000c3",
    )

    // Keyed by store id (prod-NNN), since that is the stable product identity account-service
    // references; the catalogue carries no UUID column of its own.
    fun canonicalId(productId: String): String =
        canonicalProductIds[productId] ?: UUID.nameUUIDFromBytes("openbank-product:$productId".toByteArray()).toString()

    fun findById(id: String): Product? = store[id] ?: store.values.find { it.code == id || canonicalId(it.id) == id }

    fun findByCode(code: String): Product? = store.values.find { it.code == code }

    fun create(req: ProductRequest): Product {
        if (store.values.any { it.code == req.code }) {
    ...
    fun activate(id: String): Product {
        val p = store[id] ?: throw NoSuchElementException("Product $id not found")
        val updated = p.copy(status = ProductStatus.ACTIVE, updatedAt = Instant.now(clock))
        store[id] = updated
        return updated
    }

    fun deactivate(id: String): Product {
        val p = store[id] ?: throw NoSuchElementException("Product $id not found")
        val updated = p.copy(status = ProductStatus.INACTIVE, updatedAt = Instant.now(clock))
        store[id] = updated
        return updated
    }

    /**
     * Flattens the per-product [Fee] model into a single fee schedule the admin UI
     * renders as the "Fees" pricing catalog. This is the catalog's own source of
     * truth for pricing — the UI must not hardcode fees. (DB-backed persistence of
