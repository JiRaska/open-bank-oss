...
/**
 * #10486 batch 5: document-service looks accounts up (`GET /api/v1/accounts`, `account.list`) as its
 * OWN principal, `service-account-openbank-document` (ROLE_API only).
 ...
 * `quarkus.oidc-client.m2m` must exist with client `openbank-document` and its own secret variable (a
...
class M2mOidcClientIdentityWiringTest {
    ...
    @Test
    fun `the named oidc-client authenticates as openbank-document with its own secret`() {
        val named = oidcClient[NAMED] as Map<*, *>
        assertThat(named["client-id"]).isEqualTo("openbank-document")
