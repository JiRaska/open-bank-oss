...
import com.openbank.authzaudit.PostgresTestResource
 ...
 *
 * Deliberately not a direct call to [AuthzPolicyCheckScheduler.runScheduledCheck]. A direct call proves the method
...
@TestProfile(AuthzPolicyCheckSchedulerCronIT.EverySecondProfile::class)
class AuthzPolicyCheckSchedulerCronIT {

    class EverySecondProfile : QuarkusTestProfile {
        override fun getConfigOverrides(): Map<String, String> = mapOf(
            // Literals only — see the class KDoc.
            "openbank.authz-policy-auditor.check-cron" to "*/1 * * * * ?",
            "openbank.temporal.enabled" to "false",
        )
    }

    @Inject
    lateinit var scheduler: AuthzPolicyCheckScheduler
    ...
    fun `the real cron fires the scheduled sweep`() {
        ...
        assertTrue(
            ...
            "The scheduled authz-policy-auditor sweep never fired within $TIMEOUT_SECONDS s. The cron " +
