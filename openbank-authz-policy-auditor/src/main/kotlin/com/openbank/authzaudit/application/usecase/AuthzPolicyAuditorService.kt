...
class AuthzPolicyAuditorService(
    ...
    override suspend fun startDetached(trigger: RunTrigger): String {
        ...
        val options = WorkflowOptions.newBuilder()
            ...
            .build()
        val stub = workflowClient.newWorkflowStub(AuthzPolicyAuditorWorkflow::class.java, options)
        try {
            WorkflowClient.start({ stub.runCheck(trigger) })
            log.infof("Started authz-policy-auditor sweep workflow %s (trigger=%s)", workflowId, trigger)
    ...
    companion object {
        ...
        fun scheduledWorkflowId(trigger: RunTrigger, at: Instant): String =
            "authz-policy-auditor-check-${trigger.name.lowercase()}-${DAY.format(at)}"
    }
}
