...
/**
 * The periodic sweep ADR-0167 decided and nothing ever built.
 *
 * ADR-0167 chose the cadence and not the hour: "Runs on a weekly sweep plus a reactive trigger". The weekly half
 * is what catches a rule that became unreachable because the INTERCEPTOR's vocabulary moved — a change in a file
 * the reactive trigger does not watch.
 *
 * `RunTrigger.SCHEDULED` has existed in this agent's domain model since the service was written, and nothing ever
 * emitted it. The agent was trigger-only (`POST /api/v1/authz-policy-auditor/check/trigger`), so it had checked
 ...
 */
@ApplicationScoped
class AuthzPolicyCheckScheduler {

    @Inject
    lateinit var runCheck: RunAuthzPolicyCheckUseCase
    ...
    private val log = Logger.getLogger(AuthzPolicyCheckScheduler::class.java)
    ...
    @Scheduled(
        cron = "{openbank.authz-policy-auditor.check-cron}",
        concurrentExecution = Scheduled.ConcurrentExecution.SKIP,
    )
    suspend fun runScheduledCheck() {
        fires.incrementAndGet()
        if (!temporalConfig.enabled()) {
            log.info("Temporal disabled; skipping the scheduled authz-policy-auditor sweep")
            return
        }
        val workflowId = runCheck.startDetached(RunTrigger.SCHEDULED)
        liveness?.recordSuccess()
        log.infof("Scheduled authz-policy-auditor sweep dispatched as workflow %s", workflowId)
    }

    private companion object {
        const val WORKFLOW_NAME = "authz-policy-check"
