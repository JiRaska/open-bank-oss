 ...
 *
 * Sufficient for the initial scaffolding phase (ADR-0167), matching the finops-agent/devops-agent/
 * control-liveness-sentinel/governance-auditor/release-steward/docs-truth-agent bootstrap pattern.
...
class InMemoryFindingRepository : FindingRepository {
    ...
    private val store = ConcurrentHashMap<String, AuthzPolicyFinding>()
    ...
    override suspend fun findActive(): List<AuthzPolicyFinding> =
        store.values.filter { it.status !in setOf(FindingStatus.RESOLVED, FindingStatus.REJECTED) }

    override suspend fun findById(id: String): AuthzPolicyFinding? = store[id]
