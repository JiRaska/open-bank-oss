...
/** Four-eyes record of one void of a backfill's synthetic loans (#10969, V20). Every column named explicitly. */
@Entity
@Table(name = "ledger_backfill_void_request")
class LedgerBackfillVoidRequestEntity : PanacheEntityBase() {
    @Id
    ...
    /** The EXECUTED backfill request whose loans this void undoes. */
    @Column(name = "source_request_id", columnDefinition = "uuid")
    var sourceRequestId: UUID = Ids.newId()

    /** The open accounting day the mirror journals are booked into. */
    @Column(name = "void_date")
    var voidDate: LocalDate = LocalDate.EPOCH
