 ...
 * defaults.
 *
 * Both are needed for [com.openbank.lending.integration.LendingBootSmokeIT] to reach a green
 * `/q/health/ready`: Hibernate Reactive + Flyway need a real Postgres (the V1/V2/V3 migrations
 * run on boot), and the redis-client extension contributes a readiness health check, so a real
 * Valkey is needed too. The single `@Channel("lending-events-out")` Kafka emitter is switched to
 * the in-memory connector in the IT, so no broker container is required (and the default
 * `lending.ledger.backend=none` keeps the no-op ledger adapter bound, so the IT boots offline).
 *
...
class PostgresRedisTestResource : QuarkusTestResourceLifecycleManager {
    ...
    override fun start(): Map<String, String> {
        ...
        val pg = PostgreSQLContainer(DockerImageName.parse("postgres:16.3-alpine"))
            ...
            .withDatabaseName("openbank_lending_it")
        pg.start()
        postgres = pg
        ...
        return mapOf(
            "quarkus.datasource.reactive.url" to "vertx-reactive:postgresql://$pgHost:$pgPort/openbank_lending_it",
            "quarkus.datasource.jdbc.url" to "jdbc:postgresql://$pgHost:$pgPort/openbank_lending_it",
