 ...
 */
class LendingOutboxBacklogGaugeTest {

    @Test
    fun `registers a lending backlog supplier whose value tracks the refreshed cache`(): Unit = runBlocking {
        val repo = mockk<LendingOutboxRepositoryImpl>()
        val metrics = mockk<DomainMetrics>(relaxed = true)
        val supplier = slot<() -> Number>()
        every { metrics.registerOutboxBacklog(eq("lending"), capture(supplier)) } returns Unit

        val gauge = LendingOutboxBacklogGauge(repo, metrics)
        gauge.register()
