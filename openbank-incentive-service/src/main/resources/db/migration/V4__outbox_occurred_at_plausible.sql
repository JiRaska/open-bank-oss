...
-- all real work in the dispatcher's ORDER BY occurred_at ASC and lands in the 1970-01 warehouse
...
--   SELECT count(*) FROM incentive_outbox WHERE occurred_at < TIMESTAMPTZ '2020-01-01';
--
-- Rollback: ALTER TABLE incentive_outbox DROP CONSTRAINT IF EXISTS incentive_outbox_occurred_at_plausible;
ALTER TABLE incentive_outbox
    ADD CONSTRAINT incentive_outbox_occurred_at_plausible
