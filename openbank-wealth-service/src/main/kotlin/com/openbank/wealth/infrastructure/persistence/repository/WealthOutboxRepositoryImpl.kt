...
@ApplicationScoped
class WealthOutboxRepositoryImpl(private val clock: Clock) :
    ...
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, WealthOutboxEntity::class.java)
    ...
    private fun WealthOutboxEntity.toEntry() = OutboxEntry(
        eventId = eventId,
    ...
    companion object {
        private val CLAIM_SQL = """
            UPDATE wealth_outbox AS claimed
            SET status = :dispatching, claimed_at = :now, updated_at = :now
            WHERE claimed.id IN (
                SELECT candidate.id FROM wealth_outbox AS candidate
