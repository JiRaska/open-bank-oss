...
import com.openbank.consent.infrastructure.persistence.entity.ConsentOutboxEntity
...
@ApplicationScoped
class ConsentOutboxRepositoryImpl(private val clock: Clock) :
    ConsentOutboxRepository,
    PanacheRepository<ConsentOutboxEntity> {
    ...
    override suspend fun claimProcessable(limit: Int, staleAfter: Duration): List<OutboxEntry> {
        ...
        return Panache.withTransaction {
            Panache.getSession().chain { session ->
                session.createNativeQuery(CLAIM_SQL, ConsentOutboxEntity::class.java)
