...
/**
 * Publishes the consent outbox **backlog** (PENDING + FAILED rows) as the `openbank.outbox.backlog`
 * gauge tagged `service="consent"` (ADR-0077 / ADR-0079). A rising backlog means consent lifecycle
...
@Startup
@ApplicationScoped
class ConsentOutboxBacklogGauge : AbstractOutboxBacklogGauge {
    private lateinit var outboxRepository: ConsentOutboxRepository

    @Inject
    constructor(outboxRepository: ConsentOutboxRepository, metrics: DomainMetrics) : super(metrics) {
    ...
    override val service: String = "consent"
