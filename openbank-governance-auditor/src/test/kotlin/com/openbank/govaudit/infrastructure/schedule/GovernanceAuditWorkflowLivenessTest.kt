...
class GovernanceAuditWorkflowLivenessTest {
    ...
    fun `scheduler registers gauge at startup and records success after workflow dispatch`(): Unit = runBlocking {
        ...
        val runAudit = mockk<RunGovernanceAuditUseCase>()
        val temporalConfig = mockk<TemporalConfig>()
        every { temporalConfig.enabled() } returns true
        coEvery { runAudit.startDetached(RunTrigger.SCHEDULED) } returns "wf-1"

        val scheduler = GovernanceAuditScheduler().apply {
    ...
    fun `failed dispatch leaves liveness old`() {
        ...
        val temporalConfig = mockk<TemporalConfig>()
        every { temporalConfig.enabled() } returns true
        coEvery { runAudit.startDetached(RunTrigger.SCHEDULED) } throws IllegalStateException("temporal down")
        ...
        assertThatThrownBy { runBlocking { scheduler.runScheduledAudit() } }
    ...
    private companion object {
        const val WORKFLOW = "governance-audit"
