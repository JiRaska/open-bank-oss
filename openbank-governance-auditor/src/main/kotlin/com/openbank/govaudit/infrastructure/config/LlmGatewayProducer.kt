...
/**
 * Produces the shared [LlmGatewayPort] (ADR-0174) for governance-auditor, wired to the same
 * LiteLLM gateway endpoint + model the gitops config points at.
 *
...
class LlmGatewayProducer {
    ...
    @Produces
    @ApplicationScoped
    fun llmGateway(config: GovernanceAuditorConfig, metrics: LlmCallMetrics): LlmGatewayPort {
        val apiKey = ConfigProvider.getConfig()
            .getOptionalValue("governance-auditor.model.api-key", String::class.java)
            .orElse("")
